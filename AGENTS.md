...
## What Seple T Agent Is

Seple T Agent is a personal AI agent that runs the same agent core across a CLI, a
...
Read the balance right: Seple T Agent ships a **lot** — most merges are bug fixes to
...
- **Expand reach at the edges.** New platform adapters, channels, providers,
  ...
  (`Seple T Agent tools`, `Seple T Agent setup`, auto-install) rather than bolting on a raw
  env var.
...
- **Outbound telemetry / usage attribution without opt-in gating.** No new
  ...
  generic user-facing opt-in (config gate + setup prompt + `Seple T Agent tools`
...
- **Third-party products / other people's projects integrated into the core
  ...
  **standalone plugin repo** users install into `~/.Seple T Agent/plugins/` (or via a
...
2. **CLI command + skill** — manages config/state/infra expressible as shell
   commands. The agent runs `Seple T Agent <subcommand>` guided by a skill. Zero
   model-tool footprint. Default choice for subscriptions, scheduled tasks,
   service setup. Examples: `Seple T Agent webhook`, `Seple T Agent cron`, `Seple T Agent tools`.
...
4. **Plugin** — third-party/niche/user-specific capability that doesn't ship in
   core. Lives in `~/.Seple T Agent/plugins/` or a pip package, discovered at runtime.
...
`$HOME/.Seple T Agent/hermes-agent/venv` (for worktrees that share a venv with the
main checkout).
...
├── ui-tui/               # Ink (React) terminal UI — `Seple T Agent --tui`
...
**User config:** `~/.Seple T Agent/config.yaml` (settings), `~/.Seple T Agent/.env` (API keys only).
**Logs:** `~/.Seple T Agent/logs/` — `agent.log` (INFO+), `errors.log` (WARNING+),
`gateway.log` when running the gateway. Profile-aware via `get_hermes_home()`.
Browse with `Seple T Agent logs [--follow] [--level ...] [--session ...]`.

## TypeScript Style

Applies to TypeScript across Seple T Agent: desktop, TUI, website, and future TS packages.
...
- Skill slash commands: `agent/skill_commands.py` scans `~/.Seple T Agent/skills/`, injects as **user message** (not system prompt) to preserve prompt caching
...
- **Slack** — `slack_subcommand_map()` generates `/Seple T Agent` subcommand routing
...
The TUI is a full replacement for the classic (prompt_toolkit) CLI, activated via `Seple T Agent --tui` or `HERMES_TUI=1`.
...
### TUI in the Dashboard (`Seple T Agent dashboard` → `/chat`)

The dashboard embeds the real `Seple T Agent --tui` — **not** a rewrite.  See `hermes_cli/pty_bridge.py` + the `@app.websocket("/api/pty")` endpoint in `hermes_cli/web_server.py`.

- Browser loads `web/src/pages/ChatPage.tsx`, which mounts xterm.js's `Terminal` with the WebGL renderer, `@xterm/addon-fit` for container-driven resize, and `@xterm/addon-unicode11` for modern wide-character widths.
- `/api/pty?token=…` upgrades to a WebSocket; auth uses the same ephemeral `_SESSION_TOKEN` as REST, via query param (browsers can't set `Authorization` on WS upgrade).
- The server spawns whatever `Seple T Agent --tui` would spawn, through `ptyprocess` (POSIX PTY — WSL works, native Windows does not).
- Frames: raw PTY bytes each direction; resize via `\x1b[RESIZE:<cols>;<rows>]` intercepted on the server and applied with `TIOCSWINSZ`.

**Do not re-implement the primary chat experience in React.** The main transcript, composer/input flow (including slash-command behavior), and PTY-backed terminal belong to the embedded `Seple T Agent --tui` — anything new you add to Ink shows up in the dashboard automatically. If you find yourself rebuilding the transcript or composer for the dashboard, stop and extend Ink instead.

**Structured React UI around the TUI is allowed when it is not a second chat surface.** Sidebar widgets, inspectors, summaries, status panels, and similar supporting views (e.g. `ChatSidebar`, `ModelPickerDialog`, `ToolCall`) are fine when they complement the embedded TUI rather than replacing the transcript / composer / terminal. Keep their state independent of the PTY child's session and surface their failures non-destructively so the terminal pane keeps working unimpaired.

### Electron Desktop Chat App (`apps/desktop/`)

A **separate** chat surface from both the classic CLI and the dashboard's embedded TUI. It is an Electron + React + nanostore renderer (`@assistant-ui/react`) that talks to a `tui_gateway` backend over JSON-RPC (`requestGateway(method, params)`). The WebSocket/JSON-RPC transport lives in the framework-agnostic `apps/shared` package (`@Seple T Agent/shared` — `JsonRpcGatewayClient` + WS URL helpers), which the web dashboard (`web/`) also consumes; **desktop has no build/runtime dependency on the dashboard frontend** — it spawns a headless `Seple T Agent serve` backend server (the same gateway `dashboard` serves, minus the browser UI entirely: `serve` sets `headless_backend=True`, so `cmd_dashboard` skips `_build_web_ui` AND exports `HERMES_SERVE_HEADLESS=1` so `mount_spa()` disables the SPA even if a stray `web_dist/` exists — only the JSON-RPC/WS/API surface is reachable). `dashboard` and `serve` share `cmd_dashboard`/`start_server` but are independent surfaces — neither launches the other. The one exception is a backward-compat *fallback*: `serve` is newer, so the desktop spawn (`electron/backend-command.ts` + `backendSupportsServe()` in `electron/main.ts`) detects whether the resolved runtime registers `serve` and, only when it does not (an older managed install / PATH `Seple T Agent` the app hasn't updated yet), rewrites the argv to the legacy `dashboard --no-open`. Without that, a new app against an un-upgraded runtime would crash on an unknown subcommand and brick every mid-upgrade user. It does NOT embed `Seple T Agent --tui` — it has its own composer, transcript, and slash-command pipeline. For scoped Desktop architecture, state, resolver, transport, and testing rules, read `apps/desktop/AGENTS.md`.
...
- **The renderer curates via `apps/desktop/src/lib/desktop-slash-commands.ts`.** This is the load-bearing file. It holds `DESKTOP_COMMAND_SPECS` (the built-ins and their Desktop surfaces) plus `NO_DESKTOP_SURFACE` block-lists for terminal-only / messaging-only / picker-owned / settings-owned / advanced commands that should NOT clutter the desktop popover.
  ...
  - `isDesktopSlashExtensionCommand(name)` — true when the command is NOT a known Seple T Agent built-in (i.e. a skill or user quick command). Both suggestion and catalog-filter paths allow extensions through so skill commands surface in the palette. (Added when fixing "skill commands missing from the desktop slash palette" — the curated allow-list was silently dropping every skill/quick command from completions even though they executed fine when typed.)
...
be core tools. For custom or local-only tools, do **not** edit Seple T Agent core.
Use the plugin route instead: create `~/.Seple T Agent/plugins/<name>/plugin.yaml`
and `~/.Seple T Agent/plugins/<name>/__init__.py`, then register tools with
`ctx.register_tool(...)`. Plugin toolsets are discovered automatically and can be
enabled or disabled without touching `tools/` or `toolsets.py`.

Use the built-in route below only when the user is explicitly contributing a new
core Seple T Agent tool that should ship in the base system.
...
**State files**: If a tool stores persistent state (caches, logs, checkpoints), use `get_hermes_home()` for the base directory — never `Path.home() / ".Seple T Agent"`. This ensures each profile gets its own state.
...
| `load_config()` | `Seple T Agent tools`, `Seple T Agent setup`, most CLI subcommands | `hermes_cli/config.py` — merges `DEFAULT_CONFIG` + user YAML |
...
~/.Seple T Agent/skins/*.yaml       # User-installed custom skins (drop-in)
```
...
- `default` — Classic Seple T Agent gold/kawaii (the current look)
...
Users create `~/.Seple T Agent/skins/<name>.yaml`:
...
Seple T Agent has two plugin surfaces. Both live under `plugins/` in the repo so
repo-shipped plugins can be discovered alongside user-installed ones in
`~/.Seple T Agent/plugins/` and pip-installed entry points.

### General plugins (`hermes_cli/plugins.py` + `plugins/<name>/`)

`PluginManager` discovers plugins from `~/.Seple T Agent/plugins/`, `./.Seple T Agent/plugins/`,
...
- Register CLI subcommands via `ctx.register_cli_command(...)` — the
  plugin's argparse tree is wired into `Seple T Agent` at startup so
  `Seple T Agent <pluginname> <subcmd>` works with no change to `main.py`
...
it at argparse setup time and wires it into `Seple T Agent <plugin>`. The
framework only exposes CLI commands for the **currently active** memory
provider (read from `memory.provider` in config.yaml), so disabled
providers don't clutter `Seple T Agent --help`.
...
into `~/.Seple T Agent/plugins/` (or via pip entry points) — they implement
the same `MemoryProvider` ABC, register through the same discovery
path, and integrate via `Seple T Agent memory setup` / `post_setup()` without
...
`~/.Seple T Agent/plugins/` (or via pip entry points). They register through
...
- **`optional-skills/`** — heavier or niche skills shipped with the repo but
  ...
  `Seple T Agent skills install official/<category>/<skill>`. Adapter lives in
...
`metadata.Seple T Agent.tags`, `metadata.Seple T Agent.category`,
`metadata.Seple T Agent.related_skills`, `metadata.Seple T Agent.config` (config.yaml
settings the skill needs — stored under `skills.config.<key>`, prompted
during setup, injected at load time).

Top-level `tags:` and `category:` are also accepted and mirrored from
`metadata.Seple T Agent.*` by the loader.
...
2. **Tools referenced in SKILL.md prose must be native Seple T Agent tools or
...
4. **`author` credits the human contributor first.** For external
   ...
   first; "Seple T Agent" is the secondary collaborator. If the
   contributor's commit shows "Seple T Agent" as author (because they
   used Seple T Agent to draft the skill), replace it with their actual name
...
Enable/disable per platform via `Seple T Agent tools` (the curses UI) or the
...
Seple T Agent returns a delegation id immediately and the result re-enters the
...
go to `~/.Seple T Agent/skills/.archive/` and are restorable.

- **Core:** `agent/curator.py` (review loop, auto-transitions, LLM review
  prompt) + `agent/curator_backup.py` (pre-run tar.gz snapshots).
- **CLI:** `hermes_cli/curator.py` wires `Seple T Agent curator <verb>` where
  verbs are: `status`, `run`, `pause`, `resume`, `pin`, `unpin`,
  `archive`, `restore`, `prune`, `backup`, `rollback`.
- **Telemetry:** `tools/skill_usage.py` owns the sidecar
  `~/.Seple T Agent/skills/.usage.json` — per-skill `use_count`, `view_count`,
...
schedule jobs via the `cronjob` tool; users via `Seple T Agent cron <verb>`
...
- File lock at `~/.Seple T Agent/cron/.tick.lock` prevents duplicate ticks
  across processes.
...
collaborate on shared tasks. Users drive it via `Seple T Agent kanban <verb>`;
workers spawned by the dispatcher drive it via a dedicated `kanban_*`
toolset so their schema footprint is zero when they're not inside a
kanban task.

- **CLI:** `hermes_cli/kanban.py` wires `Seple T Agent kanban` with verbs
...
Seple T Agent supports **profiles** — multiple fully isolated instances, each with its own
...
1. **Use `get_hermes_home()` for all HERMES_HOME paths.** Import from `hermes_constants`.
   NEVER hardcode `~/.Seple T Agent` or `Path.home() / ".Seple T Agent"` in code that reads/writes state.
   ```python
   # GOOD
   ...
   # BAD — breaks profiles
   config_path = Path.home() / ".Seple T Agent" / "config.yaml"
   ```

2. **Use `display_hermes_home()` for user-facing messages.** Import from `hermes_constants`.
   This returns `~/.Seple T Agent` for default or `~/.Seple T Agent/profiles/<name>` for profiles.
   ```python
   # GOOD
   ...
   print("Config saved to ~/.Seple T Agent/config.yaml")
   ```

3. **Module-level constants are fine** — they cache `get_hermes_home()` at import time,
   which is AFTER `_apply_profile_override()` sets the env var. Just use `get_hermes_home()`,
   not `Path.home() / ".Seple T Agent"`.

4. **Tests that mock `Path.home()` must also set `HERMES_HOME`** — since code now uses
   `get_hermes_home()` (reads env var), not `Path.home() / ".Seple T Agent"`:
   ```python
   with patch.object(Path, "home", return_value=tmp_path), \
        patch.dict(os.environ, {"HERMES_HOME": str(tmp_path / ".Seple T Agent")}):
       ...
   ```
...
6. **Profile operations are HOME-anchored, not HERMES_HOME-anchored** — `_get_profiles_root()`
   returns `Path.home() / ".Seple T Agent" / "profiles"`, NOT `get_hermes_home() / "profiles"`.
   This is intentional — it lets `Seple T Agent -p coder profile list` see all profiles regardless
   of which one is active.

## Known Pitfalls

### DO NOT hardcode `~/.Seple T Agent` paths
Use `get_hermes_home()` from `hermes_constants` for code paths. Use `display_hermes_home()`
for user-facing print/log messages. Hardcoding `~/.Seple T Agent` breaks profiles — each profile
...
### Tests must not write to `~/.Seple T Agent/`
The `_isolate_hermes_home` autouse fixture in `tests/conftest.py` redirects `HERMES_HOME` to a temp dir. Never hardcode `~/.Seple T Agent/` paths in tests.
...
| HOME / `~/.Seple T Agent/` | Your real config+auth.json                  | Temp dir per test                         |
