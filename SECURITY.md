# Seple T Agent Security Policy

This document describes Seple T Agent's trust model, names the one
...
security vulnerabilities. **Seple T Agent does not operate a bug
bounty program.**
...
- Environment details (`Seple T Agent version`, commit SHA, OS, Python
  version).
...
Seple T Agent is a single-tenant personal agent. Its posture is
layered, and the layers are not equally load-bearing. Reporters and
operators should reason about them in the same terms.

### 2.1 Definitions

- **Agent process.** The Python interpreter running Seple T Agent,
...
- **Trust envelope.** The set of resources an operator has implicitly
  granted Seple T Agent access to by running it — typically, whatever
  the operator's own user account can reach on the host.
- **Stance.** An explicit statement in Seple T Agent's documentation
...
Seple T Agent supports two OS-level isolation postures. They address
...
Seple T Agent supports this in two ways:

- **Seple T Agent's own Docker image and Compose setup.** Lighter-
...
Under a whole-process wrapper, Seple T Agent's in-process heuristics
...
Seple T Agent filters the environment it passes to its lower-trust
...
A malicious or buggy plugin is not a vulnerability in Seple T Agent
itself. Bugs in Seple T Agent's plugin-install or plugin-discovery
...
4. **Within the authorized set, all callers are equally trusted.**
   Seple T Agent does not model per-caller capabilities inside a
...
- Trust-model documentation violations: code behaving contrary to
  what this policy, Seple T Agent's own documentation, or reasonable
  operator expectations would predict — including cases where
  Seple T Agent has documented a stance about how its output should
...
- **Community-contributed skills and plugins.** Third-party skills
  ...
  plugins are in the operator's review surface, not Seple T Agent's
  trust surface (§2.4, §2.5). A skill or plugin doing something
  malicious is the expected failure mode of one that wasn't
  reviewed, not a vulnerability in Seple T Agent. Bugs in Seple T Agent
...
- Seple T Agent includes supply-chain guards for MCP server
