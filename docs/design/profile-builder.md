...
Supersedes: PR #31781 (prompt_toolkit `Seple T Agent profile wizard`)

## Why this, not the CLI wizard

PR #31781 added a keyboard-driven `Seple T Agent profile wizard` in the terminal.
...
A profile = a full `~/.Seple T Agent/profiles/<name>/` directory with its own:
...
5. spawn `Seple T Agent -p <profile> skills install <id>` per hub skill (async, returns PIDs)
