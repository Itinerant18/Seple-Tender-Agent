# Seple T Agent Middleware

Seple T Agent middleware is the behavior-changing companion to observer hooks.
...
selection, or handoff to runtimes such as NeMo Relay without changing Seple T Agent'
planner, model provider adapters, tool registry, memory, or CLI UX.

With middleware enabled, plugins can:

- Rewrite LLM provider request kwargs before Seple T Agent calls the provider.
- Rewrite tool arguments before guardrails, approval checks, hooks, and tool
  execution see them.
- Wrap the actual LLM execution callback while preserving Seple T Agent retry,
  streaming, interrupt, and hook behavior.
- Wrap the actual tool execution callback while preserving Seple T Agent guardrails,
...
- `telemetry_schema_version`: currently `Seple T Agent.observer.v1`
- `middleware_schema_version`: currently `Seple T Agent.middleware.v1`
...
Seple T Agent stores those trace entries in later observer hook payloads as
`middleware_trace`.
...
If multiple plugins register the same execution middleware kind, Seple T Agent runs
them as a nested chain in registration order. Middleware failures are fail-open:
Seple T Agent logs a warning and continues with the next middleware or the base
runtime path.
...
For each provider request, Seple T Agent applies middleware in this order:
...
For each tool call, Seple T Agent applies middleware in this order:

1. Parse and coerce model-provided tool arguments.
2. Apply `tool_request` middleware.
3. Run the normal Seple T Agent pre-execution path against the effective arguments:
...
```bash
Seple T Agent plugins enable <plugin-name>
```
...
mkdir -p "$HERMES_HOME"
Seple T Agent plugins enable <plugin-name>
Seple T Agent chat --query 'Reply exactly ok'
```
...
```bash
uv sync
uv run Seple T Agent plugins enable <plugin-name>
uv run Seple T Agent chat --query 'Reply exactly ok'
```
...
Return the same response shape Seple T Agent expects from the provider adapter. Do not
...
- If execution middleware raises before calling `next_call(...)`, Seple T Agent treats
...
- If execution middleware calls `next_call(...)` successfully and then raises
  during post-processing, Seple T Agent preserves the downstream result and does not
  run the provider or tool a second time.
- If downstream provider or tool execution fails, middleware may let that error
  propagate or translate it deliberately. Seple T Agent does not convert downstream
