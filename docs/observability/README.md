# Seple T Agent Observer Hooks

Seple T Agent observer hooks are the read-only telemetry contract for plugins that
...
They do not replace Seple T Agent' planner, model providers, memory, tool registry,
...
```text
telemetry_schema_version = "Seple T Agent.observer.v1"
```

Hook callbacks are fail-open. Seple T Agent catches callback exceptions, logs a
...
payload construction is gated behind `has_hook(...)`, so Seple T Agent only builds
