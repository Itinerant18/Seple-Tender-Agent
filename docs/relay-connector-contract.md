...
This document is the formal interface between the **Seple T Agent gateway** (Python,
`gateway/relay/`) and the **connector** (Node/TypeScript,
`Novaedge/gateway-gateway`). The connector implementer's first action is to
read this file.
...
- **Registration.** The gateway registers a **wake URL** at enroll/provision —
  ...
  a dashboard host). Self-hosted: `Seple T Agent gateway enroll --wake-url <url>` (or
...
**per-gateway secret** (`Seple T Agent gateway enroll` → connector `/relay/enroll`, or
...
(`Novaedge/gateway-gateway`); this section is the gateway-facing summary.
...
driven by the managed Portal / `Seple T Agent` CLI.
