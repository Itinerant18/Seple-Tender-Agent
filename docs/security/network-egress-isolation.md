...
When running Seple T Agent inside Docker, the default `network_mode: host` gives the
...
The Seple T Agent [SECURITY.md](../../SECURITY.md) §2 defines the trust model. The
...
services:
  gateway:
    ...
    environment:
      ...
      - NO_PROXY=Seple T Agent,hermes-dashboard,localhost
...
- [SECURITY.md](../../SECURITY.md) — Seple T Agent trust model and vulnerability reporting
