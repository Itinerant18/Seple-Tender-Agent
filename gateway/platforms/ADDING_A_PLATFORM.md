...
There are two ways to add a platform to the Seple T Agent gateway:

## Plugin Path (Recommended for Community/Third-Party)

Create a plugin directory in `~/.Seple T Agent/plugins/` (or under `plugins/platforms/`
...
**zero changes to core Seple T Agent code**.
...
- `env_enablement_fn: () -> Optional[dict]` — seeds `PlatformConfig.extra`
  ...
  `Seple T Agent gateway status` or `get_connected_platforms()` until the SDK
  instantiates.
...
Checklist for integrating a platform directly into the Seple T Agent core.
