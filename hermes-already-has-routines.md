# Seple T Agent Has Had "Routines" Since March
...
Seple T Agent equivalent — works today:
```bash
...
Seple T Agent equivalent — works today:
```bash
Seple T Agent webhook subscribe auth-watch \
  --events "pull_request" \
...
Every use case in their blog post — backlog triage, docs drift, deploy verification, alert correlation, library porting, bespoke PR review — has a working Seple T Agent implementation. No new features needed. It's been shipping since March 2026.

---

## What's Different

| | Claude Code Routines | Seple T Agent |
|---|---|---|
...
## Things Seple T Agent Does That Routines Can't
...
```bash
Seple T Agent cron create "every 1h" \
  "If CHANGE DETECTED, summarize what changed. If NO_CHANGE, respond with [SILENT]." \
  --script ~/.Seple T Agent/scripts/watch-site.py \
...
Seple T Agent has no daily limit. Run 500 automations a day if you want. The only constraint is your API budget, and you choose which models to use for which tasks.
...
Seple T Agent is open source and free. The automation infrastructure — cron scheduler, webhook platform, skill system, multi-platform delivery — is built in.
...
```bash
Seple T Agent gateway setup    # enable webhooks
Seple T Agent webhook subscribe pr-review \
  --events "pull_request" \
...
*Seple T Agent is built by [Nous Research](https://nousresearch.com). Open source, model-agnostic, runs on your infrastructure.*
