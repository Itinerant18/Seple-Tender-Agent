...
the actual `setup.sh` that creates the project workspace, configures Seple T Agent
...
2. **Create profiles** — `Seple T Agent profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.Seple T Agent/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `Seple T Agent kanban create` assigned to the director
...
```bash
Seple T Agent profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.Seple T Agent/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.Seple T Agent/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Seple T Agent
...
Each profile gets a `SOUL.md` at `~/.Seple T Agent/profiles/<name>/SOUL.md` that
...
```bash
Seple T Agent kanban create "Direct production of <video title>" \
    --assignee director \
...
the Seple T Agent `.env` (`${HERMES_HOME:-$HOME/.Seple T Agent}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hermes_env="${HERMES_HOME:-$HOME/.Seple T Agent}/.env"
...
check_key ELEVENLABS_API_KEY Seple T Agent ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY Seple T Agent OPENROUTER_API_KEY || exit 1
# ...
```
