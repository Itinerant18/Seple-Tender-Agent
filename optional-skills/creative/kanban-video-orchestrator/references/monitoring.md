...
Seple T Agent kanban watch --tenant <project-slug>

# Snapshot of the board
Seple T Agent kanban list --tenant <project-slug>
Seple T Agent kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
Seple T Agent kanban stats --tenant <project-slug>

# Visual dashboard (browser)
Seple T Agent dashboard

# Inspect a specific task (includes comments + events)
Seple T Agent kanban show <task-id>

# Follow a single task's event stream
Seple T Agent kanban tail <task-id>
```

Verify available subcommands with `Seple T Agent kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `Seple T Agent kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `Seple T Agent kanban block <id>` then `Seple T Agent kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `Seple T Agent kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `Seple T Agent kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `Seple T Agent profile list` (profile exists?), `Seple T Agent status` (gateway/dispatcher up?), and verify tenant. |
...
Seple T Agent kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
Seple T Agent kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(Seple T Agent kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
Seple T Agent kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
Seple T Agent kanban block <task-id>
Seple T Agent kanban archive <task-id>

# Diagnose what happened
Seple T Agent kanban show <task-id>      # task body, comments, recent events
Seple T Agent kanban tail <task-id>      # follow the live event stream
Seple T Agent kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `Seple T Agent kanban create` for the director
...
while true; do
    clear
    Seple T Agent kanban list --tenant <slug>
    echo "---"
    Seple T Agent kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `Seple T Agent kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `Seple T Agent profile list` and start
