...
Maps each role archetype to the Seple T Agent skills it should `always_load` and the
...
## Seple T Agent skills relevant to video production
...
These are **not** Seple T Agent skills but external CLIs / APIs that profiles invoke.
...
## Built-in Seple T Agent tools for media review

These are native Seple T Agent tools — not invoked via terminal but through their own
...
key is present in `${HERMES_HOME:-~/.Seple T Agent}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Seple T Agent profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${HERMES_HOME:-~/.Seple T Agent}/.env` → environment variable.
