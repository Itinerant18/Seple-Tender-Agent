...
## 5. Seple T Agent Configuration

### Config location

`$HERMES_HOME/config.yaml` (defaults to `~/.Seple T Agent/config.yaml` when `HERMES_HOME` is unset)
...
Restart the Seple T Agent session for changes to take effect. The MCP connection is
