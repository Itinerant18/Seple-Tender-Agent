...
> **Docker HOME pitfall:** In the official Seple T Agent Docker layout, `/opt/data` is `HERMES_HOME`, but Seple T Agent tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Hermes-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
