...
def test_pyproject_defines_termux_all_without_known_blockers() -> None:
    ...
    assert '"seple-t-agent[termux]"' in text
    assert '"seple-t-agent[matrix]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
    assert '"seple-t-agent[voice]"' not in text.split("termux-all = [", 1)[1].split("]", 1)[0]
