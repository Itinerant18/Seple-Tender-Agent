...
React + Ink terminal UI for Seple T Agent. TypeScript owns the screen. Python owns sessions, tools, model calls, and most command logic.
...
- `src/app/setupHandoff.ts` — launches external `Seple T Agent setup`, suspends Ink while it runs, opens a new session on success
...
- Input history is stored in `~/.Seple T Agent/.hermes_history` or under `HERMES_HOME`.
...
### Setup (`setup.ts`)
`/setup` — launches external `Seple T Agent setup` wizard, suspends Ink while it runs
...
ui-tui/
  ...
  src/
    ...
    app/
      ...
      setupHandoff.ts               launches external Seple T Agent setup, suspends Ink while it runs
    ...
    lib/
      ...
      parentLog.ts               append-only log to ~/.Seple T Agent/tui-parent.log
    ...
    types/
      hermes-ink.d.ts            type declarations for @Seple T Agent/ink
