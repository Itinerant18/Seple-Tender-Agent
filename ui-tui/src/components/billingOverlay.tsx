...
function ConfirmScreen({
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text color={t.color.muted}>{payLine}</Text>
      <Text color={t.color.muted}>By confirming, you allow Novaedge to charge your card.</Text>
      <Text />
...
function AutoReloadScreen({ ctx, onClose, onPatch, s, t }: ScreenProps) {
  ...
  return (
    <Box flexDirection="column">
      ...
      <Text />
      <Text color={t.color.muted}>
        By confirming, you authorize Novaedge to charge {s.card ? s.card.masked : 'your card'} whenever your balance
        falls below the threshold. Turn off any time here or on the portal.
      </Text>
      {error && <Text color={t.color.error}>{error}</Text>}
      <Text />
