...
This guide covers adding a new messaging platform to the Seple T Agent gateway. A platform adapter connects Seple T Agent to an external messaging service (Telegram, Discord, WeCom, etc.) so users can interact with the agent through that service.

:::tip
There are two ways to add a platform:
- **Plugin** (recommended for community/third-party): Drop a plugin directory into `~/.Seple T Agent/plugins/` — zero core code changes needed. See [Plugin Path](#plugin-path-recommended) below.
...
The plugin system lets you add a platform adapter without modifying any core Seple T Agent code. Your plugin is a directory with two files:

```
~/.Seple T Agent/plugins/my-platform/
...
Plugin metadata. The `requires_env` and `optional_env` blocks auto-populate `Seple T Agent config` UI entries (see [Surfacing Env Vars](#surfacing-env-vars-in-hermes-config) below).
...
def register(ctx):
    """Plugin entry point — called by the Seple T Agent plugin system."""
...
| `Seple T Agent config` UI entries | `requires_env` / `optional_env` in `plugin.yaml` auto-populate |
...
| `Seple T Agent status` | Shows plugin platforms with `(plugin)` tag |
| `Seple T Agent gateway setup` | Plugin platforms appear in setup menu |
| `Seple T Agent tools` / `Seple T Agent skills` | Plugin platforms in per-platform config |
...
Most users set up a platform by dropping env vars into `~/.Seple T Agent/.env` rather than editing `config.yaml`. The `env_enablement_fn` hook lets your plugin pick those env vars up **before** the adapter is constructed, so `Seple T Agent gateway status`, `get_connected_platforms()`, and cron delivery see the correct state without instantiating the platform SDK.
...
`cron_deliver_env_var` makes your platform a recognized `deliver=` target. To make the actual send succeed when the cron job runs in a separate process from the gateway (i.e., `Seple T Agent cron run` separate from `Seple T Agent gateway`), register a `standalone_sender_fn`:
...
## Surfacing Env Vars in `Seple T Agent config`
...
version: 1.0.0
description: >
  My Platform gateway adapter for Seple T Agent.
author: Your Name
requires_env:
  ...
  - name: MY_PLATFORM_CHANNEL
    description: "Channel to join (e.g. #Seple T Agent)"
    prompt: "Channel"
    password: false
optional_env:
  - name: MY_PLATFORM_HOME_CHANNEL
...
:::note
This checklist is for adding a platform directly to the Seple T Agent core codebase — typically done by core contributors for officially supported platforms. Community/third-party platforms should use the [Plugin Path](#plugin-path-recommended) above.
:::
