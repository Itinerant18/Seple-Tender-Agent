...
description: "How to add a new inference provider to Seple T Agent — auth, runtime resolution, CLI flows, adapters, tests, and docs"
---

# Adding Providers

Seple T Agent can already talk to any OpenAI-compatible endpoint through the custom provider path. Do not add a built-in provider unless you want first-class UX for that service:

- provider-specific auth or token refresh
- a curated model catalog
- setup / `Seple T Agent model` menu entries
...
:::tip
`hermes_cli/setup.py` does **not** need changes. The setup wizard delegates provider/model selection to `select_provider_and_model()` in `main.py` — any provider added there is automatically available in `Seple T Agent setup`.
:::
...
7. `Seple T Agent model` menu includes the provider
8. `Seple T Agent setup` wizard delegates to `main.py` automatically
...
- Provider-specific `Seple T Agent model` menu entries with bespoke auth flows
...
- What env vars should Seple T Agent check, and in what priority order?
...
Be careful with API-key precedence. Seple T Agent already contains logic to avoid leaking an OpenRouter key to unrelated endpoints. A new provider should be equally explicit about which key goes to which base URL.

## Step 5: Wire the CLI in `hermes_cli/main.py`

A provider is not discoverable until it shows up in the interactive `Seple T Agent model` flow.
...
:::tip
`hermes_cli/setup.py` does not need changes — it calls `select_provider_and_model()` from `main.py`, so your new provider appears in both `Seple T Agent model` and `Seple T Agent setup` automatically.
:::
...
When you add a native provider, double-check that Seple T Agent is only sending fields that provider actually understands.
...
### 7. Updating `Seple T Agent model` but not `Seple T Agent setup`
