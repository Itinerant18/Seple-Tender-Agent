...
sidebar_position: 1
title: "Architecture"
description: "Seple T Agent internals — major subsystems, execution paths, data flow, and where to read next"
---

# Architecture

This page is the top-level map of Seple T Agent internals. Use it to orient yourself in the codebase, then dive into subsystem-specific docs for implementation details.
...
├── cli.py                    # Seple T AgentCLI — interactive terminal UI (large file)
...
│   ├── main.py               # Entry point — all `Seple T Agent` subcommands (large file)
...
│   ├── skills_config.py      # Seple T Agent skills — enable/disable per platform
│   ├── skills_hub.py         # /skills slash command
│   ├── tools_config.py       # Seple T Agent tools — enable/disable per platform
│   ├── plugins.py            # PluginManager — discovery, loading, hooks
│   ├── callbacks.py          # Terminal callbacks (clarify, sudo, approval)
│   └── gateway.py            # Seple T Agent gateway start/stop
│
...
```text
User input → Seple T AgentCLI.process_input()
...
Three discovery sources: `~/.Seple T Agent/plugins/` (user), `.Seple T Agent/plugins/` (project), and pip entry points. Plugins register tools, hooks, and CLI commands through a context API. Two specialized plugin types exist: memory providers (`plugins/memory/`) and context engines (`plugins/context_engine/`). Both are single-select — only one of each can be active at a time, configured via `Seple T Agent plugins` or `config.yaml`.
...
Exposes Seple T Agent as an editor-native agent over stdio/JSON-RPC for VS Code, Zed, and JetBrains.
...
| **Profile isolation** | Each profile (`Seple T Agent -p <name>`) gets its own HERMES_HOME, config, memory, sessions, and gateway PID. Multiple profiles run concurrently. |
