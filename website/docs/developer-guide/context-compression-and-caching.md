...
Seple T Agent uses a dual compression system and Anthropic prompt caching to
...
Configure via `Seple T Agent plugins` → Provider Plugins → Context Engine, or edit `config.yaml` directly.

For building a context engine plugin, see [Context Engine Plugins](/developer-guide/context-engine-plugin).

## Dual Compression System

Seple T Agent has two separate compression layers that operate independently:
...
compression:
  ...
  codex_app_server_auto: native  # native|Seple T Agent|off for Codex app-server thread compaction
...
| `codex_app_server_auto` | `native` | `native`, `Seple T Agent`, `off` | Thread-compaction mode for Codex app-server sessions (see below) |
...
OAuth (`provider: openai-codex`) and the model is gpt-5.5, Seple T Agent raises the
...
```bash
Seple T Agent config set compression.codex_gpt55_autoraise false
```

To keep the 85% autoraise but hide only the one-time notice:

```bash
Seple T Agent config set compression.codex_gpt55_autoraise_notice false
```
...
thread context, so Seple T Agent' auxiliary summarizer cannot shrink it — rewriting the
...
- Automatic compaction is controlled by `compression.codex_app_server_auto`:
  the default `native` lets the app-server decide when to compact and Seple T Agent
  records the resulting compaction events (compression counters, session
  events). Set `Seple T Agent` to let Seple T Agent' compression threshold initiate
  app-server compaction, or `off` to disable Seple T Agent-initiated automatic
  compaction entirely (codex may still compact natively).

Seple T Agent' local transcript is never rewritten on this runtime — state.db records
the compaction boundary while the visible transcript stays intact. All other
routes (including Codex OAuth chat sessions) keep Seple T Agent' summary compressor.
...
Anthropic allows a maximum of 4 `cache_control` breakpoints per request. Seple T Agent
...
5. **Model identity is part of the cache key**: Provider-side caches are scoped
   ...
   Seple T Agent can avoid; user-facing docs for `/model`, fallback providers, and
