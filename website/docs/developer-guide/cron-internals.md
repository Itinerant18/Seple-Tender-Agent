...
description: "How Seple T Agent stores, schedules, edits, pauses, skill-loads, and delivers cron jobs"
---
...
| `hermes_cli/cron.py` | CLI `Seple T Agent cron` subcommands |
...
Jobs are stored in `~/.Seple T Agent/cron/jobs.json` with atomic write semantics (write to temp file, then rename). Each job record contains:
...
In CLI mode, cron jobs only fire when `Seple T Agent cron` commands are run or during active CLI sessions.
...
```python
# ~/.Seple T Agent/scripts/check_competitors.py
...
| Local file | `local` | Save to `~/.Seple T Agent/cron/output/` |
...
The scheduler uses cross-process file-based locking (`fcntl.flock` on Unix, `msvcrt.locking` on Windows) to prevent overlapping ticks from executing the same due-job batch twice — even between the gateway's in-process ticker and a standalone `Seple T Agent cron` / manual `tick()` call. If the lock cannot be acquired, `tick()` returns 0 immediately.

## CLI Interface

The `Seple T Agent cron` CLI provides direct job management:

```bash
Seple T Agent cron list                    # Show all jobs
Seple T Agent cron create                  # Interactive job creation (alias: add)
Seple T Agent cron edit <job_id>           # Edit job configuration
Seple T Agent cron pause <job_id>          # Pause a running job
Seple T Agent cron resume <job_id>         # Resume a paused job
Seple T Agent cron run <job_id>            # Trigger immediate execution
Seple T Agent cron remove <job_id>         # Delete a job
```
