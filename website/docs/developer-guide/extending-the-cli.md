...
description: "Build wrapper CLIs that extend the Seple T Agent TUI with custom widgets, keybindings, and layout changes"
---

# Extending the CLI

Seple T Agent exposes protected extension hooks on `Seple T AgentCLI` so wrapper CLIs can add widgets, keybindings, and layout customizations without overriding the 1000+ line `run()` method. This keeps your extension decoupled from internal changes.
...
```python
#!/usr/bin/env python3
"""my_cli.py — Example wrapper CLI that extends Seple T Agent."""

from cli import Seple T AgentCLI
from prompt_toolkit.layout import FormattedTextControl, Window
from prompt_toolkit.filters import Condition


class MyCLI(Seple T AgentCLI):
...
Called after Seple T Agent registers its own keybindings and before the layout is built. Add your keybindings to `kb`.
