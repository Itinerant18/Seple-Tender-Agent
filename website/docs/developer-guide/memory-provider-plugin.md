...
description: "How to build a memory provider plugin for Seple T Agent"
---

# Building a Memory Provider Plugin

Memory provider plugins give Seple T Agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. This guide covers how to build one.

:::tip
Memory providers are one of two **provider plugin** types. The other is [Context Engine Plugins](/developer-guide/context-engine-plugin), which replace the built-in context compressor. Both follow the same pattern: single-select, config-driven, managed via `Seple T Agent plugins`.
:::
...
|--------|---------|-----------------|
| `get_config_schema()` | Declare config fields for `Seple T Agent memory setup` | **Yes** |
...
`get_config_schema()` returns a list of field descriptors used by `Seple T Agent memory setup`:
...
the `messages` parameter; Seple T Agent will continue calling them with the legacy
signature.
...
All storage paths **must** use the `hermes_home` kwarg from `initialize()`, not hardcoded `~/.Seple T Agent`:
...
data_dir = Path("~/.Seple T Agent/my-provider").expanduser()
```
...
Memory provider plugins can register their own CLI subcommand tree (e.g. `Seple T Agent my-provider status`, `Seple T Agent my-provider config`). This uses a convention-based discovery system — no changes to core files needed.
...
4. Your commands appear under `Seple T Agent <provider-name> <subcommand>`

**Active-provider gating:** Your CLI commands only appear when your provider is the active `memory.provider` in config. If a user hasn't configured your provider, your commands won't show in `Seple T Agent --help`.
...
def my_command(args):
    ...
    if sub == "status":
        ...
        print("Usage: Seple T Agent my-provider <status|config>")

def register_cli(subparser) -> None:
    """Build the Seple T Agent my-provider argparse tree.
