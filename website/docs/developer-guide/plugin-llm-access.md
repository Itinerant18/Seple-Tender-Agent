...
* **Host-owned credentials.** OAuth tokens, refresh flows, the
  ...
  concept Seple T Agent already has applies. The plugin never sees a
...
[`hermes-example-plugins`](https://github.com/Novaedge/hermes-example-plugins/tree/main/plugin-llm-example)
...
[`plugin-llm-async-example`](https://github.com/Novaedge/hermes-example-plugins/tree/main/plugin-llm-async-example)
in the same repo.
...
result = ctx.llm.complete(
    ...
    provider=None,         # optional, gated — Seple T Agent provider id (e.g. "openrouter")
...
plugins:
  entries:
    my-plugin:
      llm:
        # Allow this plugin to choose a different Seple T Agent provider
        # (must be one Seple T Agent already knows about — same names as
        # `Seple T Agent model` and config.yaml model.provider).
        ...
        # plugin sends — Seple T Agent does not look anything up.
        allowed_models:
          - openai/gpt-4o-mini
...
* **Auth.** Pulls API keys, OAuth tokens, or refresh tokens from
  `~/.Seple T Agent/auth.json` / env, including the credential pool when
...
* **Fallback chain.** If the user's primary provider 5xxs or 429s,
  the request goes through Seple T Agent' usual aggregator-aware fallback
...
Existing `ctx.*` methods extend an existing Seple T Agent subsystem:
...
* Implementation: [`agent/plugin_llm.py`](https://github.com/Novaedge/hermes-agent/blob/main/agent/plugin_llm.py)
* Tests: [`tests/agent/test_plugin_llm.py`](https://github.com/Novaedge/hermes-agent/blob/main/tests/agent/test_plugin_llm.py)
* Reference plugins (companion repo):
  * [`plugin-llm-example`](https://github.com/Novaedge/hermes-example-plugins/tree/main/plugin-llm-example) — sync structured extraction with image input
  * [`plugin-llm-async-example`](https://github.com/Novaedge/hermes-example-plugins/tree/main/plugin-llm-async-example) — async with `asyncio.gather()`
