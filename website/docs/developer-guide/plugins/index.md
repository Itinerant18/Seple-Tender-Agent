...
title: "Build a Seple T Agent Plugin"
description: "Step-by-step guide to building a complete Seple T Agent plugin with tools, hooks, data files, and skills"
---

# Build a Seple T Agent Plugin

This guide walks through building a complete Seple T Agent plugin from scratch. By the end you'll have a working plugin with multiple tools, lifecycle hooks, shipped data files, and a bundled skill — everything the plugin system supports.

:::info Not sure which guide you need?
Seple T Agent has several distinct pluggable interfaces — some use Python `register_*` APIs, others are config-driven or drop-in directories. Use this map first:
...
| **Gateway event hooks** (fire on startup, session events, commands) | [Event Hooks](/user-guide/features/hooks#gateway-event-hooks) — drop `HOOK.yaml` + `handler.py` into `~/.Seple T Agent/hooks/<name>/` |
...
| **Additional skill sources** (custom GitHub repos, private skill indexes) | [Skills](/user-guide/features/skills) — `Seple T Agent skills tap add <repo>` · [Publishing a tap](/user-guide/features/skills#publishing-a-custom-skill-tap) |
...
Plugins that integrate **someone else's product or project** — observability/metrics backends, vendor SaaS connectors, analytics dashboards, paid-service tie-ins — are built and distributed as **standalone plugin repos**, not merged into `Novaedge/hermes-agent`. Users install them into `~/.Seple T Agent/plugins/` or via a pip entry point; everything in this guide works the same way from a standalone repo. This is a coupling-and-maintenance decision (the core moves fast and we don't own your backend), not a quality bar — a plugin can be excellent and still belong in its own repo. Promote it in the Novaedge Discord `#plugins-skills-and-skins` channel. See [CONTRIBUTING.md](https://github.com/Novaedge/hermes-agent/blob/main/CONTRIBUTING.md) for the policy.
:::
...
```bash
mkdir -p ~/.Seple T Agent/plugins/calculator
cd ~/.Seple T Agent/plugins/calculator
```
...
This tells Seple T Agent: "I'm a plugin called calculator, I provide tools and hooks." The `provides_tools` and `provides_hooks` fields are lists of what the plugin registers.
...
4. **Accept `**kwargs`:** Seple T Agent may pass additional context in the future.
...
- `ctx.register_cli_command()` registers a CLI subcommand (e.g. `Seple T Agent my-plugin <subcommand>`)
...
- If this function crashes, the plugin is disabled but Seple T Agent continues fine
...
```bash
HERMES_PLUGINS_DEBUG=1 Seple T Agent plugins list
```
...
The same logs are always written to `~/.Seple T Agent/logs/agent.log` at WARNING level (failures only) and DEBUG level (everything) when the env var is set. So if you can't run with the env var (e.g. from inside the gateway), tail the log file instead:

```bash
Seple T Agent logs --level WARNING | grep -i plugin
```

Common reasons a plugin doesn't appear:

- **Not enabled in config** — plugins are opt-in. Run `Seple T Agent plugins enable <name>` (the name comes from the `plugins list` output, which can be `<category>/<plugin>` for nested layouts).
- **Wrong directory layout** — must be `~/.Seple T Agent/plugins/<plugin-name>/plugin.yaml` (flat) or `~/.Seple T Agent/plugins/<category>/<plugin-name>/plugin.yaml` (one level of category nesting, max). Anything deeper is ignored.
...
```
~/.Seple T Agent/plugins/calculator/
...
```
~/.Seple T Agent/plugins/my-plugin/
├── __init__.py
├── plugin.yaml
└── skills/
    ├── my-workflow/
...
**Key properties:**
- Plugin skills are **read-only** — they don't enter `~/.Seple T Agent/skills/` and can't be edited via `skill_manage`.
...
:::tip Legacy pattern
The old `shutil.copy2` pattern (copying a skill into `~/.Seple T Agent/skills/`) still works but creates name collision risk with built-in skills. Prefer `ctx.register_skill()` for new plugins.
:::
...
When users run `Seple T Agent plugins install`, they're **prompted interactively** for any missing `requires_env` variables. Values are saved to `.env` automatically.
...
| Rule | Why |
|---|---|
| Your feature key must appear in the in-tree `LAZY_DEPS` allowlist | Prevents a malicious config from coaxing Seple T Agent into installing arbitrary packages — only specs Seple T Agent itself ships are eligible |
...
auditable in `~/.Seple T Agent/logs/agent.log`. Plugins load after built-in
...
- **The system prompt is Seple T Agent's territory** — it contains model-specific guidance, tool enforcement rules, personality instructions, and cached skill content. Plugins contribute context alongside the user's input, not by altering the agent's core instructions.
...
Plugins can add their own `Seple T Agent <plugin>` subcommand tree:

```python
def _my_command(args):
    """Handler for Seple T Agent my-plugin <subcommand>."""
    ...
    if sub == "status":
        ...
        print("Usage: Seple T Agent my-plugin <status|config>")

def _setup_argparse(subparser):
    """Build the argparse tree for Seple T Agent my-plugin."""
...
After registration, users can run `Seple T Agent my-plugin status`, `Seple T Agent my-plugin config`, etc.
...
|---|---|---|
| Invoked as | `/name` in a session | `Seple T Agent name` in a terminal |
...
`ctx._cli_ref` is only populated in an **interactive CLI** session. It is `None` in the gateway, in non-interactive `Seple T Agent chat -q` runs, and in **kanban-spawned worker sessions** — so any plugin logic that reaches through `_cli_ref` silently no-ops in exactly those contexts. Two stable, session-agnostic APIs cover what hooks actually need:
...
For running a full `Seple T Agent <subcommand>` (e.g. `Seple T Agent kanban show`), shell out with the `terminal` tool via `ctx.dispatch_tool("terminal", {"command": "Seple T Agent kanban show ..."})` — there is no in-process slash-command bridge for headless worker sessions, and tools are the supported way to drive Seple T Agent from a hook.
...
Seple T Agent has five specialized plugin types beyond the general surface. Each ships as a directory under `plugins/<category>/<name>/` (bundled) or `~/.Seple T Agent/plugins/<category>/<name>/` (user). The contract differs by category — pick the one you need, then read its full guide.
...
def register(ctx):
    ctx.register_platform(
        ...
        # show up in `Seple T Agent gateway status` without SDK instantiation.
...
Seple T Agent also accepts extensions that aren't Python plugins at all. These are shown in the [Pluggable interfaces table](/user-guide/features/plugins#pluggable-interfaces--where-to-go-for-each); the sections below sketch each authoring style briefly.

### MCP servers — register external tools

Model Context Protocol (MCP) servers register their own tools into Seple T Agent without any Python plugin. Declare them in `~/.Seple T Agent/config.yaml`:
...
Seple T Agent connects to each server at startup, lists its tools, and registers them alongside built-ins. The LLM sees them exactly like any other tool. **Full guide:** [MCP](/user-guide/features/mcp).

### Gateway event hooks — fire on lifecycle events

Drop a manifest + handler into `~/.Seple T Agent/hooks/<name>/`:

```yaml
# ~/.Seple T Agent/hooks/long-task-alert/HOOK.yaml
...
```python
# ~/.Seple T Agent/hooks/long-task-alert/handler.py
...
Seple T Agent skills search my-workflow --source myorg/skills-repo
Seple T Agent skills install myorg/skills-repo/my-workflow
```
...
# Plugin auto-discovered on next Seple T Agent startup
```
...
```python
# Wrong — will break if Seple T Agent passes extra context
def handler(args):
    ...
