...
Seple T Agent uses a SQLite database (`~/.Seple T Agent/state.db`) to persist session
...
Multiple Seple T Agent processes (gateway + CLI sessions + worktree agents) share one
...
db = SessionDB()                           # Default: ~/.Seple T Agent/state.db
...
Default path: `~/.Seple T Agent/state.db`

This is derived from `hermes_constants.get_hermes_home()` which resolves to
`~/.Seple T Agent/` by default, or the value of `HERMES_HOME` environment variable.
