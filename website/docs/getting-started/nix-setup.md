...
description: "Install and deploy Seple T Agent with Nix — from quick `nix run` to fully declarative NixOS module with container mode"
---
...
Seple T Agent ships a Nix flake & a NixOS module.
...
**For non-NixOS users**, this only changes the install step. Everything after (`Seple T Agent setup`, `Seple T Agent gateway install`, config editing) works identically to the standard install.

**For NixOS module users**, the entire lifecycle is different: configuration lives in `configuration.nix`, secrets go through sops-nix/agenix, the service is a systemd unit, and CLI config commands are blocked. You manage Seple T Agent the same way you manage any other NixOS service.
:::
...
```bash
# Run the desktop app
nix run github:Novaedge/hermes-agent#desktop

# Or install persistently
nix profile install github:Novaedge/hermes-agent#desktop

# run the tui
nix run github:Novaedge/hermes-agent -- setup
nix run github:Novaedge/hermes-agent -- --tui

# or install it in your profile
nix profile install github:Novaedge/hermes-agent
Seple T Agent setup
Seple T Agent --tui
```

After `nix profile install`, `Seple T Agent`, `hermes-agent`, and `hermes-acp` are on your PATH. From here, the workflow is identical to the [standard installation](./installation.md) — `Seple T Agent setup` walks you through provider selection, `Seple T Agent gateway install` sets up a launchd (macOS) or systemd user service, and config lives in `~/.Seple T Agent/`.
...
```bash
git clone https://github.com/Novaedge/hermes-agent.git
cd hermes-agent
nix develop
Seple T Agent setup
```
...
# /etc/nixos/flake.nix (or your system flake)
{
  inputs = {
    ...
    hermes-agent.url = "github:Novaedge/hermes-agent";
  };
...
That's it. `nixos-rebuild switch` creates the `Seple T Agent` user, generates `config.yaml`, wires up secrets, and starts the gateway — a long-running service that connects the agent to messaging platforms (Telegram, Discord, etc.) and listens for incoming messages.
...
```bash
echo "OPENROUTER_API_KEY=sk-or-your-key" | sudo install -m 0600 -o Seple T Agent /dev/stdin /var/lib/Seple T Agent/env
```

```nix
services.hermes-agent.environmentFiles = [ "/var/lib/Seple T Agent/env" ];
```
:::

:::tip addToSystemPackages
Setting `addToSystemPackages = true` does two things: puts the `Seple T Agent` CLI on your system PATH **and** sets `HERMES_HOME` system-wide so the interactive CLI shares state (sessions, skills, cron) with the gateway service. Without it, running `Seple T Agent` in your shell creates a separate `~/.Seple T Agent/` directory.
:::

### Container-aware CLI

:::info
When `container.enable = true` and `addToSystemPackages = true`, **every** `Seple T Agent` command on the host automatically routes into the managed container. This means your interactive CLI session runs inside the same environment as the gateway service — with access to all container-installed packages and tools.

- The routing is transparent: `Seple T Agent chat`, `Seple T Agent sessions list`, `Seple T Agent version`, etc. all exec into the container under the hood
- All CLI flags are forwarded as-is
- If the container isn't running, the CLI retries briefly (5s with a spinner for interactive use, 10s silently for scripts) then fails with a clear error — no silent fallback
- For developers working on the Seple T Agent codebase, set `HERMES_DEV=1` to bypass container routing and run the local checkout directly

Set `container.hostUsers` to create a `~/.Seple T Agent` symlink to the service state directory, so the host CLI and the container share sessions, config, and memories:
...
Users listed in `hostUsers` are automatically added to the `Seple T Agent` group for file permission access.
...
The CLI auto-detects when sudo is needed and uses it transparently. Without this, you'll need to run `sudo Seple T Agent chat` manually.
:::
...
Seple T Agent config       # shows the generated config
```
...
:::note Model naming
`settings.model.default` uses the model identifier your provider expects. With [OpenRouter](https://openrouter.ai) (the default), these look like `"anthropic/claude-sonnet-4"` or `"google/gemini-3-flash"`. If you're using a provider directly (Anthropic, OpenAI), set `settings.model.base_url` to point at their API and use their native model IDs (e.g., `"claude-sonnet-4-20250514"`). When no `base_url` is set, Seple T Agent defaults to OpenRouter.
:::
...
```nix
services.hermes-agent.configFile = /etc/Seple T Agent/config.yaml;
```
...
| Give the agent a personality | `${services.hermes-agent.stateDir}/.Seple T Agent/SOUL.md` | manage the file directly |
...
| Override the Seple T Agent package | `package` | `inputs.hermes-agent.packages.${system}.default.override { ... }` |
| Change state directory | `stateDir` | `"/opt/Seple T Agent"` |
...
Both `environment` (non-secret vars) and `environmentFiles` (secret files) are merged into `$HERMES_HOME/.env` at activation time (`nixos-rebuild switch`). Seple T Agent reads this file on every startup, so changes take effect with a `systemctl restart hermes-agent` — no container recreation needed.
...
{
  sops = {
    defaultSopsFile = ./secrets/Seple T Agent.yaml;
...
```yaml
# secrets/Seple T Agent.yaml (encrypted with sops)
hermes-env: |
...
{
  services.hermes-agent = {
    authFile = config.sops.secrets."Seple T Agent/auth.json".path;
...
The `documents` option installs files into the agent's working directory (the `workingDirectory`, which the agent reads as its workspace). Seple T Agent looks for specific filenames by convention:

- **`USER.md`** — context about the user the agent is interacting with.
- Any other files you place here are visible to the agent as workspace files.

The agent identity file is separate: Seple T Agent loads its primary `SOUL.md` from `$HERMES_HOME/SOUL.md`, which in the NixOS module is `${services.hermes-agent.stateDir}/.Seple T Agent/SOUL.md`. Putting `SOUL.md` in `documents` only creates a workspace file and will not replace the main persona file.
...
Set `auth = "oauth"` for servers using OAuth 2.1. Seple T Agent implements the full PKCE flow — metadata discovery, dynamic client registration, token exchange, and automatic refresh.
...
The first OAuth authorization requires a browser-based consent flow. In a headless deployment, Seple T Agent prints the authorization URL to stdout/logs instead of opening a browser.

**Option A: Interactive bootstrap** — run the flow once via `docker exec` (container) or `sudo -u Seple T Agent` (native):

```bash
# Container mode
docker exec -it hermes-agent \
  Seple T Agent mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth

# Native mode
sudo -u Seple T Agent HERMES_HOME=/var/lib/Seple T Agent/.Seple T Agent \
  Seple T Agent mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
```
...
```bash
Seple T Agent mcp add my-oauth-server --url https://mcp.example.com/mcp --auth oauth
scp ~/.Seple T Agent/mcp-tokens/my-oauth-server{,.client}.json \
    server:/var/lib/Seple T Agent/.Seple T Agent/mcp-tokens/
# Ensure: chown Seple T Agent:Seple T Agent, chmod 0600
```
...
When Seple T Agent runs via the NixOS module, the following CLI commands are **blocked** with a descriptive error pointing you to `configuration.nix`:

| Blocked command | Why |
|---|---|
| `Seple T Agent setup` | Config is declarative — edit `settings` in your Nix config |
| `Seple T Agent config edit` | Config is generated from `settings` |
| `Seple T Agent config set <key> <value>` | Config is generated from `settings` |
| `Seple T Agent gateway install` | The systemd service is managed by NixOS |
| `Seple T Agent gateway uninstall` | The systemd service is managed by NixOS |

This prevents drift between what Nix declares and what's on disk. Detection uses two signals:

1. **`HERMES_MANAGED=true`** environment variable — set by the systemd service, visible to the gateway process
2. **`.managed` marker file** in `HERMES_HOME` — set by the activation script, visible to interactive shells (e.g., `docker exec -it hermes-agent Seple T Agent config set ...` is also blocked)
...
When container mode is enabled, Seple T Agent runs inside a persistent Ubuntu container with the Nix-built binary bind-mounted read-only from the host:

```
Host                                    Container
────                                    ─────────
/nix/store/...-hermes-agent-0.1.0  ──►  /nix/store/... (ro)
~/.Seple T Agent -> /var/lib/Seple T Agent/.Seple T Agent       (symlink bridge, per hostUsers)
/var/lib/Seple T Agent/                    ──►  /data/          (rw)
  ├── current-package -> /nix/store/...    (symlink, updated each rebuild)
  ├── .gc-root -> /nix/store/...           (prevents nix-collect-garbage)
  ├── .container-identity                  (sha256 hash, triggers recreation)
  ├── .Seple T Agent/                             (HERMES_HOME)
...
The Nix-built binary works inside the Ubuntu container because `/nix/store` is bind-mounted — it brings its own interpreter and all dependencies, so there's no reliance on the container's system libraries. The container entrypoint resolves through a `current-package` symlink: `/data/current-package/bin/Seple T Agent gateway run --replace`. On `nixos-rebuild switch`, only the symlink is updated — the container keeps running.

### What Persists Across What

| Event | Container recreated? | `/data` (state) | `/home/Seple T Agent` | Writable layer (`apt`/`pip`/`npm`) |
|---|---|---|---|---|
...
The container is only recreated when its **identity hash** changes. The hash covers: schema version, image, `extraVolumes`, `extraOptions`, and the entrypoint script. Changes to environment variables, settings, documents, or the Seple T Agent package itself do **not** trigger recreation.

:::warning Writable layer loss
When the identity hash changes (image upgrade, new volumes, new container options), the container is destroyed and recreated from a fresh pull of `container.image`. Any `apt install`, `pip install`, or `npm install` packages in the writable layer are lost. State in `/data` and `/home/Seple T Agent` is preserved (these are bind mounts).
...
The `preStart` script creates a GC root at `${stateDir}/.gc-root` pointing to the current Seple T Agent package. This prevents `nix-collect-garbage` from removing the running binary. If the GC root somehow breaks, restarting the service recreates it.

---

## Plugins

The NixOS module supports declarative plugin installation — no imperative `Seple T Agent plugins install` needed.
...
Plugins are symlinked into `$HERMES_HOME/plugins/` at activation time. Seple T Agent discovers them via its normal directory scan. Removing a plugin from the list and running `nixos-rebuild switch` removes the symlink.
...
The package's `site-packages` is added to PYTHONPATH in the Seple T Agent wrapper. `importlib.metadata` discovers the entry point at session start.
...
```nix
{
  inputs.hermes-agent.url = "github:Novaedge/hermes-agent";
...
:::note
A build-time collision check prevents plugin packages from shadowing core Seple T Agent dependencies. If a plugin provides a package already in the sealed venv, `nixos-rebuild` fails with a clear error.
:::
...
| Check | What it tests |
|---|---|
| `package-contents` | `Seple T Agent` and `hermes-agent` binaries exist and `Seple T Agent version` runs |
| `entry-points-sync` | Every `[project.scripts]` entry in `pyproject.toml` has a wrapped binary in the Nix package |
| `cli-commands` | `Seple T Agent --help` exposes `gateway` and `config` subcommands |
| `managed-guard` | `HERMES_MANAGED=true Seple T Agent config set ...` prints the NixOS error |
...
| `user` | `str` | `"Seple T Agent"` | System user |
| `group` | `str` | `"Seple T Agent"` | System group |
| `createUser` | `bool` | `true` | Auto-create user/group |
| `stateDir` | `str` | `"/var/lib/Seple T Agent"` | State directory (`HERMES_HOME` parent) |
| `workingDirectory` | `str` | `"${stateDir}/workspace"` | Agent working directory |
| `addToSystemPackages` | `bool` | `false` | Add `Seple T Agent` CLI to system PATH and set `HERMES_HOME` system-wide |
...
| Option | Type | Default | Description |
|---|---|---|---|
| `extraArgs` | `listOf str` | `[]` | Extra args for `Seple T Agent gateway` |
| `extraPackages` | `listOf package` | `[]` | Extra packages available to the agent. Added to the Seple T Agent user's per-user profile so terminal commands, skills, and cron jobs all see them |
...
| `container.hostUsers` | `listOf str` | `[]` | Interactive users who get a `~/.Seple T Agent` symlink to the service stateDir and are auto-added to the `Seple T Agent` group |
...
```
/var/lib/Seple T Agent/                     # stateDir (owned by Seple T Agent:Seple T Agent, 0750)
├── .Seple T Agent/                         # HERMES_HOME
...
|---|---|---|---|
| `/nix/store` | `/nix/store` | `ro` | Seple T Agent binary + all Nix deps |
| `/data` | `/var/lib/Seple T Agent` | `rw` | All state, config, workspace |
| `/home/Seple T Agent` | `${stateDir}/home` | `rw` | Persistent agent home — `pip install --user`, tool caches |
...
sudo rm /var/lib/Seple T Agent/.container-identity
...
```bash
# Native mode
sudo -u Seple T Agent cat /var/lib/Seple T Agent/.Seple T Agent/.env

# Container mode
docker exec hermes-agent cat /data/.Seple T Agent/.env
```
...
| `Seple T Agent version` shows old version | Container not restarted | `systemctl restart hermes-agent` |
| Permission denied on `/var/lib/Seple T Agent` | State dir is `0750 Seple T Agent:Seple T Agent` | Use `docker exec` or `sudo -u Seple T Agent` |
| `nix-collect-garbage` removed Seple T Agent | GC root missing | Restart the service (preStart recreates the GC root) |
...
| `unable to find user Seple T Agent` | Container still starting (entrypoint hasn't created user yet) | Wait a few seconds and retry — the CLI retries automatically |
