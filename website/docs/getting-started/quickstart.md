...
sidebar_position: 1
title: "Quickstart"
description: "Your first conversation with Seple T Agent — from install to chatting in under 5 minutes"
---

# Quickstart

This guide gets you from zero to a working Seple T Agent setup that survives real use. Install, choose a provider, verify a working chat, and know exactly what to do when something breaks.

## Prefer to watch?

**Onchain AI Garage** put together a Masterclass walkthrough of installation, setup, and basic commands — a good companion to this page if you'd rather follow along on video. For more, see the full [Seple T Agent Tutorials & Use Cases](https://www.youtube.com/playlist?list=PLmpUb_PWAkDxewld5ZYyKifuHxgIbiq2d) playlist.

<div style={{position: 'relative', paddingBottom: '56.25%', height: 0, overflow: 'hidden', maxWidth: '100%', marginBottom: '1.5rem'}}>
  <iframe
    style={{position: 'absolute', top: 0, left: 0, width: '100%', height: '100%'}}
    src="https://www.youtube-nocookie.com/embed/R3YOGfTBcQg"
    title="Seple T Agent Masterclass: Installation, Setup, Basic Commands"
    frameBorder="0"
...
- Setting up Seple T Agent for a team, bot, or always-on workflow
...
|---|---|---|
| I just want Seple T Agent working on my machine | `Seple T Agent setup` | Run a real chat and verify it responds |
| I already know my provider | `Seple T Agent model` | Save the config, then start chatting |
| I want a bot or always-on setup | `Seple T Agent gateway setup` after CLI works | Connect Telegram, Discord, Slack, or another platform |
| I want a local or self-hosted model | `Seple T Agent model` → custom endpoint | Verify the endpoint, model name, and context length |
| I want multi-provider fallback | `Seple T Agent model` first | Add routing and fallback only after the base chat works |

**Rule of thumb:** if Seple T Agent cannot complete a normal chat, do not add more features yet. Get one clean conversation working first, then layer on gateway, cron, skills, voice, or routing.

---

## 1. Install Seple T Agent
### With the Seple T Agent Desktop installer on macOS or Windows (recommended)
To easily install the command-line and desktop applications, [download the Seple T Agent Desktop installer](https://hermes-agent.Novaedge.com/) from our website and run it.

### Without Seple T Agent Desktop:
For a command-line only install without Seple T Agent Desktop, run:
...
The single most important setup step. Use `Seple T Agent model` to walk through the choice interactively:
...
:::info Setup modes
On a fresh install, `Seple T Agent setup` offers three modes:
...
Blank Slate writes an explicit `platform_toolsets.cli` list plus `agent.disabled_toolsets`, so nothing you didn't choose ever loads — not even after `Seple T Agent update`. Re-enable anything later with `Seple T Agent tools`, seed skills with `Seple T Agent skills opt-in --sync`, or tune settings with `Seple T Agent setup agent`.
:::
...
|----------|-----------|---------------|
| **Nous Portal** | Subscription-based, zero-config | OAuth login via `Seple T Agent model` |
| **OpenAI Codex** | ChatGPT OAuth, uses Codex models | Device code auth via `Seple T Agent model` |
| **Anthropic** | Claude models directly — Max plan + extra usage credits (OAuth), or API key for pay-per-token | `Seple T Agent model` → OAuth login (requires Max + extra credits), or an Anthropic API key |
...
| **MiniMax (OAuth)** | MiniMax frontier model via browser OAuth — no API key needed (model name in `hermes_cli/models.py` may change between releases) | `Seple T Agent model` → MiniMax (OAuth) |
...
| **xAI Grok OAuth** | SuperGrok / Premium+ subscription, no API key needed | `Seple T Agent model` → xAI Grok OAuth |
...
| **Qwen OAuth** | Qwen Portal browser OAuth — no API key needed | `Seple T Agent model` → Qwen OAuth |
...
| **GitHub Copilot** | GitHub Copilot subscription (GPT-5.x, Claude, Gemini, etc.) | OAuth via `Seple T Agent model`, or `COPILOT_GITHUB_TOKEN` / `GH_TOKEN` |
| **GitHub Copilot ACP** | Copilot ACP agent backend (spawns local `copilot` CLI) | `Seple T Agent model` (requires `copilot` CLI + `copilot login`) |
...
Seple T Agent requires a model with at least **64,000 tokens** of context. Models with smaller windows cannot maintain enough working memory for multi-step tool-calling workflows and will be rejected at startup. Most hosted models (Claude, GPT, Gemini, Qwen, DeepSeek) meet this easily. If you're running a local model, set its context size to at least 64K (e.g. `--ctx-size 65536` for llama.cpp or `-c 65536` for Ollama).
:::

:::tip
You can switch providers at any time with `Seple T Agent model` — no lock-in. For a full list of all supported providers and setup details, see [AI Providers](../integrations/providers.md).
:::

### How settings are stored

Seple T Agent separates secrets from normal config:

- **Secrets and tokens** → `~/.Seple T Agent/.env`
- **Non-secret settings** → `~/.Seple T Agent/config.yaml`

The easiest way to set values correctly is through the CLI:

```bash
Seple T Agent config set model anthropic/claude-opus-4.6
Seple T Agent config set terminal.backend docker
Seple T Agent config set OPENROUTER_API_KEY sk-or-...
```
...
```bash
Seple T Agent            # classic CLI
Seple T Agent --tui      # modern TUI (recommended)
```

You'll see a welcome banner with your model, available tools, and skills. Use a prompt that's specific and easy to verify:

:::tip Pick your interface
Seple T Agent ships with two terminal interfaces: the classic `prompt_toolkit` CLI and a newer [TUI](../user-guide/tui.md) with modal overlays, mouse selection, and non-blocking input. Both share the same sessions, slash commands, and config — try each with `Seple T Agent` vs `Seple T Agent --tui`.
:::
...
- Seple T Agent replies without error
...
```bash
Seple T Agent --continue    # Resume the most recent session
Seple T Agent -c            # Short form
```
...
```bash
Seple T Agent gateway setup    # Interactive platform configuration
```
...
- `Seple T Agent tools` — tune tool access per platform
- `Seple T Agent skills` — browse and install reusable workflows
...
```bash
Seple T Agent config set terminal.backend docker    # Docker isolation
Seple T Agent config set terminal.backend ssh       # Remote server
```

### Voice mode

```bash
# From the Seple T Agent install directory (the curl installer placed it at
# ~/.Seple T Agent/hermes-agent on Linux/macOS or %LOCALAPPDATA%\Seple T Agent\hermes-agent on Windows):
cd ~/.Seple T Agent/hermes-agent
...
Skills are on-demand instruction documents that teach Seple T Agent how to do a specific task — deploy to Kubernetes, open a GitHub PR, fine-tune a model, search for GIFs. Each is a `SKILL.md` file with a name, a description, and a step-by-step procedure. The agent reads the short descriptions for free and only loads a skill's full content when a task actually calls for it, so adding skills doesn't bloat every request.

Seple T Agent ships with a catalog of bundled skills already installed in `~/.Seple T Agent/skills/`. You can add more from the Skills Hub, or write your own.

**Browse and install from the hub:**

```bash
Seple T Agent skills browse                      # list everything available
Seple T Agent skills search kubernetes           # find skills by keyword
Seple T Agent skills install openai/skills/k8s   # install one (runs a security scan first)
```

The install argument is a `source/path` slug from the hub — `openai/skills/k8s` means the `k8s` skill from OpenAI's catalog. `Seple T Agent skills browse` shows the exact slugs to use.
...
/k8s                                       # load it and let Seple T Agent ask what you need
```
...
```yaml
# Add to ~/.Seple T Agent/config.yaml
mcp_servers:
  github:
    command: npx
...
(If you installed without `[all]`, run `cd ~/.Seple T Agent/hermes-agent && uv pip install -e ".[acp]"` first.)
...
|---|---|---|
| Seple T Agent opens but gives empty or broken replies | Provider auth or model selection is wrong | Run `Seple T Agent model` again and confirm provider, model, and auth |
...
| Gateway starts but nobody can message it | Bot token, allowlist, or platform setup is incomplete | Re-run `Seple T Agent gateway setup` and check `Seple T Agent gateway status` |
| `Seple T Agent --continue` can't find old session | Switched profiles or session never saved | Check `Seple T Agent sessions list` and confirm you're in the right profile |
...
| `Seple T Agent doctor` flags config problems | Config values are missing or stale | Fix the config, retest a plain chat before adding features |

## Recovery Toolkit

When something feels off, use this order:

1. `Seple T Agent doctor`
2. `Seple T Agent model`
3. `Seple T Agent setup`
4. `Seple T Agent sessions list`
5. `Seple T Agent --continue`
6. `Seple T Agent gateway status`
...
| Command | Description |
|---------|-------------|
| `Seple T Agent` | Start chatting |
| `Seple T Agent model` | Choose your LLM provider and model |
| `Seple T Agent tools` | Configure which tools are enabled per platform |
| `Seple T Agent setup` | Full setup wizard (configures everything at once) |
| `Seple T Agent doctor` | Diagnose issues |
| `Seple T Agent update` | Update to latest version |
| `Seple T Agent gateway` | Start the messaging gateway |
| `Seple T Agent --continue` | Resume last session |
