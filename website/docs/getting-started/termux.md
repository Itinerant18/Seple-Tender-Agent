...
description: "Run Seple T Agent directly on an Android phone with Termux"
---

# Seple T Agent on Android with Termux
...
Seple T Agent can run directly on an Android phone through [Termux](https://termux.dev/).
...
That does not stop Seple T Agent from working well as a phone-native CLI agent — it just means the recommended mobile install is intentionally narrower than the desktop/server install.

---

## Option 1: One-line installer

Seple T Agent now ships a Termux-aware installer path:
...
- links `Seple T Agent` into `$PREFIX/bin` so it stays on your Termux PATH
...
### 5. Put `Seple T Agent` on your Termux PATH

```bash
ln -sf "$PWD/venv/bin/Seple T Agent" "$PREFIX/bin/Seple T Agent"
```

`$PREFIX/bin` is already on PATH in Termux, so this makes the `Seple T Agent` command persist across new shells without re-activating the venv every time.
...
Or set keys directly in `~/.Seple T Agent/.env`.
...
### `Seple T Agent doctor` says ripgrep or Node is missing
