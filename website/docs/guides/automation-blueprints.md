...
Copy-paste blueprints for common automation patterns. Each blueprint uses Seple T Agent's built-in [cron scheduler](/user-guide/features/cron) for time-based triggers and [webhook platform](/user-guide/messaging/webhooks) for event-driven triggers.
...
| **GitHub Event** | Fires on PR opens, pushes, issues, CI results | Webhook platform (`Seple T Agent webhook subscribe`) |
| **API Call** | External service POSTs JSON to your endpoint | Webhook platform (config.yaml routes or `Seple T Agent webhook subscribe`) |
...
```bash
Seple T Agent cron create "0 2 * * *" \
  "You are a project manager triaging the Novaedge/hermes-agent GitHub repo.

1. Run: gh issue list --repo Novaedge/hermes-agent --state open --json number,title,labels,author,createdAt --limit 30
...
```bash
Seple T Agent webhook subscribe github-pr-review \
  --events "pull_request" \
...
```bash
Seple T Agent cron create "0 9 * * 1" \
  "Scan the Novaedge/hermes-agent repo for documentation drift.

1. Run: gh pr list --repo Novaedge/hermes-agent --state merged --json number,title,files,mergedAt --limit 30
...
```bash
Seple T Agent cron create "0 6 * * *" \
  "Run a dependency security audit on the hermes-agent project.

1. cd ~/.Seple T Agent/hermes-agent && source .venv/bin/activate
...
```bash
Seple T Agent webhook subscribe deploy-verify \
  --events "deployment" \
...
```bash
Seple T Agent webhook subscribe alert-triage \
...
```python title="~/.Seple T Agent/scripts/check-uptime.py"
...
for ep in ENDPOINTS:
    try:
        start = time.time()
        req = urllib.request.Request(ep["url"], headers={"User-Agent": "Seple T Agent-Monitor/1.0"})
...
```bash
Seple T Agent cron create "every 30m" \
  ...
  --script ~/.Seple T Agent/scripts/check-uptime.py \
  --name "Uptime monitor" \
  --deliver telegram
```
...
```bash
Seple T Agent cron create "0 8 * * *" \
  "Search arXiv for the 3 most interesting papers on 'language model reasoning' OR 'tool-use agents' from the past day. For each paper, create an Obsidian note with the title, authors, abstract summary, key contribution, and potential relevance to Seple T Agent development." \
...
```bash
Seple T Agent webhook subscribe auto-port \
  --events "pull_request" \
...
```bash
Seple T Agent webhook subscribe stripe-payments \
...
```bash
Seple T Agent cron create "0 3 * * 0" \
...
```bash
Seple T Agent cron create "0 10 * * 3" \
