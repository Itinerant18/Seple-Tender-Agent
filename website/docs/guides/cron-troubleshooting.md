...
description: "Diagnose and fix common Seple T Agent cron issues — jobs not firing, delivery failures, skill loading errors, and performance problems"
---
...
If you're expecting jobs to fire automatically, you need a running gateway (`Seple T Agent gateway` for foreground, or `Seple T Agent gateway start` for the installed service). For one-off debugging, you can manually trigger a tick with `Seple T Agent cron tick`.
...
```bash
date
Seple T Agent cron list   # Compare next_run times with local time
```
...
| Target | Requires |
|--------|----------|
| `telegram` | `TELEGRAM_BOT_TOKEN` in `~/.Seple T Agent/.env` |
| `discord` | `DISCORD_BOT_TOKEN` in `~/.Seple T Agent/.env` |
| `slack` | `SLACK_BOT_TOKEN` in `~/.Seple T Agent/.env` |
...
| `local` | Write access to `~/.Seple T Agent/cron/output/` |
...
If delivery fails, the job still runs — it just won't send anywhere. Check `Seple T Agent cron list` for updated `last_error` field (if available).
...
```bash
Seple T Agent skills list
```

Skills must be installed before they can be attached to cron jobs. If a skill is missing, install it first with `Seple T Agent skills install <skill-name>` or via `/skills` in the CLI.

### Check 2: Check skill name vs. skill folder name

Skill names are case-sensitive and must match the installed skill's folder name. If your job specifies `ai-funding-daily-report` but the skill folder is `ai-funding-daily-report`, confirm the exact name from `Seple T Agent skills list`.
...
2. `~/.Seple T Agent/logs/agent.log` for scheduler messages (or `errors.log` for warnings)
3. The job's `last_run` metadata via `Seple T Agent cron list`

### Check 2: Common error patterns

**"No such file or directory" for scripts**
The `script` path must be an absolute path (or relative to the Seple T Agent config directory). Verify:
```bash
ls ~/.Seple T Agent/scripts/your-script.py   # Must exist
Seple T Agent cron edit <job_id> --script ~/.Seple T Agent/scripts/your-script.py
```

**"Skill not found" at job execution**
The skill must be installed on the machine running the scheduler. If you move between machines, skills don't automatically sync — reinstall them with `Seple T Agent skills install <skill-name>`.
...
```bash
ps aux | grep Seple T Agent
# Kill duplicate processes, keep only one
```

### Check 4: Permissions on jobs.json

Jobs are stored in `~/.Seple T Agent/cron/jobs.json`. If this file is not readable/writable by your user, the scheduler will fail silently:

```bash
ls -la ~/.Seple T Agent/cron/jobs.json
chmod 600 ~/.Seple T Agent/cron/jobs.json   # Your user should own it
```
...
```bash
Seple T Agent cron list                    # Show all jobs, states, next_run times
Seple T Agent cron run <job_id>            # Schedule for next tick (for testing)
Seple T Agent cron edit <job_id>           # Fix configuration issues
Seple T Agent logs                         # View recent Seple T Agent logs
Seple T Agent skills list                  # Verify installed skills
```
...
1. Run the job with `Seple T Agent cron run <job_id>` (fires on next gateway tick) and watch for errors in the chat output
2. Check `~/.Seple T Agent/logs/agent.log` for scheduler messages and `~/.Seple T Agent/logs/errors.log` for warnings
3. Open an issue at [github.com/Novaedge/hermes-agent](https://github.com/Novaedge/hermes-agent) with:
