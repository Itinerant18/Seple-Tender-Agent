...
:::tip
This recipe hits web search, summarization, and optional TTS — all bundled in a Portal subscription. The fastest setup is `Seple T Agent setup --portal`. See [Nous Portal](/integrations/nous-portal).
:::
...
2. **Seple T Agent spins up** a fresh agent session with your prompt
...
- **Gateway running** — the gateway daemon handles cron execution:
  ...
  sudo Seple T Agent gateway install --system   # Linux servers: boot-time system service
  # or
...
You can still follow this tutorial using `deliver: "local"`. Briefings will be saved to `~/.Seple T Agent/cron/output/` and you can read them anytime.
:::
...
Seple T Agent will search the web, read through results, and produce something like:
...
---
3 stories • Sources searched: 8 • Generated by Seple T Agent
```
...
Before creating cron jobs, ensure Seple T Agent has a default model and provider configured globally. If you want a specific job to use different values, set explicit per-job model/provider overrides when creating it.

### Option A: Natural Language (in chat)

Just tell Seple T Agent what you want:
...
Seple T Agent will create the cron job for you using the unified `cronjob` tool.
...
For faster briefings, tell Seple T Agent to delegate each topic to a sub-agent:
...
Seple T Agent will use `cronjob(action="list")` to find it and `cronjob(action="remove")` to delete it.
