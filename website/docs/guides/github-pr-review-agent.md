...
┌───────────────────────────────────────────────────────────────────┐
│                                                                   │
│   Cron Timer  ──▶  Seple T Agent  ──▶  GitHub API  ──▶  Review     │
...
If you have a public endpoint available, check out [Automated GitHub PR Comments with Webhooks](./webhook-github-pr-review.md) — GitHub pushes events to Seple T Agent instantly when PRs are opened or updated.
:::
...
- **Seple T Agent installed** — see the [Installation guide](/getting-started/installation)
- **Gateway running** for cron jobs:
  ```bash
  Seple T Agent gateway install   # Install as a service
  # or
...
Use `deliver: "local"` to save reviews to `~/.Seple T Agent/cron/output/`. Great for testing before wiring up notifications.
:::
...
Make sure Seple T Agent can access GitHub. Start a chat:
...
```
Run: gh pr list --repo Novaedge/hermes-agent --state open --limit 3
```
...
Still in the chat, ask Seple T Agent to review a real PR:

```
Review this pull request. Read the diff, check for bugs, security issues,
and code quality. Be specific about line numbers and quote problematic code.

Run: gh pr diff 3888 --repo Novaedge/hermes-agent
```
...
A skill gives Seple T Agent consistent review guidelines that persist across sessions and cron runs. Without one, review quality varies.

```bash
mkdir -p ~/.Seple T Agent/skills/code-review
```

Create `~/.Seple T Agent/skills/code-review/SKILL.md`:
...
Verify it loaded — start `Seple T Agent` and you should see `code-review` in the skills list at startup.

---

## Step 4: Teach It Your Conventions

This is what makes the reviewer actually useful. Start a session and teach Seple T Agent your team's standards:
...
```bash
Seple T Agent cron run pr-review
```
...
2. Teach Seple T Agent your conventions via memory (Step 4)
3. The more context it has about your stack, the better the reviews

### Cron job doesn't run
```bash
Seple T Agent gateway status    # Is the gateway running?
Seple T Agent cron list         # Is the job enabled?
```
