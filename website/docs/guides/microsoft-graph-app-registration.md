...
You need **tenant admin rights** (or an admin to grant consent on your behalf) to finish this. Bookmark the values you collect — they go into `~/.Seple T Agent/.env` at the end.
...
4. Fill in:
   - **Name:** `Seple T Agent Teams Meeting Pipeline` (or any name you'll recognize).
...
Put the three values you collected into `~/.Seple T Agent/.env`:
...
Seple T Agent ships a Graph auth smoke-test. From your Seple T Agent install:
...
2. Update `MSGRAPH_CLIENT_SECRET` in `~/.Seple T Agent/.env` with the new value.
3. Restart the gateway so the new secret is picked up: `Seple T Agent gateway restart`.
