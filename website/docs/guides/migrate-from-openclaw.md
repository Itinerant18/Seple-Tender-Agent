...
description: "Complete guide to migrating your OpenClaw / Clawdbot setup to Seple T Agent — what gets migrated, how config maps, and what to check after."
---

# Migrate from OpenClaw

`Seple T Agent claw migrate` imports your OpenClaw (or legacy Clawdbot/Moldbot) setup into Seple T Agent. This guide covers exactly what gets migrated, the config key mappings, and what to verify after migration.

:::tip
If your OpenClaw setup was multi-provider, `Seple T Agent setup --portal` collapses it to one OAuth — 300+ models plus the Tool Gateway in a single login. See [Nous Portal](/integrations/nous-portal).
:::
...
| `--no-backup` | Skip the pre-migration zip snapshot of `~/.Seple T Agent/` (by default a single restore-point archive is written before apply, under `~/.Seple T Agent/backups/pre-migration-*.zip`; restorable with `Seple T Agent import`). |
...
| What | OpenClaw source | Seple T Agent destination | Notes |
|------|----------------|-------------------|-------|
| Persona | `workspace/SOUL.md` | `~/.Seple T Agent/SOUL.md` | Direct copy |
| Workspace instructions | `workspace/AGENTS.md` | `AGENTS.md` in `--workspace-target` | Requires `--workspace-target` flag |
| Long-term memory | `workspace/MEMORY.md` | `~/.Seple T Agent/memories/MEMORY.md` | Parsed into entries, merged with existing, deduped. Uses `§` delimiter. |
| User profile | `workspace/USER.md` | `~/.Seple T Agent/memories/USER.md` | Same entry-merge logic as memory. |
| Daily memory files | `workspace/memory/*.md` | `~/.Seple T Agent/memories/MEMORY.md` | All daily files merged into main memory. |
...
| Source | OpenClaw location | Seple T Agent destination |
|--------|------------------|-------------------|
| Workspace skills | `workspace/skills/` | `~/.Seple T Agent/skills/openclaw-imports/` |
| Managed/shared skills | `~/.openclaw/skills/` | `~/.Seple T Agent/skills/openclaw-imports/` |
| Personal cross-project | `~/.agents/skills/` | `~/.Seple T Agent/skills/openclaw-imports/` |
| Project-level shared | `workspace/.agents/skills/` | `~/.Seple T Agent/skills/openclaw-imports/` |

Skill conflicts are handled by `--skill-conflict`: `skip` leaves the existing Seple T Agent skill, `overwrite` replaces it, `rename` creates a `-imported` copy.

### Model and provider configuration

| What | OpenClaw config path | Seple T Agent destination | Notes |
|------|---------------------|-------------------|-------|
...
| Provider API keys | `models.providers.*.apiKey` | `~/.Seple T Agent/.env` | Requires `--migrate-secrets`. See [API key resolution](#api-key-resolution) below. |

### Agent behavior

| What | OpenClaw config path | Seple T Agent config path | Mapping |
|------|---------------------|-------------------|---------|
...
| OpenClaw config path | Seple T Agent config path | Notes |
|---------------------|-------------------|-------|
...
| OpenClaw field | Seple T Agent field | Notes |
|----------------|-------------|-------|
...
| What | Seple T Agent destination |
|------|-------------------|
...
| TTS assets | `~/.Seple T Agent/tts/` (file copy) |

### Messaging platforms

| Platform | OpenClaw config path | Seple T Agent `.env` variable | Notes |
|----------|---------------------|----------------------|-------|
...
| What | OpenClaw path | Seple T Agent path | Notes |
|------|-------------|-------------|-------|
...
### Archived (no direct Seple T Agent equivalent)

These are saved to `~/.Seple T Agent/migration/openclaw/<timestamp>/archive/` for manual review:

| What | Archive file | How to recreate in Seple T Agent |
|------|-------------|--------------------------|
| `IDENTITY.md` | `archive/workspace/IDENTITY.md` | Merge into `SOUL.md` |
| `TOOLS.md` | `archive/workspace/TOOLS.md` | Seple T Agent has built-in tool instructions |
...
| Cron jobs | `archive/cron-config.json` | Recreate with `Seple T Agent cron create` |
| Plugins | `archive/plugins-config.json` | See [plugins guide](/user-guide/features/hooks) |
| Hooks/webhooks | `archive/hooks-config.json` | Use `Seple T Agent webhook` or gateway hooks |
| Memory backend | `archive/memory-backend-config.json` | Configure via `Seple T Agent honcho` |
| Skills registry | `archive/skills-registry-config.json` | Use `Seple T Agent skills config` |
...
| Multi-agent list | `archive/agents-list.json` | Use Seple T Agent profiles |
...
The migration resolves all three formats. For env templates and SecretRef objects with `source: "env"`, it looks up the value in `~/.openclaw/.env` and the `openclaw.json` env sub-object. SecretRef objects with `source: "file"` or `source: "exec"` can't be resolved automatically — the migration warns about these, and those values must be added to Seple T Agent manually via `Seple T Agent config set`.

## After migration

1. **Check the migration report** — printed on completion with counts of migrated, skipped, and conflicting items.

2. **Review archived files** — anything in `~/.Seple T Agent/migration/openclaw/<timestamp>/archive/` needs manual attention.

3. **Start a new session** — imported skills and memory entries take effect in new sessions, not the current one.

4. **Verify API keys** — run `Seple T Agent status` to check provider authentication.

5. **Test messaging** — if you migrated platform tokens, restart the gateway: `systemctl --user restart hermes-gateway`

6. **Check session policies** — run `Seple T Agent config show` and verify the `session_reset` value matches your expectations.

7. **Re-pair WhatsApp** — WhatsApp uses QR code pairing (Baileys), not token migration. Run `Seple T Agent whatsapp` to pair.

8. **Archive cleanup** — after confirming everything works, run `Seple T Agent claw cleanup` to rename leftover OpenClaw directories to `.pre-migration/` (prevents state confusion).
...
Keys might be stored in several places depending on your OpenClaw version: inline in `openclaw.json` under `models.providers.*.apiKey`, in `~/.openclaw/.env`, in the `openclaw.json` `"env"` sub-object, or in `agents/main/agent/auth-profiles.json`. The migration checks all four. If keys use `source: "file"` or `source: "exec"` SecretRefs, they can't be resolved automatically — add them via `Seple T Agent config set`.

### Skills not appearing after migration

Imported skills land in `~/.Seple T Agent/skills/openclaw-imports/`. Start a new session for them to take effect, or run `/skills` to verify they're loaded.

### TTS voice not migrated

OpenClaw stores TTS settings in two places: `messages.tts.providers.*` and the top-level `talk` config. The migration checks both. If your voice ID was set via the OpenClaw UI (stored in a different path), you may need to set it manually: `Seple T Agent config set tts.elevenlabs.voice_id YOUR_VOICE_ID`.
