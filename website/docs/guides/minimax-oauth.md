...
sidebar_position: 15
title: "MiniMax OAuth"
description: "Log into MiniMax via browser OAuth and use MiniMax-M2.7 models in Seple T Agent — no API key required"
---

# MiniMax OAuth

Seple T Agent supports **MiniMax** through a browser-based OAuth login flow, using the same credentials as the [MiniMax portal](https://www.minimax.io). No API key or credit card is required — log in once and Seple T Agent automatically refreshes your session.
...
# → Seple T Agent opens your browser to the MiniMax authorization page
...
After the first login, credentials are stored under `~/.Seple T Agent/auth.json` and are refreshed automatically before each session.
...
```bash
Seple T Agent auth add minimax-oauth
```
...
```bash
echo 'MINIMAX_CN_API_KEY=your-key' >> ~/.Seple T Agent/.env
```
...
```bash
Seple T Agent auth add minimax-oauth --no-browser
```

Seple T Agent will print the verification URL and user code — open the URL on any device and enter the code when prompted.

## The OAuth Flow

Seple T Agent implements a PKCE browser OAuth flow against the MiniMax OAuth endpoints:

1. Seple T Agent generates a PKCE verifier / challenge pair and a random state value.
...
4. Seple T Agent polls `{base_url}/oauth/token` until the token arrives (or the deadline passes).
5. Tokens (`access_token`, `refresh_token`, expiry) are saved to `~/.Seple T Agent/auth.json` under the `minimax-oauth` key.
...
```bash
Seple T Agent config set model.default MiniMax-M2.7
Seple T Agent config set model.provider minimax-oauth
```

## Configuration Reference

After login, `~/.Seple T Agent/config.yaml` will contain entries similar to:
...
```bash
Seple T Agent --provider minimax-oauth    # canonical
Seple T Agent --provider minimax-portal   # alias
Seple T Agent --provider minimax-global   # alias
Seple T Agent --provider minimax_oauth    # alias (underscore form)
```
...
To use `minimax-oauth` as the active provider, set `model.provider: minimax-oauth` in `config.yaml` (use `Seple T Agent setup` for the guided flow), or pass `--provider minimax-oauth` for a single invocation:

```bash
Seple T Agent --provider minimax-oauth
```
...
Seple T Agent refreshes the token on every session start if it is within 60 seconds of expiry. If the access token is already expired (for example, after a long offline period), the refresh happens automatically on the next request. If refresh fails with `refresh_token_reused` or `invalid_grant`, Seple T Agent marks the session as requiring re-login.
...
**Fix:** run `Seple T Agent auth add minimax-oauth` again to start a fresh login. The quarantine clears on the next successful exchange.

### Authorization timed out

The device-code flow has a finite expiry window. If you don't approve the login in time, Seple T Agent raises a timeout error.

**Fix:** re-run `Seple T Agent auth add minimax-oauth` (or `Seple T Agent model`). The flow starts fresh.

### State mismatch (possible CSRF)

Seple T Agent detected that the `state` value returned by the authorization server does not match what it sent.
...
If `Seple T Agent` cannot open a browser window, use `--no-browser`:

```bash
Seple T Agent auth add minimax-oauth --no-browser
```

Seple T Agent prints the URL and code. Open the URL on any device and complete the flow there.
...
**Fix:** run `Seple T Agent model` and select MiniMax (OAuth), or run `Seple T Agent auth add minimax-oauth`.
...
```bash
Seple T Agent auth logout minimax-oauth
```
...
- [Seple T Agent doctor](../reference/cli-commands.md)
