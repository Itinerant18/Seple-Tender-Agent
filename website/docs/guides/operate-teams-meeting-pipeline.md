...
```bash
Seple T Agent teams-pipeline validate
```
...
```bash
Seple T Agent teams-pipeline token-health
Seple T Agent teams-pipeline token-health --force-refresh
```
...
```bash
Seple T Agent teams-pipeline subscriptions
```

### Renew near-expiry subscriptions

```bash
Seple T Agent teams-pipeline maintain-subscriptions
Seple T Agent teams-pipeline maintain-subscriptions --dry-run
```
...
#### Option 1: Seple T Agent cron (recommended if you already run the Seple T Agent gateway)

Seple T Agent ships a built-in cron scheduler. The `--no-agent` mode runs a script as the job (rather than using an LLM), and `--script` must point at a file under `~/.Seple T Agent/scripts/`. First create the script:

```bash
mkdir -p ~/.Seple T Agent/scripts
cat > ~/.Seple T Agent/scripts/maintain-teams-subscriptions.sh <<'EOF'
#!/usr/bin/env bash
exec Seple T Agent teams-pipeline maintain-subscriptions
EOF
chmod +x ~/.Seple T Agent/scripts/maintain-teams-subscriptions.sh
```

Then register a script-only cron job that runs every 12 hours (gives 6x headroom against the 72h expiry window):

```bash
Seple T Agent cron create "0 */12 * * *" \
...
```ini
[Unit]
Description=Seple T Agent Teams pipeline subscription maintenance
After=network-online.target

[Service]
Type=oneshot
User=Seple T Agent
EnvironmentFile=/etc/Seple T Agent/env
ExecStart=/usr/local/bin/Seple T Agent teams-pipeline maintain-subscriptions
```
...
```ini
[Unit]
Description=Run Seple T Agent Teams pipeline subscription maintenance every 12 hours
...
```cron
0 */12 * * * /usr/local/bin/Seple T Agent teams-pipeline maintain-subscriptions >> /var/log/Seple T Agent/teams-pipeline-maintain.log 2>&1
```

Make sure the cron environment has the `MSGRAPH_*` credentials. Simplest fix: source `~/.Seple T Agent/.env` at the top of a wrapper script that crontab calls.
...
```bash
Seple T Agent teams-pipeline subscriptions   # should show expirationDateTime advanced
Seple T Agent teams-pipeline maintain-subscriptions --dry-run   # should show "0 expiring soon" most of the time
```
...
```bash
Seple T Agent teams-pipeline list
Seple T Agent teams-pipeline list --status failed
Seple T Agent teams-pipeline show <job-id>
```

### Replay a stored job

```bash
Seple T Agent teams-pipeline run <job-id>
```

### Dry-run meeting artifact fetches

```bash
Seple T Agent teams-pipeline fetch --meeting-id <meeting-id>
Seple T Agent teams-pipeline fetch --join-web-url "<join-url>"
```
...
- run `Seple T Agent teams-pipeline maintain-subscriptions --dry-run`
- inspect `Seple T Agent teams-pipeline list --status failed`
...
- run `Seple T Agent teams-pipeline validate`
...
Check:
- whether you manually replayed a job with `Seple T Agent teams-pipeline run`
...
- [ ] `Seple T Agent teams-pipeline validate` returns an OK snapshot
- [ ] `Seple T Agent teams-pipeline token-health --force-refresh` succeeds
- [ ] **`maintain-subscriptions` is scheduled** (Seple T Agent cron, systemd timer, or crontab — see [Automating subscription renewal](#automating-subscription-renewal-required-for-production)). Without this, Graph subscriptions silently expire within 72 hours.
