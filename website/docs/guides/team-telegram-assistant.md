...
This tutorial walks you through setting up a Telegram bot powered by Seple T Agent that multiple team members can use. By the end, your team will have a shared AI assistant they can message for help with code, research, system administration, and anything else — secured with per-user authorization.
...
- **Seple T Agent installed** on a server or VPS (not your laptop — the bot needs to stay running). Follow the [installation guide](/getting-started/installation) if you haven't yet.
- **A Telegram account** for yourself (the bot owner)
- **An LLM provider configured** — at minimum, an API key for OpenAI, Anthropic, or another supported provider in `~/.Seple T Agent/.env`

:::tip
A $5/month VPS is plenty for running the gateway. Seple T Agent itself is lightweight — the LLM API calls are what cost money, and those happen remotely.
:::
...
2. **Send `/newbot`** — BotFather will ask you two things:
   - **Display name** — what users see (e.g., `Team Seple T Agent Assistant`)
...
4. **Set a description** (optional but recommended):
   ...
   ```
   Team AI assistant powered by Seple T Agent. DM me for help with code, research, debugging, and more.
   ```
...
Add these lines to `~/.Seple T Agent/.env`:
...
```
[Gateway] Starting Seple T Agent Gateway...
...
:::tip macOS PATH
The launchd plist captures your shell PATH at install time so gateway subprocesses can find tools like Node.js and ffmpeg. If you install new tools later, re-run `Seple T Agent gateway install` to update the plist.
:::
...
```bash
Seple T Agent gateway stop && Seple T Agent gateway start
```
...
Seple T Agent pairing revoke telegram 987654321
...
**Option 2:** Set it manually in `~/.Seple T Agent/.env`:
...
Control how much detail the bot shows when using tools. In `~/.Seple T Agent/config.yaml`:
...
Customize how the bot communicates by editing `~/.Seple T Agent/SOUL.md`:

For a full guide, see [Use SOUL.md with Seple T Agent](/guides/use-soul-with-hermes).
...
```bash
# From the CLI
Seple T Agent cron list          # View all scheduled jobs
Seple T Agent cron status        # Check if scheduler is running
...
| What | Location |
|------|----------|
| Gateway logs | `journalctl --user -u hermes-gateway` (Linux) or `~/.Seple T Agent/logs/gateway.log` (macOS) |
| Cron job output | `~/.Seple T Agent/cron/output/{job_id}/{timestamp}.md` |
| Cron job definitions | `~/.Seple T Agent/cron/jobs.json` |
| Pairing data | `~/.Seple T Agent/pairing/` |
| Session history | `~/.Seple T Agent/sessions/` |
