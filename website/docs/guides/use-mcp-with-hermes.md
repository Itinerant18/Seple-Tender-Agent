---
sidebar_position: 6
title: "Use MCP with Seple T Agent"
description: "A practical guide to connecting MCP servers to Seple T Agent, filtering their tools, and using them safely in real workflows"
---

# Use MCP with Seple T Agent

This guide shows how to actually use MCP with Seple T Agent in day-to-day workflows.
...
Use MCP when:
- a tool already exists in MCP form and you do not want to build a native Seple T Agent tool
- you want Seple T Agent to operate against a local or remote system through a clean RPC layer
- you want fine-grained per-server exposure control
- you want to connect Seple T Agent to internal APIs, databases, or company systems without modifying Seple T Agent core

Do not use MCP when:
- a built-in Seple T Agent tool already solves the job well
...
- Seple T Agent remains the agent
- MCP servers contribute tools
- Seple T Agent discovers those tools at startup or reload time
...
If you installed Seple T Agent with the standard install script, MCP support is already included (the installer runs `uv pip install -e ".[all]"`).
...
- Seple T Agent banner/status should show MCP integration when configured
- ask Seple T Agent what tools it has available
...
## WSL2: bridge Seple T Agent in WSL to Windows Chrome

This is the practical setup when:

- Seple T Agent runs inside WSL2
...
In this setup, Seple T Agent does **not** connect to Chrome directly. Instead:

- Seple T Agent runs in WSL
- Seple T Agent starts a local stdio MCP server
...
```text
Seple T Agent (WSL) -> MCP stdio bridge -> Windows Chrome
```

### Why this mode is useful

- you keep your real Windows browser profile, cookies, and logins
- Seple T Agent stays in its supported Unix environment (WSL2)
- browser control is exposed as MCP tools instead of relying on Seple T Agent core browser transport
...
```bash
Seple T Agent mcp add chrome-devtools-win --command cmd.exe --args /c npx -y chrome-devtools-mcp@latest --autoConnect --no-usage-statistics
```

After saving the server:

```bash
Seple T Agent mcp test chrome-devtools-win
```

Then start a fresh Seple T Agent session or run:
...
Once loaded, Seple T Agent can use the MCP-prefixed browser tools directly. For example:
...
If Seple T Agent runs in WSL and Chrome runs on Windows, `/browser connect` may fail even though Chrome is open and debuggable.
...
- Start Seple T Agent from a Windows-mounted path like `/mnt/c/Users/<you>` or `/mnt/c/workspace/...` when using Windows stdio executables through MCP.
- If you start Seple T Agent from `/root` or `/home/...`, Windows may emit a `UNC` current-directory warning before the MCP server starts.
...
There are two categories of MCP-exposed functionality in Seple T Agent:
...
2. Seple T Agent-added utility wrappers
- filtered with:
  - `tools.resources`
  - `tools.prompts`
...
So Seple T Agent will not pretend a server has resources/prompts if it does not.

## Common patterns

### Pattern 1: local project assistant

Use MCP for a repo-local filesystem or git server when you want Seple T Agent to reason over a bounded workspace.
...
Use [Open Scaffold](https://github.com/graphanov/open-scaffold) when you want Seple T Agent to read a repository's durable AI-work record: mission, plans, evidence notes, handoff packets, and review/gate results. Seple T Agent remains the agent; Open Scaffold remains the repo-local record.

Add the server for one scaffolded repository:

```bash
Seple T Agent mcp add open_scaffold --command npx --args -y open-scaffold@latest mcp serve --repo /absolute/path/to/repo
Seple T Agent mcp test open_scaffold
```

Then keep the exposed surface read-oriented. Choose `select` in the `Seple T Agent mcp add` prompt, or edit `config.yaml` afterward:
...
- Its write tools require the server to be started with `--allow-write`; do not enable that until you explicitly want Seple T Agent to mutate `.osc` files.
- Open Scaffold records and gates work; it does not authorize Seple T Agent to merge, publish, deploy, or spawn runtimes.
...
Now Seple T Agent can combine them:

```text
Inspect the local project files, then create a GitHub issue summarizing the bug you find.
```

That is where MCP gets powerful: multi-system workflows without changing Seple T Agent core.
...
Because Seple T Agent now respects your per-server policy and capability-aware registration. That is expected, and usually desirable.
