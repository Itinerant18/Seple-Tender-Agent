...
of fields, and Seple T Agent schedules it as a cron job — no cron syntax required.
...
- **CLI, TUI, and messengers** — type `/blueprint <name>` (e.g.
  `/blueprint morning-brief`) and Seple T Agent asks you for what it needs, one
...
A blueprint is just a skill with a `metadata.Seple T Agent.blueprint` block in its
