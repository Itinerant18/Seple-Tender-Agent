...
description: "Frequently asked questions and solutions to common issues with Seple T Agent"
---
...
### What LLM providers work with Seple T Agent?

Seple T Agent works with any OpenAI-compatible API. Supported providers include:

- **[OpenRouter](https://openrouter.ai/)** — access hundreds of models through one API key (recommended for flexibility)
- **[Nous Portal](/integrations/nous-portal)** — Novaedge's subscription gateway — 300+ models plus web/image/TTS/browser through one OAuth login (recommended for newcomers)
- **OpenAI** — GPT-5.4, GPT-5-codex, GPT-4.1, GPT-4o, etc.
- **Anthropic** — Claude models (direct API, OAuth via `Seple T Agent auth add anthropic`, OpenRouter, or any compatible proxy)
...
Set your provider with `Seple T Agent model` or by editing `~/.Seple T Agent/.env`. See the [Environment Variables](./environment-variables.md) reference for all provider keys.
...
### I run Seple T Agent in WSL2. What's the best way to control my normal Windows Chrome?

Prefer an MCP bridge over `/browser connect`.

Recommended pattern:

- run Seple T Agent inside WSL2
...
- let Seple T Agent use the resulting MCP browser tools

This is more reliable than trying to force Seple T Agent core browser transport to attach directly across the WSL2/Windows boundary.

See:

- [Use MCP with Seple T Agent](../guides/use-mcp-with-hermes.md#wsl2-bridge-hermes-in-wsl-to-windows-chrome)
- [Browser Automation](../user-guide/features/browser.md#wsl2--windows-chrome-prefer-mcp-over-browser-connect)

### Is my data sent anywhere?

API calls go **only to the LLM provider you configure** (e.g., OpenRouter, your local Ollama instance). Seple T Agent does not collect telemetry, usage data, or analytics. Your conversations, memory, and skills are stored locally in `~/.Seple T Agent/`.

### Can I use it offline / with local models?

Yes. Run `Seple T Agent model`, select **Custom endpoint**, and enter your server's URL:
...
# Context length: 64000   ← Seple T Agent minimum; set this to match your server's actual context window
```
...
Seple T Agent persists the endpoint, provider, and base URL in `config.yaml` so it survives restarts. If your local server has exactly one model loaded, `/model custom` auto-detects it. You can also set `provider: custom` in config.yaml — it's a first-class provider, not an alias for anything else.
...
:::tip Ollama users
If you set a custom `num_ctx` in Ollama (e.g., `ollama run --num_ctx 64000`), make sure to set the matching context length in Seple T Agent — Ollama's `/api/show` reports the model's *maximum* context, not the effective `num_ctx` you configured.
:::

:::tip Timeouts with local models
Seple T Agent auto-detects local endpoints and relaxes streaming timeouts (read timeout raised from 120s to 1800s, stale stream detection disabled). If you still hit timeouts on very large contexts, set `HERMES_STREAM_READ_TIMEOUT=1800` in your `.env`. See the [Local LLM guide](../guides/local-llm-on-mac.md#timeouts) for details.
:::

### How much does it cost?

Seple T Agent itself is **free and open-source** (MIT license). You pay only for the LLM API usage from your chosen provider. Local models are completely free to run.

### Can multiple people use one instance?

Yes. The [messaging gateway](../user-guide/messaging/index.md) lets multiple users interact with the same Seple T Agent instance via Telegram, Discord, Slack, WhatsApp, or Home Assistant. Access is controlled through allowlists (specific user IDs) and DM pairing (first user to message claims access).
...
Yes. Import the `AIAgent` class and use Seple T Agent programmatically:
...
#### `Seple T Agent: command not found` after installation
...
**Cause:** Seple T Agent requires Python 3.11 or newer.
...
**Cause:** Seple T Agent builds a per-session environment snapshot by running `bash -l` once at startup. A bash login shell reads `/etc/profile`, `~/.bash_profile`, and `~/.profile`, but **does not source `~/.bashrc`** — so tools that install themselves there (`nvm`, `asdf`, `pyenv`, `cargo`, custom `PATH` exports) stay invisible to the snapshot. This most commonly happens when Seple T Agent runs under systemd or in a minimal shell where nothing has pre-loaded the interactive shell profile.

**Solution:** Seple T Agent auto-sources `~/.bashrc` by default. If that's not enough — e.g. you're a zsh user whose PATH lives in `~/.zshrc`, or you init `nvm` from a standalone file — list the extra files to source in `~/.Seple T Agent/config.yaml`:
...
sudo rm /usr/local/bin/Seple T Agent
...
**Solution:** Exit your session and use `Seple T Agent model` from your terminal to add new providers:

```bash
# Exit the Seple T Agent chat session first (Ctrl+C or /quit)
...
After adding a new provider via `Seple T Agent model`, start a new chat session — `/model` will now show all your configured providers.

:::tip Quick reference
| Want to... | Use |
|-----------|-----|
| Add a new provider | `Seple T Agent model` (from terminal) |
| Enter/change API keys | `Seple T Agent model` (from terminal) |
...
Seple T Agent config show

# Re-configure your provider
Seple T Agent model

# Or set directly
Seple T Agent config set OPENROUTER_API_KEY sk-or-v1-xxxxxxxxxxxx
```

:::warning
Make sure the key matches the provider. An OpenAI key won't work with OpenRouter and vice versa. Check `~/.Seple T Agent/.env` for conflicting entries.
:::
...
# Set a valid model
Seple T Agent config set HERMES_MODEL anthropic/claude-opus-4.7

# Or specify per-session
Seple T Agent chat --model openrouter/meta-llama/llama-3.1-70b-instruct
```
...
- Using `Seple T Agent chat --provider <alternative>` to route to a different backend

#### Context length exceeded

**Cause:** The conversation has grown too long for the model's context window, or Seple T Agent detected the wrong context length for your model.
...
Seple T Agent chat --model openrouter/google/gemini-3-flash-preview
```

If this happens on the first long conversation, Seple T Agent may have the wrong context length for your model. Check what it detected:
...
**Cause:** Seple T Agent detected a potentially destructive command (e.g., `rm -rf`, `DROP TABLE`). This is a safety feature.
...
:::tip
This is working as intended — Seple T Agent never silently runs destructive commands. The approval prompt shows you exactly what will execute.
:::
...
- Or switch to the terminal interface for administrative tasks: `Seple T Agent chat`
...
cat ~/.Seple T Agent/logs/gateway.log | tail -50
```
...
**Solution:**
- Verify your bot token is valid with `Seple T Agent gateway setup`
- Check gateway logs: `cat ~/.Seple T Agent/logs/gateway.log | tail -50`
...
Configure in `~/.Seple T Agent/config.yaml` under your gateway's settings. See the [Messaging docs](../user-guide/messaging/index.md).
...
cd ~/.Seple T Agent/hermes-agent && uv pip install -e ".[messaging]"  # Telegram, Discord, Slack, and shared gateway deps
...
#### WSL: Gateway keeps disconnecting or `Seple T Agent gateway start` fails
...
Seple T Agent gateway run

# Option 2: Persistent via tmux (survives terminal close)
tmux new -s Seple T Agent 'Seple T Agent gateway run'
# Reattach later: tmux attach -t Seple T Agent

# Option 3: Background via nohup
nohup Seple T Agent gateway run > ~/.Seple T Agent/logs/gateway.log 2>&1 &
```
...
1. Create a task that runs `wsl -d Ubuntu -- bash -lc 'Seple T Agent gateway run'`
...
**Solution:** The gateway captures your shell PATH when you run `Seple T Agent gateway install`. If you installed tools after setting up the gateway, re-run the install to capture the updated PATH:

```bash
Seple T Agent gateway install    # Re-snapshots your current PATH
Seple T Agent gateway start      # Detects the updated plist and reloads
```

You can verify the plist has the correct PATH:
```bash
/usr/libexec/PlistBuddy -c "Print :EnvironmentVariables:PATH" \
  ~/Library/LaunchAgents/ai.Seple T Agent.gateway.plist
```
...
**Solution:**
- Try a faster/smaller model: `Seple T Agent chat --model openrouter/meta-llama/llama-3.1-8b-instruct`
- Reduce active toolsets: `Seple T Agent chat -t "terminal"`
...
cd ~/.Seple T Agent/hermes-agent && uv pip install -e ".[mcp]"
...
Verify your `~/.Seple T Agent/config.yaml` MCP configuration:
```yaml
mcp_servers:
  filesystem:
    command: "npx"
...
Seple T Agent config show | grep -A 12 mcp_servers

# Restart Seple T Agent or reload MCP after config changes
Seple T Agent chat
```

See also:
- [MCP (Model Context Protocol)](/user-guide/features/mcp)
- [Use MCP with Seple T Agent](/guides/use-mcp-with-hermes)
...
:::warning
If an MCP server crashes mid-request, Seple T Agent will report a timeout. Check the server's own logs (not just Seple T Agent logs) to diagnose the root cause.
:::
...
No. Each profile has its own memory store, session database, and skills directory. They are completely isolated. If you want to start a new profile with existing memories and sessions, use `Seple T Agent profile create newname --clone-all` to copy everything from the current profile, or add `--clone-from <profile>` to copy from a specific source profile.

### What happens when I run `Seple T Agent update`?

`Seple T Agent update` pulls the latest code and reinstalls dependencies **once** (not per-profile). It then syncs updated skills to all profiles automatically. You only need to run `Seple T Agent update` once — it covers every profile on the machine.


### How many profiles can I run?

There is no hard limit. Each profile is just a directory under `~/.Seple T Agent/profiles/`. The practical limit depends on your disk space and how many concurrent gateways your system can handle (each gateway is a lightweight Python process). Running dozens of profiles is fine; each idle profile uses no resources.
...
**Solution: Delegation config.** Seple T Agent can route subagents to a different model automatically. Set this in `~/.Seple T Agent/config.yaml`:
...
Now when you tell Seple T Agent "write me a Twitter thread about X" and it spawns a `delegate_task` subagent, that subagent runs on Gemini instead of your main model. Your primary conversation stays on GPT-5.4.
...
**Scenario:** In OpenClaw, you had multiple independent agents bound to specific WhatsApp chats — one for a family shopping list group, another for your private chat. Can Seple T Agent do this?

**Current limitation:** Seple T Agent profiles each require their own WhatsApp number/session. You cannot bind multiple profiles to different chats on the same WhatsApp number — the WhatsApp bridge (Baileys) uses one authenticated session per number.
...
**Scenario:** You see gateway exec logs, Seple T Agent reasoning, and tool call details in Telegram instead of just the final output.
...
**Scenario:** Telegram has a 100 slash command limit, and your skills are pushing past it. You want to disable skills you don't need on Telegram, but `Seple T Agent skills config` settings don't seem to take effect.

**Solution:** Use `Seple T Agent skills config` to disable skills per-platform. This writes to `config.yaml`:
...
After changing this, **restart the gateway** (`Seple T Agent gateway restart` or kill and relaunch). The Telegram bot command menu rebuilds on startup.
...
**Current behavior:** Seple T Agent creates sessions keyed by user ID on most platforms, so each person gets their own conversation context. This is by design for privacy and context isolation.
...
### Exporting Seple T Agent to another machine
...
1. Install Seple T Agent on the new machine:
   ```bash
   curl -fsSL https://hermes-agent.Novaedge.com/install.sh | bash
   ```
...
2. On the **source machine**, create a full backup:
   ...
   ```
   This creates a zip of your entire `~/.Seple T Agent/` directory — config, API keys, memories, skills, sessions, and profiles — saved to your home directory as `~/hermes-backup-<timestamp>.zip`.
...
3. Copy the zip to the new machine and import it:
   ...
   # On the new machine
   Seple T Agent import ~/hermes-backup-<timestamp>.zip
   ```

4. On the new machine, run `Seple T Agent setup` to verify API keys and provider config are working.
...
```bash
# On the source machine
Seple T Agent profile export work ./work-backup.tar.gz

# Copy the file to the target machine, then:
Seple T Agent profile import ./work-backup.tar.gz work
```
...
### `Seple T Agent backup` vs `Seple T Agent profile export`

| Feature | `Seple T Agent backup` | `Seple T Agent profile export` |
| :--- | :--- | :--- |
| **Use Case** | **Full machine migration** | **Porting/sharing a specific profile** |
| **Scope** | Global (entire `~/.Seple T Agent` directory) | Local (single profile directory) |
...
```bash
rsync -av --exclude='hermes-agent' ~/.Seple T Agent/ newmachine:~/.Seple T Agent/
```

:::tip
`Seple T Agent backup` produces a consistent snapshot even while Seple T Agent is actively running. The restored archive excludes machine-local runtime files like `gateway.pid` and `cron.pid`.
:::

### Permission denied when reloading shell after install

**Scenario:** After running the Seple T Agent installer, `source ~/.zshrc` gives a permission denied error.

**Cause:** This usually happens when `~/.zshrc` (or `~/.bashrc`) has incorrect file permissions, or when the installer couldn't write to it cleanly. It's not a Seple T Agent-specific issue — it's a shell config permissions problem.
...
Seple T Agent config show | head -20

# Re-run model selection
Seple T Agent model

# Or test with a known-good model
Seple T Agent chat -q "hello" --model anthropic/claude-opus-4.7
```
...
1. **Search existing issues:** [GitHub Issues](https://github.com/Novaedge/hermes-agent/issues)
2. **Ask the community:** [Novaedge Discord](https://discord.gg/Novaedge)
3. **File a bug report:** Include your OS, Python version (`python3 --version`), Seple T Agent version (`Seple T Agent --version`), and the full error message
