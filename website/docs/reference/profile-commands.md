...
This page covers all commands related to [Seple T Agent profiles](../user-guide/profiles.md). For general CLI commands, see [CLI Commands Reference](./cli-commands.md).
...
Top-level command for managing profiles. Running `Seple T Agent profile` without a subcommand shows help.
...
## `Seple T Agent profile use`

```bash
Seple T Agent profile use <name>
```

Sets `<name>` as the active profile. All subsequent `Seple T Agent` commands (without `-p`) will use this profile.
...
Seple T Agent profile use default
```

## `Seple T Agent profile create`

```bash
Seple T Agent profile create <name> [options]
```
...
| `--description "<text>"` | One- or two-sentence description of what this profile is good at. Used by the kanban orchestrator to route tasks based on role instead of profile name alone. Skip and add later via `Seple T Agent profile describe`. Persisted in `<profile_dir>/profile.yaml`. |
| `--no-skills` | Create an **empty** profile with zero bundled skills enabled. Writes a `.no-bundled-skills` marker into the profile so future `Seple T Agent update` runs won't re-seed the bundled set, and refuses to combine with `--clone`, `--clone-from`, or `--clone-all` (which would copy skills in anyway). Useful for narrow orchestrator profiles or sandbox profiles that should not inherit the full skill catalog. To toggle this on an already-created profile (including the default `~/.Seple T Agent`), use `Seple T Agent skills opt-out` / `Seple T Agent skills opt-in`. |
...
Seple T Agent profile create mybot

# Clone config only from current profile
Seple T Agent profile create work --clone

# Clone everything from current profile
Seple T Agent profile create backup --clone-all

# Clone config from a specific profile
Seple T Agent profile create work2 --clone-from work

# Clone everything from a specific profile
Seple T Agent profile create work2-backup --clone-from work --clone-all
```

## `Seple T Agent profile describe`

```bash
Seple T Agent profile describe [<name>] [options]
```
...
Seple T Agent profile describe researcher

# Set it explicitly
Seple T Agent profile describe researcher --text "Reads source code and writes findings."

# Let the LLM generate one
Seple T Agent profile describe researcher --auto

# Fill in descriptions for every profile that doesn't have one
Seple T Agent profile describe --all --auto
```

## `Seple T Agent profile delete`

```bash
Seple T Agent profile delete <name> [options]
```
...
```bash
Seple T Agent profile delete mybot
Seple T Agent profile delete mybot --yes
```
...
## `Seple T Agent profile show`

```bash
Seple T Agent profile show <name>
```

Displays details about a profile including its home directory, configured model, gateway status, skills count, and configuration file status.

This shows the profile's Seple T Agent home directory, not the terminal working directory. Terminal commands start from `terminal.cwd` (or the launch directory on the local backend when `cwd: "."`).
...
```bash
$ Seple T Agent profile show work
Profile: work
Path:    ~/.Seple T Agent/profiles/work
...
## `Seple T Agent profile alias`

```bash
Seple T Agent profile alias <name> [options]
```

Regenerates the shell alias script at `~/.local/bin/<name>`. Useful if the alias was accidentally deleted or if you need to update it after moving your Seple T Agent installation.
...
```bash
Seple T Agent profile alias work
# Creates/updates ~/.local/bin/work

Seple T Agent profile alias work --name mywork
# Creates ~/.local/bin/mywork

Seple T Agent profile alias work --remove
# Removes the wrapper script
```

## `Seple T Agent profile rename`

```bash
Seple T Agent profile rename <old-name> <new-name>
```
...
```bash
Seple T Agent profile rename mybot assistant
# ~/.Seple T Agent/profiles/mybot → ~/.Seple T Agent/profiles/assistant
# ~/.local/bin/mybot → ~/.local/bin/assistant
```

## `Seple T Agent profile export`

```bash
Seple T Agent profile export <name> [options]
```
...
```bash
Seple T Agent profile export work
# Creates work.tar.gz in the current directory

Seple T Agent profile export work -o ./work-2026-03-29.tar.gz
```

## `Seple T Agent profile import`

```bash
Seple T Agent profile import <archive> [options]
```
...
```bash
Seple T Agent profile import ./work-2026-03-29.tar.gz
# Infers profile name from the archive

Seple T Agent profile import ./work-2026-03-29.tar.gz --name work-restored
```
...
:::info
`Seple T Agent profile export` / `import` are still the right commands for
...
### `Seple T Agent profile install`

```bash
Seple T Agent profile install <source> [--name <name>] [--alias] [--force] [--yes]
```
...
| `--alias` | Also create a shell wrapper (e.g. `telemetry` → `Seple T Agent -p telemetry`). |
...
Seple T Agent profile install github.com/kyle/telemetry-distribution --alias

# Install from a full HTTPS git URL
Seple T Agent profile install https://github.com/kyle/telemetry-distribution.git

# Install from SSH
Seple T Agent profile install git@github.com:kyle/telemetry-distribution.git

# Install from a local directory during development
Seple T Agent profile install ./telemetry/
```

### `Seple T Agent profile update`

```bash
Seple T Agent profile update <name> [--force-config] [--yes]
```
...
### `Seple T Agent profile info`

```bash
Seple T Agent profile info <name>
```

Prints the profile's distribution manifest — name, version, required
Seple T Agent version, author, env var requirements, the source URL/path, and
...
`Seple T Agent profile list` also shows the distribution name and version in a
`Distribution` column, and `Seple T Agent profile show <name>` / `delete <name>`
...
Seple T Agent profile install git@github.com:your-org/internal-assistant.git

# Uses your git credential helper
Seple T Agent profile install https://github.com/your-org/internal-assistant.git
```
...
Seple T Agent version doesn't satisfy the spec.
...
2. Initialize a git repo (or use an existing one) and push to GitHub /
   GitLab / any host Seple T Agent can clone from.
3. Tell recipients to run `Seple T Agent profile install <your-repo-url>`.
...
## `Seple T Agent -p` / `Seple T Agent --profile`

```bash
Seple T Agent -p <name> <command> [options]
Seple T Agent --profile <name> <command> [options]
```

Global flag to run any Seple T Agent command under a specific profile without changing the sticky default. This overrides the active profile for the duration of the command.
...
```bash
Seple T Agent -p work chat -q "Check the server status"
Seple T Agent --profile dev gateway start
Seple T Agent -p personal skills list
Seple T Agent -p work config edit
```

## `Seple T Agent completion`

```bash
Seple T Agent completion <shell>
```
...
- `Seple T Agent profile <TAB>` — subcommands (list, use, create, etc.)
- `Seple T Agent profile use <TAB>` — profile names
- `Seple T Agent -p <TAB>` — profile names
