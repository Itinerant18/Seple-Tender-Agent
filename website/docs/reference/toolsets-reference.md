...
description: "Reference for Seple T Agent core, composite, platform, and dynamic toolsets"
---
...
```bash
Seple T Agent chat --toolsets web,file,terminal
Seple T Agent chat --toolsets debugging        # composite — expands to file + terminal + web
Seple T Agent chat --toolsets all              # everything
```
...
```bash
Seple T Agent tools                            # curses UI to enable/disable per platform
```
...
| `code_execution` | `execute_code` | Run Python scripts that call Seple T Agent tools programmatically. |
...
| `x_search` | `x_search` | Search X (Twitter) posts and threads via xAI's built-in `x_search` Responses tool. Off by default; opt in via `Seple T Agent tools`. Schema only registered when xAI credentials (SuperGrok OAuth or `XAI_API_KEY`) are configured. |
...
## Relationship to `Seple T Agent tools`

The `Seple T Agent tools` command provides a curses-based UI for toggling individual tools on or off per platform. This operates at the tool level (finer than toolsets) and persists to `config.yaml`. Disabled tools are filtered out even if their toolset is enabled.
