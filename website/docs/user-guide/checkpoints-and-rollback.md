...
Seple T Agent can automatically snapshot your project before **destructive operations** and restore it with a single command. Checkpoints are **opt-in** as of v2 — most users never use `/rollback`, and the shadow-store storage is non-trivial over time, so the default is off.

Enable checkpoints per-session with `--checkpoints`:

```bash
Seple T Agent chat --checkpoints
```

Or enable globally in `~/.Seple T Agent/config.yaml`:
...
| Command | Description |
|---------|-------------|
| `Seple T Agent checkpoints` | Show total size, project count, per-project breakdown |
| `Seple T Agent checkpoints status` | Same as bare `checkpoints` |
| `Seple T Agent checkpoints list` | Alias for `status` |
| `Seple T Agent checkpoints prune` | Force a sweep: delete orphans/stale, GC, enforce size cap |
| `Seple T Agent checkpoints clear` | Nuke the entire checkpoint base (asks first) |
| `Seple T Agent checkpoints clear-legacy` | Delete only the `legacy-*` archives from v1 migration |

## How Checkpoints Work

At a high level:

- Seple T Agent detects when tools are about to **modify files** in your working tree.
- Once per conversation turn (per directory), it:
  - Resolves a reasonable project root for the file.
  - Initialises or reuses the **single shared shadow store** at `~/.Seple T Agent/checkpoints/store/`.
  - Stages into a per-project index, builds a tree, and commits to a per-project ref (`refs/Seple T Agent/<project-hash>`).
...
flowchart LR
  ...
  store["Shared shadow store\n~/.Seple T Agent/checkpoints/store/"]
...
checkpoints:
  ...
  # Auto-maintenance (on by default): sweep ~/.Seple T Agent/checkpoints/ at startup
...
When `enabled: false`, the Checkpoint Manager is a no-op and never attempts git operations. When `auto_prune: false`, the store grows until you run `Seple T Agent checkpoints prune` manually.
...
Seple T Agent responds with a formatted list showing change statistics:
...
```text
Checkpoint base: /home/you/.Seple T Agent/checkpoints
Total size:      142.3 MB
  store/         138.1 MB
  legacy-*       4.2 MB
Projects:        12
...
Clear with: Seple T Agent checkpoints clear-legacy
```

Force a full sweep (ignores the 24h idempotency marker):

```bash
Seple T Agent checkpoints prune --retention-days 3 --max-size-mb 200
```
...
Behind the scenes, Seple T Agent:
...
- **Directory scope** — Seple T Agent skips overly broad directories (root `/`, home `$HOME`).
...
~/.Seple T Agent/checkpoints/
  ...
  │   ├── refs/Seple T Agent/<hash> # per-project branch tip
...
Each `<hash>` is derived from the absolute path of the working directory. You normally never need to touch these manually — use `Seple T Agent checkpoints status` / `prune` / `clear` instead.

### Migration from v1

Before the v2 rewrite, each working directory got its own complete shadow git repo directly under `~/.Seple T Agent/checkpoints/<hash>/`. That layout couldn't dedup objects across projects and had a documented no-op pruner — the store would grow without bound.

On first v2 run, any pre-v2 shadow repos are moved into `~/.Seple T Agent/checkpoints/legacy-<timestamp>/` so the new single-store layout starts clean. Old `/rollback` history is still reachable by manually inspecting the legacy archive with `git`; once you're confident you don't need it, run:

```bash
Seple T Agent checkpoints clear-legacy
```
...
- **Enable checkpoints only when you need them** — `Seple T Agent chat --checkpoints` or per-profile `enabled: true`.
...
- **Check `Seple T Agent checkpoints status` occasionally** if you use checkpoints regularly — shows which projects are active and what the store costs you.
- **Combine with Git worktrees** for maximum safety — keep each Seple T Agent session in its own worktree/branch, with checkpoints as an extra layer.
