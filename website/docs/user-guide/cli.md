...
sidebar_position: 1
title: "CLI Interface"
description: "Master the Seple T Agent terminal interface — commands, keybindings, personalities, and more"
---

# CLI Interface

Seple T Agent's CLI is a full terminal user interface (TUI) — not a web UI. It features multiline editing, slash-command autocomplete, conversation history, interrupt-and-redirect, and streaming tool output. Built for people who live in the terminal.

:::tip First-time setup
One command — `Seple T Agent setup --portal` — and you're ready to `Seple T Agent chat`. See [Nous Portal](/integrations/nous-portal).
:::

:::tip
Seple T Agent also ships a modern TUI with modal overlays, mouse selection, and non-blocking input. Launch it with `Seple T Agent --tui` — see the [TUI](tui.md) guide.
:::
...
Seple T Agent chat --model "anthropic/claude-sonnet-4"

# With a specific provider
Seple T Agent chat --provider nous        # Use Nous Portal
Seple T Agent chat --provider openrouter  # Force OpenRouter

# With specific toolsets
Seple T Agent chat --toolsets "web,terminal,skills"

# Start with one or more skills preloaded
Seple T Agent -s hermes-agent-dev,github-auth
Seple T Agent chat -s github-pr-workflow -q "open a draft PR"

# Resume previous sessions
Seple T Agent --continue             # Resume the most recent CLI session (-c)
Seple T Agent --resume <session_id>  # Resume a specific session by ID (-r)

# Verbose mode (debug output)
Seple T Agent chat --verbose

# Isolated git worktree (for running multiple agents in parallel)
Seple T Agent -w                         # Interactive mode in worktree
Seple T Agent -w -z "Fix issue #123"     # Single query in worktree
```

## Interface Layout

<img className="docs-terminal-figure" src="/docs/img/docs/cli-layout.svg" alt="Stylized preview of the Seple T Agent CLI layout showing the banner, conversation area, and fixed input prompt." />
<p className="docs-figure-caption">The Seple T Agent CLI banner, conversation stream, and fixed input prompt rendered as a stable docs figure instead of fragile text art.</p>
...
| ⚠ YOLO | **YOLO mode warning** — shown whenever `HERMES_YOLO_MODE` is on (either `Seple T Agent --yolo` at launch or `/yolo` toggled mid-session). Mirrors the banner-line warning so you can't forget you're in auto-approve mode. |
...
| `Ctrl+D` | Exit |
| `Ctrl+Z` | Suspend Seple T Agent to background (Unix only). Run `fg` in the shell to resume. |
...
Type `/` to see the autocomplete dropdown. Seple T Agent supports a large set of CLI slash commands, dynamic skill commands, and user-defined quick commands.
...
| `/voice tts` | Toggle spoken playback for Seple T Agent replies |
...
```bash
Seple T Agent -s hermes-agent-dev,github-auth
Seple T Agent chat -s github-pr-workflow -s github-auth
```

Seple T Agent loads each named skill into the session prompt before the first turn. The same flag works in interactive mode and single-query mode.

## Skill Slash Commands

Every installed skill in `~/.Seple T Agent/skills/` is automatically registered as a slash command. The skill name becomes the command:
...
You can also define custom personalities in `~/.Seple T Agent/config.yaml`:

```yaml
personalities:
  helpful: "You are a helpful, friendly AI assistant."
  kawaii: "You are a kawaii assistant! Use cute expressions..."
  pirate: "Arrr! Ye be talkin' to Captain Seple T Agent..."
  # Add your own!
```
...
Most terminals send the same byte sequence for `Enter` and `Shift+Enter` by default, so applications cannot distinguish them. Seple T Agent recognises `Shift+Enter` only when the terminal sends a distinct sequence via the [Kitty keyboard protocol](https://sw.kovidgoyal.net/kitty/keyboard-protocol/) or xterm's `modifyOtherKeys` mode.
...
Where the terminal cannot distinguish them, `Alt+Enter` and `Ctrl+J` continue to work everywhere. **On Windows Terminal specifically, `Alt+Enter` is captured by the terminal (toggles fullscreen) and never reaches Seple T Agent — use `Ctrl+Enter` (delivered as `Ctrl+J`) or `Ctrl+J` directly for a newline.**
...
The very first time you press Enter while Seple T Agent is working, Seple T Agent prints a one-line reminder explaining the `/busy` knob (`"(tip) Your message interrupted the current run…"`). It only fires once per install — a flag in `config.yaml` under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
:::

### Suspending to Background

On Unix systems, press **`Ctrl+Z`** to suspend Seple T Agent to the background — just like any terminal process. The shell prints a confirmation:

```
Seple T Agent has been suspended. Run `fg` to bring Seple T Agent back.
```
...
Resume this session with:
  Seple T Agent --resume 20260225_143052_a1b2c3
...
```bash
Seple T Agent --continue                          # Resume the most recent CLI session
Seple T Agent -c                                  # Short form
Seple T Agent -c "my project"                     # Resume a named session (latest in lineage)
Seple T Agent --resume 20260225_143052_a1b2c3     # Resume a specific session by ID
Seple T Agent --resume "refactoring auth"         # Resume by title
Seple T Agent -r 20260225_143052_a1b2c3           # Short form
```

Resuming restores the full conversation history from SQLite. The agent sees all previous messages, tool calls, and responses — just as if you never left.

Use `/title My Session Name` inside a chat to name the current session, or `Seple T Agent sessions rename <id> <title>` from the command line. Use `Seple T Agent sessions list` to browse past sessions.

### Session Storage

CLI sessions are stored in Seple T Agent's SQLite state database under `~/.Seple T Agent/state.db`. The database keeps:
...
Seple T Agent immediately confirms the task and gives you back the prompt:
