...
description: "Plugins shipped with Seple T Agent that run automatically via lifecycle hooks — disk-cleanup and friends"
---

# Built-in Plugins

Seple T Agent ships a small set of plugins bundled with the repository. They live under `<repo>/plugins/<name>/` and load automatically alongside user-installed plugins in `~/.Seple T Agent/plugins/`. They use the same plugin surface as third-party plugins — hooks, tools, slash commands — just maintained in-tree.

See the [Plugins](/user-guide/features/plugins) page for the general plugin system, and [Build a Seple T Agent Plugin](/developer-guide/plugins) to write your own.
...
2. **User** — `~/.Seple T Agent/plugins/<name>/`
3. **Project** — `./.Seple T Agent/plugins/<name>/` (requires `HERMES_ENABLE_PROJECT_PLUGINS=1`)
...
`plugins/memory/` and `plugins/context_engine/` are deliberately excluded from bundled scanning. Those directories use their own discovery paths because memory providers and context engines are single-select providers configured through `Seple T Agent memory setup` / `context.engine` in config.

## Bundled plugins are opt-in

Bundled plugins ship disabled. Discovery finds them (they appear in `Seple T Agent plugins list` and the interactive `Seple T Agent plugins` UI), but none load until you explicitly enable them:

```bash
Seple T Agent plugins enable disk-cleanup
```

Or via `~/.Seple T Agent/config.yaml`:
...
This is the same mechanism user-installed plugins use. Bundled plugins are never auto-enabled — not on fresh install, not for existing users upgrading to a newer Seple T Agent. You always opt in explicitly.

To turn a bundled plugin off again:

```bash
Seple T Agent plugins disable disk-cleanup
# or: remove it from plugins.enabled in config.yaml
```

## Currently shipped

The repo ships these bundled plugins under `plugins/`. All are opt-in — enable them via `Seple T Agent plugins enable <name>`.
...
| `hermes-achievements` | dashboard tab | Steam-style collectible badges generated from your real Seple T Agent session history |
...
Memory providers (`plugins/memory/*`) and context engines (`plugins/context_engine/*`) are listed separately on [Memory Providers](./memory-providers.md) — they're managed through `Seple T Agent memory` and `Seple T Agent plugins` respectively. The full per-plugin detail for the two long-running hooks-based plugins follows.
...
**Enabling:** `Seple T Agent plugins enable disk-cleanup` (or check the box in `Seple T Agent plugins`).

**Disabling again:** `Seple T Agent plugins disable disk-cleanup`.
...
**Enabling:** `Seple T Agent plugins enable security-guidance` (or check the box in `Seple T Agent plugins`).

**Disabling again:** `Seple T Agent plugins disable security-guidance`.
...
Traces Seple T Agent turns, LLM calls, and tool invocations to [Langfuse](https://langfuse.com) — an open-source LLM observability platform. One span per turn, one generation per API call, one tool observation per tool call. Usage totals, per-type token counts, and cost estimates come out of Seple T Agent' canonical `agent.usage_pricing` numbers, so the Langfuse dashboard sees the same breakdown (input / output / `cache_read_input_tokens` / `cache_creation_input_tokens` / `reasoning_tokens`) that appears in `Seple T Agent logs`.
...
```bash
Seple T Agent tools          # → Langfuse Observability → Cloud or Self-Hosted
```

The wizard collects your keys, `pip install`s the `langfuse` SDK, and adds `observability/langfuse` to `plugins.enabled` for you. Restart Seple T Agent and the next turn ships a trace.

**Setup (manual):**

```bash
pip install langfuse
Seple T Agent plugins enable observability/langfuse
```

Then put the credentials in `~/.Seple T Agent/.env`:
...
| Hook | Behaviour |
|---|---|
| `pre_api_request` / `pre_llm_call` | Open (or reuse) a per-turn root span "Seple T Agent turn". Start a `generation` child observation for this API call with serialized recent messages as input. |
...
Session grouping keys off the Seple T Agent session ID (or task ID for sub-agents) via `langfuse.propagate_attributes`, so everything in a single `Seple T Agent chat` session lives under one Langfuse session.

**Verify:**

```bash
Seple T Agent plugins list                 # observability/langfuse should show "enabled"
Seple T Agent chat -q "hello"              # check the Langfuse UI for a "Seple T Agent turn" trace
```
...
Seple T Agent-prefixed and standard SDK env vars (`LANGFUSE_PUBLIC_KEY`, `LANGFUSE_SECRET_KEY`, `LANGFUSE_BASE_URL`) are both accepted — Seple T Agent-prefixed wins when both are set.
...
**Disabling:** `Seple T Agent plugins disable observability/langfuse`. The plugin module is still discovered, but no module code runs until you re-enable.
...
- Post-meeting artifacts (transcript, speaker-attributed notes, action items) saved under `~/.Seple T Agent/cache/google_meet/<meeting_id>/`

**Setup:**

```bash
Seple T Agent plugins enable google_meet
...
**Disabling:** `Seple T Agent plugins disable google_meet`. Any cached transcripts and recordings stay in `~/.Seple T Agent/cache/google_meet/` until you remove them.

### hermes-achievements

Adds a **Steam-style achievements tab to the dashboard** — 60+ collectible, tiered badges generated from your real Seple T Agent session history. Tool-chain feats, debugging patterns, vibe-coding streaks, skill/memory usage, model/provider variety, lifestyle quirks (weekend and night sessions). Originally authored by [@PCinkusz](https://github.com/PCinkusz) as an external plugin; brought in-tree so it stays in lockstep with Seple T Agent feature changes.

**How it works:**

- Scans your entire `~/.Seple T Agent/state.db` session history on the dashboard backend
...
| Secret | Hidden until Seple T Agent detects the first related signal in your history |
...
| File | Contents |
|---|---|
| `state.json` | Unlock history: which badges you've earned and when. Stable across Seple T Agent updates. |
...
**Enabling:** Nothing to enable — `hermes-achievements` is a dashboard-only plugin (no lifecycle hooks, no model-visible tools). It auto-registers as a tab in `Seple T Agent dashboard` on first launch. The `plugins.enabled` config only gates lifecycle/tool plugins; dashboard plugins are discovered purely via their `dashboard/manifest.json`.

**Opting out:** Delete or rename `plugins/hermes-achievements/dashboard/manifest.json`, or override it with a user plugin of the same name in `~/.Seple T Agent/plugins/hermes-achievements/` that ships no dashboard. The plugin's state files under `$HERMES_HOME/plugins/hermes-achievements/` survive — reinstalling preserves your unlock history.

## Adding a bundled plugin

Bundled plugins are written exactly like any other Seple T Agent plugin — see [Build a Seple T Agent Plugin](/developer-guide/plugins). The only differences are:

- Directory lives at `<repo>/plugins/<name>/` instead of `~/.Seple T Agent/plugins/<name>/`
- Manifest source is reported as `bundled` in `Seple T Agent plugins list`
