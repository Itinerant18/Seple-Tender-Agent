...
Seple T Agent can drive your desktop — clicking, typing, scrolling,
...
```
Seple T Agent computer-use install
```

This fetches and runs the upstream cua-driver installer — `install.sh`
on macOS/Linux, `install.ps1` on Windows. Use `Seple T Agent computer-use
status` to verify the install.

**Option 2: enable the toolset interactively.**

1. Run `Seple T Agent tools`, pick `🖱️  Computer Use (macOS/Windows/Linux)`.
...
| Platform | Prereqs |
|---|---|
| **macOS** | System Settings → Privacy & Security → **Accessibility** + **Screen Recording** → allow your terminal (or Seple T Agent app). `Seple T Agent computer-use doctor` will tell you which permission is missing. |
...
```
Seple T Agent -t computer_use chat
```

or add `computer_use` to your enabled toolsets in `~/.Seple T Agent/config.yaml`.

## `Seple T Agent computer-use doctor` — your first triage stop

`Seple T Agent computer-use doctor` runs cua-driver's structured
`health_report` MCP tool and prints a per-check matrix. It's the single
fastest way to find out *why* an action isn't working.

```
$ Seple T Agent computer-use doctor
...
agent is acting here." Each Seple T Agent run declares its own cua-driver
...
Seple T Agent intentionally keeps its skill (`skills/computer-use/SKILL.md`)
focused on the Seple T Agent-side `computer_use` action vocabulary — the
...
These are **platform deep dives, not duplicates of the Seple T Agent skill** —
...
Code, Codex, OpenCode, OpenClaw, and Antigravity; **Seple T Agent
...
Seple T Agent applies multi-layer guardrails:
...
Pair with `approvals.mode: manual` in `~/.Seple T Agent/config.yaml` if you
want every action confirmed.

## Token efficiency

Screenshots are expensive. Seple T Agent applies four layers of optimisation:
...
- **Windows: elevated (admin) windows can't be driven from a normal
  ...
  Seple T Agent agent) cannot enumerate the UIA tree of, or inject mouse input
  ...
  run the Seple T Agent agent itself at High integrity (launch from an
...
- **Platform-specific deployment gotchas:**
  - **macOS** uses private SkyLight SPIs. Apple can change them in any
    OS update. Seple T Agent warns when the installed cua-driver is older than
    the version it was tested against.
  - **Windows** SSH sessions run in **Session 0**, which has no
    interactive desktop. Drive Seple T Agent from inside the RDP / console
...
upstream. **Seple T Agent disables it for you** — on every cua-driver invocation
(the MCP backend, `status`, `doctor`, and install) Seple T Agent sets
...
When it's on, `Seple T Agent computer-use doctor` reports `telemetry: enabled`;
...
unreleased fix — point Seple T Agent at a binary you built from source instead
of the published release. Seple T Agent resolves the driver with
...
- `-NoAutoStart` skips registering the `cua-driver-serve` logon daemon
  — you don't need it for Seple T Agent testing (see notes).
...
### Option B — point Seple T Agent straight at the built binary (fastest loop)
...
### Confirm Seple T Agent is using your build

- `Seple T Agent computer-use status` prints the resolved binary path and
  version.
- `Seple T Agent computer-use doctor` confirms the binary is reachable and
...
- **Seple T Agent spawns its own `cua-driver mcp` child over stdio** — it does
...
- **Rebuild loop.** After editing cua-driver source, re-run
  ...
  for Option A, or just re-`cargo build` for Option B — no Seple T Agent
  change needed either way.
- **Local builds skip the version check.** Seple T Agent warns when the
...
**First action when anything's off: run `Seple T Agent computer-use doctor`.**
...
Run `Seple T Agent computer-use install` to fetch the cua-driver binary, or
run `Seple T Agent tools` and enable the Computer Use toolset.
...
Wayland without an XWayland bridge. `Seple T Agent computer-use doctor` will
...
- **Seple T Agent-side skill** — `skills/computer-use/SKILL.md` — teaches the
  Seple T Agent `computer_use` action vocabulary; this is what the agent loads.
- **cua-driver skill pack** — for platform-specific deep dives
  ...
  install` autodetects Seple T Agent (planned follow-up), this happens
...
- **cua.ai/docs** — the cua-driver project's documentation:
  ...
  - [Connect your agent](https://cua.ai/docs/how-to-guides/driver/connect-your-agent) — register cua-driver with various harnesses (Seple T Agent among them)
