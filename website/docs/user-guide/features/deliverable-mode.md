...
When Seple T Agent runs inside a messaging gateway (Slack, Discord, Telegram,
...
persona in `~/.Seple T Agent/SOUL.md`, or as a named preset under
`agent.personalities` in `~/.Seple T Agent/config.yaml` (switchable per session
via `/personality`).
...
If you use Seple T Agent' kanban multi-agent workflow, workers can attach
...
Install MCP servers via `~/.Seple T Agent/config.yaml` under the `mcp_servers`
...
back into the thread as a native attachment. Seple T Agent's deliverable
