...
[Honcho](https://github.com/plastic-labs/honcho) is an AI-native memory backend that adds dialectic reasoning and deep user modeling on top of Seple T Agent's built-in memory system. Instead of simple key-value storage, Honcho maintains a running model of who the user is — their preferences, communication style, goals, and patterns — by reasoning about conversations after they happen.
...
**Multi-agent profiles**: When multiple Seple T Agent instances talk to the same user (e.g., a coding assistant and a personal assistant), Honcho maintains separate "peer" profiles. Each peer sees only its own observations and conclusions, preventing cross-contamination of context.

## Setup

```bash
Seple T Agent memory setup    # select "honcho" from the provider list
```
...
```bash
echo 'HONCHO_API_KEY=***' >> ~/.Seple T Agent/.env
```
...
- `per-session` — each `Seple T Agent` run gets a fresh session. Clean starts, memory via tools. Recommended for new users.
...
Server-side toggles set via the [Honcho dashboard](https://app.honcho.dev) win over local defaults — Seple T Agent syncs them back at session init.
...
The `Seple T Agent honcho` subcommand is **only registered when Honcho is the active memory provider** (`memory.provider: honcho` in `config.yaml`). On a fresh install, configure Honcho directly with `Seple T Agent memory setup honcho` (or run `Seple T Agent memory setup` and pick it from the list); the `Seple T Agent honcho` subcommand then appears on the next invocation.

```bash
Seple T Agent memory setup honcho    # Configure Honcho directly (works before activation)
Seple T Agent honcho status          # Connection status, config, and key settings
Seple T Agent honcho setup           # Redirects to `Seple T Agent memory setup` (post-activation alias)
Seple T Agent honcho strategy        # Show or set session strategy (per-session/per-directory/per-repo/global)
Seple T Agent honcho peer            # Show or update peer names + dialectic reasoning level
Seple T Agent honcho mode            # Show or set recall mode (hybrid/context/tools)
Seple T Agent honcho tokens          # Show or set token budget for context and dialectic
Seple T Agent honcho identity        # Seed or show the AI peer's Honcho identity
Seple T Agent honcho sync            # Sync Honcho config to all existing profiles
Seple T Agent honcho peers           # Show peer identities across all profiles
Seple T Agent honcho sessions        # List known Honcho session mappings
Seple T Agent honcho map             # Map current directory to a Honcho session name
Seple T Agent honcho enable          # Enable Honcho for the active profile
Seple T Agent honcho disable         # Disable Honcho for the active profile
Seple T Agent honcho migrate         # Step-by-step migration guide from openclaw-honcho
```

## Migrating from `Seple T Agent honcho`

If you previously used the standalone `Seple T Agent honcho setup`:
...
No re-login or re-setup needed. Run `Seple T Agent memory setup` and select "honcho" — the wizard detects your existing config.
