...
Seple T Agent runs full language servers — pyright, gopls, rust-analyzer,
...
This is the same architecture top-tier coding agents use. Seple T Agent
...
1. Seple T Agent captures a baseline of current diagnostics for the file.
...
…). Seple T Agent auto-detects the binary on PATH or in
`<HERMES_HOME>/lsp/bin/`.
...
2. Download the latest release zip from
   ...
   and extract it.
3. Point Seple T Agent at the extracted bundle — the directory that contains
...
`Seple T Agent lsp status` reports `installed` once `pwsh` is found; if the
...
`node_modules` tree — Seple T Agent installs both packages together when you
run `Seple T Agent lsp install typescript` or auto-install fires on first
use.

## CLI

```
Seple T Agent lsp status          # service state + per-server install status
Seple T Agent lsp list            # registry, optionally --installed-only
Seple T Agent lsp install <id>    # eagerly install one server
Seple T Agent lsp install-all     # try every server with a known recipe
Seple T Agent lsp restart         # tear down running clients
Seple T Agent lsp which <id>      # print resolved binary path
```

`Seple T Agent lsp status` is the best starting point — it shows which
...
When `install_strategy: auto`, Seple T Agent installs binaries into
...
shared location — the staging dir is fully Seple T Agent-owned and is
...
Servers are kept alive for the life of the Seple T Agent process. There's
...
**`Seple T Agent lsp status` shows a server as "missing"**

The binary isn't on PATH and isn't in `<HERMES_HOME>/lsp/bin/`. Run
`Seple T Agent lsp install <server_id>` to attempt an auto-install, or
install the binary manually through the language's normal toolchain.

**`Backend warnings` section in `Seple T Agent lsp status`**
...
When `Seple T Agent lsp status` shows a `Backend warnings` section, install
...
`~/.Seple T Agent/logs/agent.log`.

**Server starts but never returns diagnostics**

Check `~/.Seple T Agent/logs/agent.log` for `[agent.lsp.client]` entries —
...
the rest of the session. Run `Seple T Agent lsp restart` to clear the set;
