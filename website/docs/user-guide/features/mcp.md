...
description: "Connect Seple T Agent to external tool servers via MCP — and control exactly which MCP tools Seple T Agent loads"
---

# MCP (Model Context Protocol)

MCP lets Seple T Agent connect to external tool servers so the agent can use tools that live outside Seple T Agent itself — GitHub, databases, file systems, browser stacks, internal APIs, and more.

If you have ever wanted Seple T Agent to use a tool that already exists somewhere else, MCP is usually the cleanest way to do it.

## What MCP gives you

- Access to external tool ecosystems without writing a native Seple T Agent tool first
- Local stdio servers and remote HTTP MCP servers in the same config
- Automatic tool discovery and registration at startup
- Utility wrappers for MCP resources and prompts when supported by the server
- Per-server filtering so you can expose only the MCP tools you actually want Seple T Agent to see

## Quick start

1. MCP support ships with the standard install — no extra step needed.

2. Add an MCP server to `~/.Seple T Agent/config.yaml`:
...
3. Start Seple T Agent:

```bash
Seple T Agent chat
```

4. Ask Seple T Agent to use the MCP-backed capability.
...
Seple T Agent will discover the MCP server's tools and use them like any other tool.

## Catalog: one-click install for Nous-approved MCPs

Seple T Agent ships a curated catalog of MCP servers that Nous staff has reviewed
and merged. They're disabled by default — install only what you actually
want.

```bash
Seple T Agent mcp                # interactive picker (default)
Seple T Agent mcp catalog        # plain-text list, scriptable
Seple T Agent mcp install n8n    # install a catalog entry by name
```

The picker shows each entry with its current status:

```
n8n          available              Manage and inspect n8n workflows from Seple T Agent
...
- **API key** — Seple T Agent prompts at install time and writes the value to
  `~/.Seple T Agent/.env`. Non-secret values (base URLs) go to the same file.
- **OAuth** (remote MCP) — written as `auth: oauth` in your config; the MCP
  client opens a browser on first connection.
- **OAuth** (third-party provider like Google/GitHub) — Seple T Agent points you at
  `Seple T Agent auth <provider>` if you haven't authenticated already.

### Tool selection at install time

After credentials are configured, Seple T Agent probes the MCP server to list every
...
written (if not). Re-run `Seple T Agent mcp configure <name>` once the server is
reachable to refine.
...
Manifests live at
[`optional-mcps/<name>/manifest.yaml`](https://github.com/Novaedge/hermes-agent/tree/main/optional-mcps)
...
PR adds an entry with a newer `manifest_version` than your installed Seple T Agent
understands, the picker will surface a warning (`⚠ '<name>' requires a newer
Seple T Agent`) for that entry instead of silently hiding it. Run `Seple T Agent update`
to install the latest Seple T Agent when you see that.
...
from environment variables (which include everything in `~/.Seple T Agent/.env`).
...
```bash
Seple T Agent mcp configure linear
```
...
MCPs are never auto-updated. Re-run `Seple T Agent mcp install <name>` to refresh
after a Seple T Agent update if a manifest version changed.

To add an MCP to the catalog, open a PR against
[`optional-mcps/`](https://github.com/Novaedge/hermes-agent/tree/main/optional-mcps).
...
HTTP MCP servers are remote endpoints Seple T Agent connects to directly.
...
- you do not want Seple T Agent spawning a local subprocess for that integration

### OAuth-authenticated HTTP servers

Most hosted MCP servers (Linear, Sentry, Atlassian, Asana, Figma, Stripe, …) require OAuth 2.1 instead of a static bearer token. Set `auth: oauth` and Seple T Agent handles discovery, dynamic client registration, PKCE, token exchange, refresh, and step-up auth via the MCP Python SDK.
...
On first connect, Seple T Agent prints an authorize URL, opens your browser when possible, and waits for the OAuth callback on a local loopback port. Tokens are cached at `~/.Seple T Agent/mcp-tokens/<server>.json` with 0o600 perms; subsequent runs reuse them silently until refresh fails.

**Remote / headless hosts.** When Seple T Agent runs on a different machine than your browser, the loopback callback can't reach your laptop. Two ways to complete the flow:

- **Paste-back (no setup):** on an interactive terminal Seple T Agent prints "Or paste the redirect URL here…" alongside the authorize URL. Open the URL in your browser, approve, copy the full URL the browser ends up on (the redirect will show a connection error — that's expected), paste it at the prompt. Bare `?code=…&state=…` query strings work too.
- **SSH port forward:** `ssh -N -L <port>:127.0.0.1:<port> user@host` in a separate terminal, then let the redirect flow normally.

See [OAuth over SSH / Remote Hosts](../../guides/oauth-over-ssh.md#mcp-servers) for the full walkthrough, including DCR-less servers (e.g. Slack), pre-registered `client_id`/`client_secret`, scope customization, and re-auth via `Seple T Agent mcp login <server>`.

**Pitfall — providers that don't support automatic registration (Google Drive, Atlassian).** Some servers reject the dynamic client registration step (RFC 7591) that bare `auth: oauth` relies on — Google's official Drive server (`https://drivemcp.googleapis.com/mcp/v1`) returns a `400 Bad Request`, so no OAuth client is created and no token is acquired. The symptom is subtle: these servers also serve `tools/list` *without* auth, so `Seple T Agent mcp login` can list the tools and look like it worked, but every real tool call later times out. `Seple T Agent mcp login` now detects this (it checks that a token actually landed on disk) and tells you to supply your own OAuth client. Create one in the provider's console and add it to config:
...
Then run `Seple T Agent mcp login googledrive` — with the pre-registered client, Seple T Agent skips registration and runs the normal browser authorization flow.

**Pitfall — config auto-reload race.** When you edit `~/.Seple T Agent/config.yaml` from inside a running Seple T Agent session, the CLI auto-reloads MCP connections with a 30s timeout. That's not enough for an interactive OAuth flow. Add the entry, then run `Seple T Agent mcp login <server>` from a fresh terminal — it waits the full 5 minutes for you to complete auth.

## mTLS / client certificates

Remote HTTP MCP servers that require mutual TLS (client-certificate authentication) are supported via `client_cert` / `client_key`. Seple T Agent passes the resolved certificate to the underlying HTTP client for the TLS handshake.
...
Seple T Agent reads MCP config from `~/.Seple T Agent/config.yaml` under `mcp_servers`.
...
| `enabled` | bool | If `false`, Seple T Agent skips the server entirely |
...
For well-known MCP servers, `Seple T Agent mcp add` accepts a `--preset` flag that fills in the transport details so you don't have to look up the command and args. The preset only supplies defaults — anything else (env vars, headers, filtering) you pass on the same command line still wins.
...
Seple T Agent mcp add codex --preset codex
```
...
You can pick any local name (`Seple T Agent mcp add my-codex --preset codex` is fine); the preset only provides the `command`/`args` defaults.

## How Seple T Agent registers MCP tools

Seple T Agent prefixes MCP tools so they do not collide with built-in names:
...
In practice, you usually do not need to call the prefixed name manually — Seple T Agent sees the tool and chooses it during normal reasoning.

## MCP utility tools

When supported, Seple T Agent also registers utility tools around MCP resources and prompts:
...
- Seple T Agent only registers resource utilities if the MCP session actually supports resource operations
- Seple T Agent only registers prompt utilities if the MCP session actually supports prompt operations

So a server that exposes callable tools but no resources/prompts will not get those extra wrappers.

## Per-server filtering

You can control which tools each MCP server contributes to Seple T Agent, allowing fine-grained management of your tool namespace.
...
If `enabled: false`, Seple T Agent skips the server completely and does not even attempt a connection.
...
You can also separately disable Seple T Agent-added utility wrappers:
...
If your config filters out all callable tools and disables or omits all supported utilities, Seple T Agent does not create an empty runtime MCP toolset for that server.
...
Seple T Agent discovers MCP servers at startup and registers their tools into the normal tool registry.

### Dynamic Tool Discovery

MCP servers can notify Seple T Agent when their available tools change at runtime by sending a `notifications/tools/list_changed` notification. When Seple T Agent receives this notification, it automatically re-fetches the server's tool list and updates the registry — no manual `/reload-mcp` required.
...
For stdio servers, Seple T Agent does not blindly pass your full shell environment.
...
Then verify your config and restart Seple T Agent.
...
Because Seple T Agent now only registers those wrappers when both are true:
...
When `supports_parallel_tool_calls` is `true`, Seple T Agent may execute multiple tools from that server at the same time within a single tool-call batch, just like it does for built-in read-only tools (web_search, read_file, etc.).
...
MCP servers can request LLM inference from Seple T Agent via the `sampling/createMessage` protocol. This allows an MCP server to ask Seple T Agent to generate text on its behalf — useful for servers that need LLM capabilities but don't have their own model access.
...
## Running Seple T Agent as an MCP server

In addition to connecting **to** MCP servers, Seple T Agent can also **be** an MCP server. This lets other MCP-capable agents (Claude Code, Cursor, Codex, or any MCP client) use Seple T Agent's messaging capabilities — list conversations, read message history, and send messages across all your connected platforms.

### When to use this

- You want Claude Code, Cursor, or another coding agent to send and read Telegram/Discord/Slack messages through Seple T Agent
- You want a single MCP server that bridges to all of Seple T Agent's connected messaging platforms at once
- You already have a running Seple T Agent gateway with connected platforms

### Quick start

```bash
Seple T Agent mcp serve
```
...
Add Seple T Agent to your MCP client config. For example, in Claude Code's `~/.claude/claude_desktop_config.json`:

```json
{
  "mcpServers": {
    "Seple T Agent": {
      "command": "Seple T Agent",
      "args": ["mcp", "serve"]
    }
  }
}
...
Or if you installed Seple T Agent in a specific location:

```json
{
  "mcpServers": {
    "Seple T Agent": {
      "command": "/home/user/.Seple T Agent/hermes-agent/venv/bin/Seple T Agent",
      "args": ["mcp", "serve"]
    }
  }
}
...
The MCP server exposes 10 tools, matching OpenClaw's channel bridge surface plus a Seple T Agent-specific channel browser:
...
The MCP server includes a live event bridge that polls Seple T Agent's session database for new messages. This gives MCP clients near-real-time awareness of incoming conversations:
...
```bash
Seple T Agent mcp serve              # Normal mode
Seple T Agent mcp serve --verbose    # Debug logging on stderr
```

### How it works

The MCP server reads conversation data directly from Seple T Agent's session store (`~/.Seple T Agent/sessions/sessions.json` and the SQLite database). A background thread polls the database for new messages and maintains an in-memory event queue. For sending messages, it uses the same internal send engine (`tools/send_message_tool.py`) that powers cron delivery and the `Seple T Agent send` CLI.
...
- The embedded `Seple T Agent mcp serve` exposes a **stdio-only** MCP server today. If you need an HTTP MCP server, run a separate adapter — or, much more commonly, use the MCP **client** side of Seple T Agent, which already speaks both stdio and HTTP (`url` + `headers` in `mcp_servers.yaml` / `config.yaml`; see [HTTP servers](#http-servers) above).
