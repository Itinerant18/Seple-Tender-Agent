...
Seple T Agent ships with 8 external memory provider plugins that give the agent persistent, cross-session knowledge beyond the built-in MEMORY.md and USER.md. Only **one** external provider can be active at a time — the built-in memory is always active alongside it.

## Quick Start

```bash
Seple T Agent memory setup      # interactive picker + configuration
Seple T Agent memory status     # check what's active
Seple T Agent memory off        # disable external provider
```

You can also select the active memory provider via `Seple T Agent plugins` → Provider Plugins → Memory Provider.

Or set manually in `~/.Seple T Agent/config.yaml`:
...
When a memory provider is active, Seple T Agent automatically:
...
**Setup Wizard:**
```bash
Seple T Agent memory setup        # select "honcho" — runs the Honcho-specific post-setup
```

The legacy `Seple T Agent honcho setup` command still works (it now redirects to `Seple T Agent memory setup`), but is only registered after Honcho is selected as the active memory provider.

**Config:** `$HERMES_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global). Resolution order: `$HERMES_HOME/honcho.json` > `~/.Seple T Agent/honcho.json` > `~/.honcho/config.json`. See the [config reference](https://github.com/Novaedge/hermes-agent/blob/main/plugins/memory/honcho/README.md) and the [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/Seple T Agent).
...
```json
{
  ...
  "hosts": {
    "Seple T Agent": {
      "enabled": true,
      "aiPeer": "Seple T Agent",
      "peerName": "your-name",
      "workspace": "Seple T Agent"
    }
  }
}
```
...
:::tip Migrating from `Seple T Agent honcho`
If you previously used `Seple T Agent honcho setup`, your config and all server-side data are intact. Just re-enable through the setup wizard again or manually set `memory.provider: honcho` to reactivate via the new system.
:::

**Multi-peer setup:**

Honcho models conversations as peers exchanging messages — one user peer plus one AI peer per Seple T Agent profile, all sharing a workspace. The workspace is the shared environment: the user peer is global across profiles, each AI peer is its own identity. Every AI peer builds an independent representation / card from its own observations, so a `coder` profile stays code-oriented while a `writer` profile stays editorial against the same user.

The mapping:

| Concept | What it is |
|---------|-----------|
| **Workspace** | Shared environment. All Seple T Agent profiles under one workspace see the same user identity. |
| **User peer** (`peerName`) | The human. Shared across profiles in the workspace. |
| **AI peer** (`aiPeer`) | One per Seple T Agent profile. Host key `Seple T Agent` → default; `Seple T Agent.<profile>` for others. |
...
```bash
Seple T Agent profile create coder --clone
```

`--clone` creates a `Seple T Agent.coder` host block in `honcho.json` with `aiPeer: "coder"`, shared `workspace`, inherited `peerName`, `recallMode`, `writeFrequency`, `observation`, etc. The AI peer is eagerly created in Honcho so it exists before the first message.

### Existing profiles, backfill Honcho peers

```bash
Seple T Agent honcho sync
```

Scans every Seple T Agent profile, creates host blocks for any profile without one, inherits settings from the default `Seple T Agent` block, and creates the new AI peers eagerly. Idempotent — skips profiles that already have a host block.
...
Off-gateway these keys do nothing. `Seple T Agent memory setup` only prompts for them when it detects a connected gateway platform. See the [Honcho page](./honcho.md#gateway-identity-mapping) for the resolver ladder and the setup flow.
...
See the [config reference](https://github.com/Novaedge/hermes-agent/blob/main/plugins/memory/honcho/README.md) and [Honcho integration guide](https://docs.honcho.dev/v3/guides/integrations/Seple T Agent).
...
Seple T Agent memory setup    # select "openviking"
# Or manually:
Seple T Agent config set memory.provider openviking
echo "OPENVIKING_ENDPOINT=http://localhost:1933" >> ~/.Seple T Agent/.env
# Authenticated servers should use a user/admin API key:
echo "OPENVIKING_API_KEY=..." >> ~/.Seple T Agent/.env
```
...
`OPENVIKING_AGENT` is Seple T Agent' peer ID in OpenViking for peer-scoped memories.
...
**Setup (Platform):**
```bash
Seple T Agent memory setup    # select "mem0" → "Platform"
# Or manually:
Seple T Agent config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.Seple T Agent/.env
```

**Setup (OSS):**
```bash
Seple T Agent memory setup    # select "mem0" → "Open Source (self-hosted)"
# Or via flags:
Seple T Agent memory setup mem0 --mode oss --oss-llm openai --oss-llm-key sk-... --oss-vector qdrant
```

Preview without writing files:
```bash
Seple T Agent memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```

**Setup (Self-Hosted Dashboard):** connect to a Mem0 server you run via Docker (the dashboard's REST API):

```bash
Seple T Agent memory setup    # select "mem0" → "Self-hosted server"
# Or via flags:
Seple T Agent memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
```

Or configure manually — either as env vars:

```bash
echo "MEM0_HOST=http://localhost:8888" >> ~/.Seple T Agent/.env
echo "MEM0_API_KEY=your-admin-api-key" >> ~/.Seple T Agent/.env
```
...
**Config:** `$HERMES_HOME/mem0.json` (behavioral settings). Only the secret `MEM0_API_KEY` belongs in `~/.Seple T Agent/.env`.
...
| `agent_id` | `Seple T Agent` | Agent identifier |
...
**Switching modes:** Re-run `Seple T Agent memory setup mem0 --mode <platform|selfhosted|oss>` or edit `mem0.json` directly.
...
**Setup:**
```bash
Seple T Agent memory setup    # select "hindsight"
# Or manually:
Seple T Agent config set memory.provider hindsight
echo "HINDSIGHT_API_KEY=your-key" >> ~/.Seple T Agent/.env
```

The setup wizard installs dependencies automatically and only installs what's needed for the selected mode (`hindsight-client` for cloud, `hindsight-all` for local). Requires `hindsight-client >= 0.4.22` (auto-upgraded on session start if outdated).

**Local mode UI:** `hindsight-embed -p Seple T Agent ui start`
...
| `bank_id` | `Seple T Agent` | Memory bank identifier |
| `recall_budget` | `mid` | Recall thoroughness: `low` / `mid` / `high` |
| `memory_mode` | `hybrid` | `hybrid` (context + tools), `context` (auto-inject only), `tools` (tools only) |
| `auto_retain` | `true` | Automatically retain conversation turns |
| `auto_recall` | `true` | Automatically recall memories before each turn |
| `retain_async` | `true` | Process retain asynchronously on the server |
| `retain_context` | `conversation between Seple T Agent and the User` | Context label for retained memories |
...
See [plugin README](https://github.com/Novaedge/hermes-agent/blob/main/plugins/memory/hindsight/README.md) for the full configuration reference.
...
**Setup:**
```bash
Seple T Agent memory setup    # select "holographic"
# Or manually:
Seple T Agent config set memory.provider holographic
```
...
**Setup:**
```bash
Seple T Agent memory setup    # select "retaindb"
# Or manually:
Seple T Agent config set memory.provider retaindb
echo "RETAINDB_API_KEY=your-key" >> ~/.Seple T Agent/.env
```
...
# Then configure Seple T Agent
Seple T Agent memory setup    # select "byterover"
# Or manually:
Seple T Agent config set memory.provider byterover
```
...
| **Requires** | `pip install supermemory` + [API key](http://app.supermemory.ai/integrations?connect=Seple T Agent) |
...
**Setup:**
```bash
Seple T Agent memory setup    # select "supermemory"
# Or manually:
Seple T Agent config set memory.provider supermemory
echo 'SUPERMEMORY_API_KEY=***' >> ~/.Seple T Agent/.env
```
...
| Key | Default | Description |
|-----|---------|-------------|
| `container_tag` | `Seple T Agent` | Container tag used for search and writes. Supports `{identity}` template for profile-scoped tags. |
...
- **Profile-scoped containers** — use `{identity}` in `container_tag` (e.g. `hermes-{identity}` → `hermes-coder`) to isolate memories per Seple T Agent profile
...
Seple T Agent config set memory.provider memori
Seple T Agent memory setup
```
