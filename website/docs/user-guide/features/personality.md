...
description: "Customize Seple T Agent's personality with a global SOUL.md, built-in personalities, and custom persona definitions"
---

# Personality & SOUL.md

Seple T Agent's personality is fully customizable. `SOUL.md` is the **primary identity** — it's the first thing in the system prompt and defines who the agent is.
...
If you want to change who Seple T Agent is — or replace it with an entirely different agent persona — edit `SOUL.md`.

## How SOUL.md works now

Seple T Agent now seeds a default `SOUL.md` automatically in:

```text
~/.Seple T Agent/SOUL.md
```

More precisely, it uses the current instance's `HERMES_HOME`, so if you run Seple T Agent with a custom home directory, it will use:
...
- Seple T Agent creates a starter `SOUL.md` automatically if one does not exist yet
- Existing user `SOUL.md` files are never overwritten
- Seple T Agent loads `SOUL.md` only from `HERMES_HOME`
- Seple T Agent does not look in the current working directory for `SOUL.md`
- If `SOUL.md` exists but is empty, or cannot be loaded, Seple T Agent falls back to a built-in default identity
...
If Seple T Agent loaded `SOUL.md` from whatever directory you happened to launch it in, your personality could change unexpectedly between projects. By loading only from `HERMES_HOME`, the personality belongs to the Seple T Agent instance itself.

That also makes it easier to teach users:
- "Edit `~/.Seple T Agent/SOUL.md` to change Seple T Agent' default personality."
...
- how Seple T Agent should handle uncertainty, disagreement, or ambiguity
...
## What Seple T Agent injects into the prompt
...
If the file is empty, whitespace-only, or cannot be read, Seple T Agent falls back to a built-in default identity ("You are Seple T Agent, an intelligent AI assistant created by Novaedge..."). This fallback also applies when `skip_context_files` is set (e.g., in subagent/delegation contexts).
...
Seple T Agent ships with built-in personalities you can switch to with `/personality`.
...
| **pirate** | Captain Seple T Agent, tech-savvy buccaneer |
...
These are convenient overlays, but your global `SOUL.md` still gives Seple T Agent its persistent default personality unless the overlay meaningfully changes it.

## Custom personalities in config

You can also define named custom personalities in `~/.Seple T Agent/config.yaml` under `agent.personalities`.
...
1. Keep a thoughtful global `SOUL.md` in `~/.Seple T Agent/SOUL.md`
...
- `SOUL.md`, `agent.system_prompt`, and `/personality` affect how Seple T Agent speaks
- `display.skin` and `/skin` affect how Seple T Agent looks in the terminal
