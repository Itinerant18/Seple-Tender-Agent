...
Seple T Agent can show an animated **pet** — a small mascot sprite that reacts to what
...
In the terminal (CLI/TUI), Seple T Agent renders the sprite at full fidelity when your
...
Seple T Agent pets list
Seple T Agent pets list cat

# Install a pet and make it active in one step
Seple T Agent pets install boba --select

# Preview / animate it in your terminal (Ctrl+C to stop)
Seple T Agent pets show

# Check your setup
Seple T Agent pets doctor
```

## `Seple T Agent pets` commands

| Goal | Command |
| --- | --- |
| Browse the gallery | `Seple T Agent pets list [query] [--limit N]` |
| List installed pets | `Seple T Agent pets list --installed` |
| Install a pet | `Seple T Agent pets install <slug> [--select] [--force]` |
...
| Preview/animate | `Seple T Agent pets show [slug] [--state <s>] [--cycle] [--once] [--mode <m>] [--scale <f>]` |
| Disable the pet | `Seple T Agent pets off` |
| Remove an installed pet | `Seple T Agent pets remove <slug>` |
| Diagnose setup | `Seple T Agent pets doctor` |

`Seple T Agent pets show` flags:
...
Beyond installing pre-made pets from the gallery, Seple T Agent can **generate a brand-new pet** from a text description — its own AI sprite-generation pipeline.

- CLI/TUI: `/hatch <description>` (alias `/generate-pet`), or `Seple T Agent pets` → the generate flow.
...
- If no reference-capable backend is configured, generation surfaces an actionable error pointing you to `Seple T Agent tools` → Image Generation. (Installing/adopting existing gallery pets needs no image backend.)
...
always-on-top desktop window. Out there it stays visible while Seple T Agent is
...
Run `Seple T Agent pets doctor` — it reports:
