...
All skills live in **`~/.Seple T Agent/skills/`** — the primary directory and source of truth. On fresh install, bundled skills are copied from the repo. Hub-installed and agent-created skills also go here. The agent can modify or delete any skill.

You can also point Seple T Agent at **external skill directories** — additional folders scanned alongside the local one. See [External Skill Directories](#external-skill-directories) below.
...
By default every profile is seeded with the bundled skill catalog, and each `Seple T Agent update` adds any newly bundled skills. If you want a profile with **no bundled skills** — and that stays empty across updates — you have two paths:

**At install time** (applies to the default `~/.Seple T Agent` profile):

```bash
curl -fsSL https://hermes-agent.Novaedge.com/install.sh | bash -s -- --no-skills
```

**At profile-create time** (named profiles):

```bash
Seple T Agent profile create research --no-skills
```

**On an already-installed profile** (default or named), toggle it at runtime:

```bash
Seple T Agent skills opt-out            # stop future seeding — nothing on disk is touched
Seple T Agent skills opt-out --remove   # also delete UNMODIFIED bundled skills (confirms first)
Seple T Agent skills opt-in --sync      # undo: remove the marker and re-seed now
```

All three paths write a `.no-bundled-skills` marker into the profile directory. While the marker is present, the installer, `Seple T Agent update`, and any skill sync all skip bundled-skill seeding for that profile. Delete the marker (or run `Seple T Agent skills opt-in`) to re-enable.

:::note Safe by default
`Seple T Agent skills opt-out` only stops *future* seeding — it never deletes anything already on disk. The optional `--remove` flag deletes bundled skills **only** when they are unmodified (byte-identical to the version Seple T Agent installed). Skills you have edited, skills installed from the hub, and skills you wrote yourself are always kept.
:::
...
The bundled `plan` skill is a good example. Running `/plan [request]` loads the skill's instructions, telling Seple T Agent to inspect context if needed, write a markdown implementation plan instead of executing the task, and save the result under `.Seple T Agent/plans/` relative to the active workspace/backend working directory.

You can also interact with skills through natural conversation:

```bash
Seple T Agent chat --toolsets skills -q "What skills do you have?"
Seple T Agent chat --toolsets skills -q "Show me the axolotl skill"
```
...
description, the standard section order, Seple T Agent-tool framing, no invented
commands).
...
/home/user/.Seple T Agent/cache/chart-q4-2025.png
...
When a missing value is encountered, Seple T Agent asks for it securely only when the skill is actually loaded in the local CLI. You can skip setup and keep using the skill. Messaging surfaces never ask for secrets in chat — they tell you to use `Seple T Agent setup` or `~/.Seple T Agent/.env` locally instead.
...
Settings are stored under `skills.config` in your config.yaml. `Seple T Agent config migrate` prompts for unconfigured settings, and `Seple T Agent config show` displays them. When a skill loads, its resolved config values are injected into the context so the agent knows the configured values automatically.
...
```text
~/.Seple T Agent/skills/                  # Single source of truth
...
and `examples/`. Unreferenced repository files are not copied. Seple T Agent scans the
...
If you maintain skills outside of Seple T Agent — for example, a shared `~/.agents/skills/` directory used by multiple AI tools — you can tell Seple T Agent to scan those directories too.

Add `external_dirs` under the `skills` section in `~/.Seple T Agent/config.yaml`:
...
- **Create locally, update in place**: New agent-created skills are written to `~/.Seple T Agent/skills/`. Existing skills are modified where they are found, including skills under `external_dirs`, when the agent uses `skill_manage` actions such as `patch`, `edit`, `write_file`, `remove_file`, or `delete`.
- **External dirs are not a write-protection boundary**: If an external skill directory is writable by the Seple T Agent process, agent-managed skill updates can change files in that directory. Use filesystem permissions or a separate profile/toolset setup if shared external skills must stay read-only.
- **Local precedence**: If the same skill name exists in both the local dir and an external dir, the local version wins.
- **Full integration**: External skills appear in the system prompt index, `skills_list`, `skill_view`, and as `/skill-name` slash commands — no different from local skills.
- **Non-existent paths are silently skipped**: If a configured directory doesn't exist, Seple T Agent ignores it without errors. Useful for optional shared directories that may not be present on every machine.

### Example

```text
~/.Seple T Agent/skills/               # Local (primary, read-write)
├── devops/deploy-k8s/
│   └── SKILL.md
└── mlops/axolotl/
    └── SKILL.md
...
Bundles live in **`~/.Seple T Agent/skill-bundles/<slug>.yaml`** and look like this:
...
- `description` (optional) — short text shown in `/bundles` and `Seple T Agent bundles list`.
...
Seple T Agent bundles create research

# Overwrite an existing bundle
Seple T Agent bundles create backend-dev --skill ... --force

# Delete a bundle
Seple T Agent bundles delete backend-dev

# Re-scan ~/.Seple T Agent/skill-bundles/ and report changes
Seple T Agent bundles reload
```
...
- You want to ship a team-wide "task profile" by checking the bundle YAML into a shared dotfiles repo and symlinking it into `~/.Seple T Agent/skill-bundles/`.

A bundle is just a YAML alias — it doesn't install skills for you. The skills themselves must already be present (in `~/.Seple T Agent/skills/` or an external skill directory). Otherwise the bundle invocation just skips the missing ones.
...
Staged writes survive restarts under `~/.Seple T Agent/pending/skills/` and are
...
```bash
Seple T Agent skills browse                              # Browse all hub skills (official first)
Seple T Agent skills browse --source official            # Browse only official optional skills
Seple T Agent skills search kubernetes                   # Search all sources
Seple T Agent skills search react --source skills-sh     # Search the skills.sh directory
Seple T Agent skills search https://mintlify.com/docs --source well-known
Seple T Agent skills inspect openai/skills/k8s           # Preview before installing
Seple T Agent skills install openai/skills/k8s           # Install with security scan
Seple T Agent skills install official/security/1password
Seple T Agent skills install skills-sh/vercel-labs/json-render/json-render-react --force
Seple T Agent skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
...
Seple T Agent skills list --source hub                   # List hub-installed skills
Seple T Agent skills check                               # Check installed hub skills for upstream updates
Seple T Agent skills update                              # Reinstall hub skills with upstream changes when needed
Seple T Agent skills audit                               # Re-scan all hub skills for security
Seple T Agent skills uninstall k8s                       # Remove a hub skill
Seple T Agent skills reset google-workspace              # Un-stick a bundled skill from "user-modified" (see below)
Seple T Agent skills reset google-workspace --restore    # Also restore the bundled version, deleting your local edits
Seple T Agent skills publish skills/my-skill --to github --repo owner/repo
Seple T Agent skills snapshot export setup.json          # Export skill config
Seple T Agent skills tap add myorg/skills-repo           # Add a custom GitHub source
```
...
|--------|---------|-------|
| `official` | `official/security/1password` | Optional skills shipped with Seple T Agent. |
| `skills-sh` | `skills-sh/vercel-labs/agent-skills/vercel-react-best-practices` | Searchable via `Seple T Agent skills search <query> --source skills-sh`. Seple T Agent resolves alias-style skills when the skills.sh slug differs from the repo folder. |
...
Seple T Agent currently integrates with these skills ecosystems and discovery sources:

#### 1. Official optional skills (`official`)

These are maintained in the Seple T Agent repository itself and install with built-in trust.
...
This is Vercel's public skills directory. Seple T Agent can search it directly, inspect skill detail pages, resolve alias-style slugs, and install from the underlying source repo.
...
```bash
Seple T Agent skills search https://mintlify.com/docs --source well-known
Seple T Agent skills inspect well-known:https://mintlify.com/docs/.well-known/skills/mintlify
Seple T Agent skills install well-known:https://mintlify.com/docs/.well-known/skills/mintlify
```

#### 4. Direct GitHub skills (`github`)

Seple T Agent can install directly from GitHub repositories and GitHub-based taps. This is useful when you already know the repo/path or want to add your own custom source repo.
...
```bash
Seple T Agent skills install openai/skills/k8s
Seple T Agent skills tap add myorg/skills-repo
```
...
[Skills Hub](https://hermes-agent.Novaedge.com/docs) page — instead of a
tag-derived guess. This is generic: any tap that ships the file gets real
categorization, no Seple T Agent-side changes required.
...
- Seple T Agent source id: `clawhub`

#### 6. Claude marketplace-style repos (`claude-marketplace`)

Seple T Agent supports marketplace repos that publish Claude-compatible plugin/marketplace manifests.

Known integrated sources include:
- [anthropics/skills](https://github.com/anthropics/skills)
- [aiskillstore/marketplace](https://github.com/aiskillstore/marketplace)

Seple T Agent source id: `claude-marketplace`

#### 7. LobeHub (`lobehub`)

Seple T Agent can search and convert agent entries from LobeHub's public catalog into installable Seple T Agent skills.

- Site: [LobeHub](https://lobehub.com/)
- Public agents index: [chat-agents.lobehub.com](https://chat-agents.lobehub.com/)
- Backing repo: [lobehub/lobe-chat-agents](https://github.com/lobehub/lobe-chat-agents)
- Seple T Agent source id: `lobehub`

#### 8. browse.sh (`browse-sh`)

Seple T Agent integrates with [browse.sh](https://browse.sh), Browserbase's catalog of 200+ site-specific browser-automation SKILL.md files (Airbnb, Amazon, arXiv, 12306.cn, Etsy, Xero, and many more). Each skill describes how to drive one website end-to-end and is suitable for use with Seple T Agent' browser tools and any browser-automation skills you already have installed.

- Site: [browse.sh](https://browse.sh/)
- Catalog API: `https://browse.sh/api/skills`
- Seple T Agent source id: `browse-sh`
- Trust level: `community`

```bash
Seple T Agent skills search airbnb --source browse-sh
Seple T Agent skills inspect browse-sh/airbnb.com/search-listings-ddgioa
Seple T Agent skills install browse-sh/airbnb.com/search-listings-ddgioa
```
...
Install `SKILL.md` directly from any HTTP(S) URL — useful when an author hosts a skill on their own site (no hub listing, no GitHub path to type). Seple T Agent also fetches explicitly referenced files under `references/`, `templates/`, `scripts/`, `assets/`, and `examples/`, then scans and installs the complete bundle.

- Seple T Agent source id: `url`
- Identifier: the URL itself (no prefix needed)
- Scope: `SKILL.md` plus exact referenced support files in the allowlisted directories. Seple T Agent does not enumerate or copy unrelated files from the host.

```bash
Seple T Agent skills install https://sharethis.chat/SKILL.md
Seple T Agent skills install https://example.com/my-skill/SKILL.md --category productivity
```
...
Seple T Agent skills install https://example.com/SKILL.md --name sharethis-chat

# Or inside a chat session:
/skills install https://example.com/SKILL.md --name sharethis-chat
```

Trust level is always `community` — the same security scan runs as for every other source. The URL is stored as the install identifier, so `Seple T Agent skills update` re-fetches from the same URL automatically when you want to refresh.
...
`Seple T Agent skills inspect ...` now also surfaces upstream metadata when available:
- repo URL
...
| Level | Source | Policy |
|-------|--------|--------|
| `builtin` | Ships with Seple T Agent | Always trusted |
...
```bash
Seple T Agent skills check          # Report which installed hub skills changed upstream
Seple T Agent skills update         # Reinstall only the skills with updates available
Seple T Agent skills update react   # Update one specific installed hub skill
```
...
If you want to share a curated set of skills — for your team, your org, or publicly — you can publish them as a **tap**: a GitHub repository other Seple T Agent users add with `Seple T Agent skills tap add <owner/repo>`. No server, no registry sign-up, no release pipeline. Just a directory of `SKILL.md` files.
...
- Each skill directory must contain a `SKILL.md` with standard [SKILL.md frontmatter](#skillmd-format) (`name`, `description`, plus optional `metadata.Seple T Agent.tags`, `version`, `author`, `platforms`, `metadata.Seple T Agent.config`).
...
Seple T Agent discovers skills by listing every subdirectory of the tap path and probing each for `SKILL.md`.
...
After pushing that to GitHub, any Seple T Agent user can subscribe and install:

```bash
Seple T Agent skills tap add my-org/hermes-skills
Seple T Agent skills search deploy
Seple T Agent skills install my-org/hermes-skills/deploy-runbook
```

#### Non-default paths

If your skills don't live under `skills/` (common when you're adding a `skills/` subtree to an existing project), edit the tap entry in `~/.Seple T Agent/.hub/taps.json`:
...
The `Seple T Agent skills tap add` CLI defaults new taps to `path: "skills/"`; edit the file directly if you need a different path. `Seple T Agent skills tap list` shows the effective path per tap.
...
```bash
Seple T Agent skills install owner/repo/skills/my-workflow
```
...
Taps are stored in `~/.Seple T Agent/.hub/taps.json` (created on demand).

## Bundled skill updates (`Seple T Agent skills reset`)

Seple T Agent ships with a set of bundled skills in `skills/` inside the repo. On install and on every `Seple T Agent update`, a sync pass copies those into `~/.Seple T Agent/skills/` and records a manifest at `~/.Seple T Agent/skills/.bundled_manifest` mapping each skill name to the content hash at the time it was synced (the **origin hash**).

On each sync, Seple T Agent recomputes the hash of your local copy and compares it to the origin hash:
...
Seple T Agent skills reset google-workspace --restore
...
:::note Profiles
Each profile has its own `.bundled_manifest` under its own `HERMES_HOME`, so `Seple T Agent -p coder skills reset <name>` only affects that profile.
:::
