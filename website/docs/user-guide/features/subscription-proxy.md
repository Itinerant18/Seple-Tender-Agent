...
chat completions — use your Seple T Agent-managed provider subscription as their
...
| Use case | "Use Seple T Agent as a chat backend" | "Use my Portal sub from another app" |
...
This opens your browser for the Nous Portal OAuth flow. Seple T Agent stores
the refresh token in `~/.Seple T Agent/auth.json` — the same place all Seple T Agent
...
Model:      Seple T Agent-4-70B    # or Seple T Agent-4.3-36B, Seple T Agent-4-405B
```
...
If you see `not logged in`, run `Seple T Agent portal`. If you see
...
supports; check `portal.Novaedge.com/models`.
...
[portal.Novaedge.com](https://portal.Novaedge.com).
