...
**Tool Search** is Seple T Agent' opt-in progressive-disclosure layer for that
...
:::info Built-in Seple T Agent tools never defer
The tools that make up Seple T Agent' core capability set (`terminal`,
...
When the model invokes `tool_call`, Seple T Agent **unwraps the bridge** and
...
- **No JS sandbox.** Seple T Agent uses the simpler "structured tools" mode
