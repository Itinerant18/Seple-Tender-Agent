...
title: "Git Worktrees"
description: "Run multiple Seple T Agent agents safely on the same repository using git worktrees and isolated checkouts"
---

# Git Worktrees

Seple T Agent is often used on large, long‑lived repositories. When you want to:
...
This page shows how to combine worktrees with Seple T Agent so each session has a clean, isolated working directory.

## Why Use Worktrees with Seple T Agent?

Seple T Agent treats the **current working directory** as the project root:

- CLI: the directory where you run `Seple T Agent` or `Seple T Agent chat`
- Messaging gateways: the directory set by `terminal.cwd` in `~/.Seple T Agent/config.yaml`
...
Now you can `cd` into the new worktree and run Seple T Agent there:

```bash
cd ../repo-feature

# Start Seple T Agent in the worktree
Seple T Agent
```
...
- Seple T Agent checkpoint data under `~/.Seple T Agent/checkpoints/` is not automatically pruned when you remove a worktree, but it is usually very small.

## Best Practices

- **One worktree per Seple T Agent experiment**
...
- **Avoid running Seple T Agent from the bare repo root when using worktrees**
  - Prefer the worktree directories instead, so each agent has a clear scope.

## Using `Seple T Agent -w` (Automatic Worktree Mode)

Seple T Agent has a built‑in `-w` flag that **automatically creates a disposable git worktree** with its own branch. You don't need to set up worktrees manually — just `cd` into your repo and run:
...
- Check out an isolated branch (e.g. `Seple T Agent/hermes-<hash>`).
...
```bash
Seple T Agent -w -z "Fix issue #123"
```

For parallel agents, open multiple terminals and run `Seple T Agent -w` in each — every invocation gets its own worktree and branch automatically.

## Putting It All Together

- Use **git worktrees** to give each Seple T Agent session its own clean checkout.
