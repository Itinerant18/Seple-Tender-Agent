...
`~/.Seple T Agent/config.yaml`, `~/.Seple T Agent/.env`, and even the shell environment — for
...
Managed scope is read from a system-level directory, default `/etc/Seple T Agent`:

```text
/etc/Seple T Agent/
├── config.yaml     # managed config layer (wins over ~/.Seple T Agent/config.yaml)
└── .env            # managed env layer (wins over ~/.Seple T Agent/.env + shell)
```
...
files. It is **never persisted** to any `.env` by Seple T Agent.
...
left user-settable. `Seple T Agent doctor` reports the *resolved* managed directory so
...
|---|---|---|
| 1 | `/etc/Seple T Agent/config.yaml` (managed) | `/etc/Seple T Agent/.env` (managed) |
| 2 | `~/.Seple T Agent/config.yaml` (user) | `~/.Seple T Agent/.env` (user) |
...
```bash
Seple T Agent config        # shows a header naming the managed source + the pinned keys
Seple T Agent doctor        # reports the resolved managed dir + pinned key counts
```

If you try to change a managed value, Seple T Agent refuses and names the source:

```bash
$ Seple T Agent config set model.default my/model
Cannot set 'model.default': it is managed by your administrator
(/etc/Seple T Agent/config.yaml) and cannot be changed.
```

The same applies to managed secrets — `Seple T Agent config set` / setup will not write
a user value for an env key pinned by the managed `.env`.

## Setting up a managed scope (administrators)

```bash
sudo mkdir -p /etc/Seple T Agent

# Pin some config values for every user on this machine
sudo tee /etc/Seple T Agent/config.yaml >/dev/null <<'YAML'
model:
  provider: nous
security:
  redact_secrets: true
...
sudo tee /etc/Seple T Agent/.env >/dev/null <<'ENV'
OPENAI_API_BASE=https://inference.example.com/v1
ENV

sudo chmod 0755 /etc/Seple T Agent
sudo chmod 0644 /etc/Seple T Agent/config.yaml /etc/Seple T Agent/.env
```

Changes take effect on the next Seple T Agent start (a malformed managed file is logged
loudly and ignored — it never blocks startup, but the admin should check
`Seple T Agent doctor` to confirm the policy is being applied).

## Security model and limitations (v1)

- **Enforcement is filesystem permissions only.** If a user has write access to
  the managed directory (or runs Seple T Agent as `root`), managed scope is advisory.
