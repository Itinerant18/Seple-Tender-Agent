...
Connect Seple T Agent to Apple iMessage via [BlueBubbles](https://bluebubbles.app/) — a free, open-source macOS server that bridges iMessage to any device.
...
- Network connectivity between Seple T Agent and the BlueBubbles server
...
Or set environment variables directly in `~/.Seple T Agent/.env`:
...
By default, Seple T Agent responds to every authorized BlueBubbles/iMessage DM or group message. To make group chats opt-in, enable mention gating:
...
With `require_mention: true`, DMs still work normally, but group-chat messages are ignored unless they match a mention pattern. If you do not configure custom patterns, Seple T Agent uses conservative defaults for `Seple T Agent` and `@Seple T Agent agent` variants.
...
When someone messages your iMessage, Seple T Agent automatically sends them a pairing code. Approve it with:
```bash
Seple T Agent pairing approve bluebubbles <CODE>
```
Use `Seple T Agent pairing list` to see pending codes and approved users.

**Pre-authorize specific users** (in `~/.Seple T Agent/.env`):
```bash
BLUEBUBBLES_ALLOWED_USERS=user@icloud.com,+15551234567
```

**Open access** (in `~/.Seple T Agent/.env`):
```bash
...
Seple T Agent will connect to your BlueBubbles server, register a webhook, and start listening for iMessage messages.

## How It Works

```
iMessage → Messages.app → BlueBubbles Server → Webhook → Seple T Agent
Seple T Agent → BlueBubbles REST API → Messages.app → iMessage
```

- **Inbound:** BlueBubbles sends webhook events to a local listener when new messages arrive. No polling — instant delivery.
- **Outbound:** Seple T Agent sends messages via the BlueBubbles REST API.
...
| `BLUEBUBBLES_MENTION_PATTERNS` | No | Seple T Agent wake words | JSON array, newline-separated, or comma-separated regex patterns for group mention matching |

Auto-marking messages as read is controlled by the `send_read_receipts` key under `platforms.bluebubbles.extra` in `~/.Seple T Agent/config.yaml` (default: `true`). There is no corresponding environment variable.
...
### Chat Addressing
You can address chats by email or phone number — Seple T Agent resolves them to BlueBubbles chat GUIDs automatically. No need to use raw GUID format.
...
- Check `Seple T Agent logs gateway` for webhook errors (or `Seple T Agent logs -f` to follow in real-time)
