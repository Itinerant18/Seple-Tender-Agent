...
sidebar_position: 11
title: "Feishu / Lark"
description: "Set up Seple T Agent as a Feishu or Lark bot"
---

# Feishu / Lark Setup

Seple T Agent integrates with Feishu and Lark as a full-featured bot. Once connected, you can chat with the agent in direct messages or group chats, receive cron job results in a home chat, and send text, images, audio, and file attachments through the normal gateway flow.

The integration supports both connection modes:

- `websocket` — recommended; Seple T Agent opens the outbound connection and you do not need a public webhook endpoint
...
| Context | Behavior |
|---------|----------|
| Direct messages | Seple T Agent responds to every message. |
| Group chats | Seple T Agent responds only when the bot is @mentioned in the chat. |
...
Select **Feishu / Lark** and scan the QR code with your Feishu or Lark mobile app. Seple T Agent will automatically create a bot application with the correct permissions and save the credentials.
...
5. Run `Seple T Agent gateway setup`, select **Feishu / Lark**, and enter the credentials when prompted.
...
Use WebSocket mode when Seple T Agent runs on your laptop, workstation, or a private server. No public URL is required. The official Lark SDK opens and maintains a persistent outbound WebSocket connection with automatic reconnection.
...
Use webhook mode only when you already run Seple T Agent behind a reachable HTTP endpoint.

```bash
FEISHU_CONNECTION_MODE=webhook
```

In webhook mode, Seple T Agent starts an HTTP server (via `aiohttp`) and serves a Feishu endpoint at:
...
## Step 3: Configure Seple T Agent
...
The `FEISHU_GROUP_POLICY` environment variable controls whether and how Seple T Agent responds in group chats:
...
| Value | Behavior |
|-------|----------|
| `open` | Seple T Agent responds to @mentions from any user in any group. |
| `allowlist` | Seple T Agent only responds to @mentions from users listed in `FEISHU_ALLOWED_USERS`. |
| `disabled` | Seple T Agent ignores all group messages entirely. |
...
Set `FEISHU_REQUIRE_MENTION=false` to let Seple T Agent read all group traffic without requiring an @mention:
...
Seple T Agent auto-detects the bot's `open_id` and display name on startup. You only need to set these manually when auto-detection cannot reach the Feishu API, or when your app uses tenant-scoped user IDs:
...
By default Seple T Agent ignores messages sent by other bots. Enable bot-to-bot messaging when you want Seple T Agent to participate in A2A orchestration or receive notifications from other bots in the same group.
...
| `mentions` | Accept only when the peer bot @mentions Seple T Agent. |
...
Gateway-driven update prompts use a native Feishu `Yes` / `No` card instead of falling back to plain text replies. When `Seple T Agent update --gateway` needs confirmation, the adapter records the selected answer in Seple T Agent's `.update_response` file and replaces the card inline with a resolved state.
...
Beyond chat, the adapter can also answer `@`-mentions left on **Feishu/Lark documents**. When a user comments on a document (local text selection or whole-doc comment) and @-mentions the bot, Seple T Agent reads the document plus the surrounding comment thread and posts an LLM reply inline on the thread.
...
Rules live in `~/.Seple T Agent/feishu_comment_rules.json` (pairing grants in `~/.Seple T Agent/feishu_comment_pairing.json`) with mtime-cached hot-reload — edits take effect on the next comment event without restarting the gateway.
...
You can invite the Seple T Agent Feishu/Lark bot into a video meeting the same way you invite a human participant. When the bot receives the meeting invitation event, Seple T Agent can automatically start an agent turn that attempts to join the meeting.

Powered by the `vc.bot.meeting_invited_v1` event, the flow is:

- A user invites the bot to a Feishu/Lark video meeting.
- Feishu/Lark sends Seple T Agent the meeting invitation event.
- Seple T Agent extracts the inviter, meeting topic, and meeting number.
...
- If the invite is malformed, or the agent cannot join, Seple T Agent drops the event or replies to the inviter with a concise explanation.
...
- Keep `im:message` and `im:message:send_as_bot` enabled so Seple T Agent can reply to the inviter.
...
Inbound messages are deduplicated using message IDs with a 24-hour TTL. The dedup state is persisted across restarts to `~/.Seple T Agent/feishu_seen_message_ids.json`.
...
| `FEISHU_APP_ID or FEISHU_APP_SECRET not set` | Set both env vars or configure via `Seple T Agent gateway setup` |
| `Another local Seple T Agent gateway is already using this Feishu app_id` | Only one Seple T Agent instance can use the same app_id at a time. Stop the other gateway first. |
...
| Peer bot messages still ignored after enabling `FEISHU_ALLOW_BOTS` | Seple T Agent can't identify itself yet — set `FEISHU_BOT_OPEN_ID` (and `FEISHU_BOT_USER_ID` if your app uses `sender_id_type=user_id`). |
