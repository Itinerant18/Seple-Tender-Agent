...
sidebar_position: 12
title: "Google Chat"
description: "Set up Seple T Agent as a Google Chat bot using Cloud Pub/Sub"
---

# Google Chat Setup

Connect Seple T Agent to Google Chat as a bot. The integration uses Cloud Pub/Sub
pull subscriptions for inbound events and the Chat REST API for outbound messages.
Equivalent ergonomics to Slack Socket Mode or Telegram long-polling: your Seple T Agent
...
> Run `Seple T Agent gateway setup` and pick **Google Chat** for a guided walk-through.
...
download the file. Save it somewhere only Seple T Agent can read (e.g.,
`~/.Seple T Agent/google-chat-sa.json`, `chmod 600`).
...
- Delivery type: **Pull**
- Message retention: **7 days** (so backlog survives a Seple T Agent restart)
...
Also grant `Pub/Sub Viewer` on the same subscription — Seple T Agent calls
...
- **App name**: whatever you want users to see ("Seple T Agent" is reasonable).
...
`ADDED_TO_SPACE` event that Seple T Agent uses to cache the bot's own `users/{id}` for
self-message filtering.

---

## Step 9: Configure Seple T Agent

Add the Google Chat section to `~/.Seple T Agent/.env`:
...
Install the dependencies the Google Chat adapter needs (no Seple T Agent extra is currently published — install them directly):
...
Send "hola" in the test DM. The bot posts a "Seple T Agent is thinking…" marker, then
...
Thread support: when a user replies inside a thread, Seple T Agent detects the
`thread.name` and posts its reply in the same thread, so each thread gets a
separate Seple T Agent session.
...
3. Download the JSON. Move it onto the host that runs Seple T Agent.
4. Register the client with Seple T Agent (run under the profile you want it scoped to):
...
Seple T Agent -p <profile> python -m plugins.platforms.google_chat.oauth \
    --client-secret /path/to/client_secret.json
```

That writes the client secret into the active profile's Seple T Agent home (e.g.
`~/.Seple T Agent/google_chat_user_client_secret.json` for the default profile). The
...
The token lands at `~/.Seple T Agent/google_chat_user_tokens/<sanitized_email>.json`.
...
single-user token at `~/.Seple T Agent/google_chat_user_token.json` (if present from
...
1. Check the Pub/Sub subscription has undelivered messages in the console.
   If it does, Seple T Agent isn't authenticated — verify `GOOGLE_CHAT_SERVICE_ACCOUNT_JSON`
...
3. Check `Seple T Agent gateway` logs for `[GoogleChat] Connected`. If you see
...
- **Attachment download protection**: Seple T Agent will only attach the SA bearer
...
- **User OAuth scope**: the per-user attachment flow requests *only*
  ...
  `~/.Seple T Agent/google_chat_user_tokens/<sanitized_email>.json` (filesystem
