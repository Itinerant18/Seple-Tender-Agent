...
description: Control your smart home with Seple T Agent via Home Assistant integration.
...
Seple T Agent integrates with [Home Assistant](https://www.home-assistant.io/) in two ways:
...
4. Click **Create Token**, give it a name like "Seple T Agent"
5. Copy the token
...
Seple T Agent registers four tools for smart home control:
...
Configure which events the agent sees in `~/.Seple T Agent/config.yaml` under the Home Assistant platform's `extra` section:
...
Outbound messages from the agent are delivered as **Home Assistant persistent notifications** (via `persistent_notification.create`). These appear in the HA notification panel with the title "Seple T Agent".
...
The adapter reads credentials from `~/.Seple T Agent/.env` (auto-merged at startup) or
from `config.yaml`. Double-check the file lives under the active Seple T Agent profile
...
running Seple T Agent — `curl -H "Authorization: Bearer <token>" <url>/api/` should
