...
description: "Chat with Seple T Agent from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Yuanbao, Microsoft Teams, LINE, Raft, Webhooks, or any OpenAI-compatible frontend via the API server — architecture and setup overview"
---

# Messaging Gateway

Chat with Seple T Agent from Telegram, Discord, Slack, WhatsApp, Signal, SMS, Email, Home Assistant, Mattermost, Matrix, DingTalk, Feishu/Lark, WeCom, Weixin, BlueBubbles (iMessage), QQ, Yuanbao, Microsoft Teams, LINE, ntfy, or your browser. The gateway is a single background process that connects to all your configured platforms, handles sessions, runs cron jobs, and delivers voice messages.

For the full voice feature set — including CLI microphone mode, spoken replies in messaging, and Discord voice-channel conversations — see [Voice Mode](/user-guide/features/voice-mode) and [Use Voice Mode with Seple T Agent](/guides/use-voice-mode-with-hermes).
...
```mermaid
flowchart TB
    subgraph Gateway["Seple T Agent Gateway"]
...
For group chats, hooks, and automation flows, Seple T Agent supports explicit silence tokens. If the agent's final response is exactly one supported token, the gateway suppresses outbound delivery and sends nothing to the chat.
...
Silence is a delivery decision only. Seple T Agent keeps the assistant silence turn in the session transcript, so the conversation still alternates normally:
...
Failed turns still surface as errors; Seple T Agent does not hide failures just because the text resembles a silence token.
...
```bash
Seple T Agent gateway setup        # Interactive setup for all messaging platforms
```
...
```bash
Seple T Agent gateway              # Run in foreground
Seple T Agent gateway setup        # Configure messaging platforms interactively
Seple T Agent gateway install      # Install as a user service (Linux) / launchd service (macOS)
sudo Seple T Agent gateway install --system   # Linux only: install a boot-time system service
Seple T Agent gateway start        # Start the default service
Seple T Agent gateway stop         # Stop the default service
Seple T Agent gateway status       # Check default service status
Seple T Agent gateway status --system         # Linux only: inspect the system service explicitly
```
...
| `/update` | Update Seple T Agent to the latest version |
...
with the `session_reset` section in `~/.Seple T Agent/config.yaml`:
...
Configure per-platform overrides in `~/.Seple T Agent/gateway.json`:
...
Seple T Agent pairing approve telegram XKGH5N7P

# Other pairing commands:
Seple T Agent pairing list          # View pending + approved users
Seple T Agent pairing revoke telegram 123456789  # Remove access
```
...
The first time you message a busy agent on any platform, Seple T Agent appends a one-line reminder to the busy-ack explaining the knob (`"💡 First-time tip — …"`). The reminder fires once per install — a flag under `onboarding.seen.busy_input_prompt` latches it. Delete that key to see the tip again.
...
Control how much tool activity is displayed in `~/.Seple T Agent/config.yaml`:
...
Off by default. When enabled, Seple T Agent prepends a human-readable timestamp
...
Seple T Agent confirms immediately:
...
When the agent running a background session uses `terminal(background=true)` to start long-running processes (servers, builds, etc.), the gateway can push status updates to your chat. Control this with `display.background_process_notifications` in `~/.Seple T Agent/config.yaml`:
...
```bash
Seple T Agent gateway install               # Install as user service
Seple T Agent gateway start                 # Start the service
Seple T Agent gateway stop                  # Stop the service
...
sudo Seple T Agent gateway install --system
sudo Seple T Agent gateway start --system
sudo Seple T Agent gateway status --system
...
The unit Seple T Agent installs already shuts the gateway down cleanly with `KillMode=mixed` + `KillSignal=SIGTERM`, and uses `Restart=always` with `RestartForceExitStatus` so updates and `/restart` respawn correctly. Do **not** add a systemd drop-in such as `ExecStopPost=/bin/kill -9 $MAINPID` — `ExecStopPost` fires on *every* stop, including clean restarts, so it `SIGKILL`s the freshly spawned instance before it stabilizes and `Restart=always` immediately respawns it. The result is an infinite restart loop (and, on Telegram, a flood of restart messages). If you've added such a drop-in, remove it: `systemctl --user edit hermes-gateway` (or `sudo systemctl edit hermes-gateway` for a system service) and delete the `ExecStopPost` line, then `systemctl --user daemon-reload`.
:::

:::tip Headless VMs: user service + linger avoids root prompts
A system service needs root for every restart — including the automatic gateway restart at the end of `Seple T Agent update`. When `Seple T Agent update` runs as a non-root user, it tries passwordless `sudo systemctl`; if that's unavailable, it skips the restart and prints the manual `sudo systemctl restart hermes-gateway` command (it never blocks on an interactive password prompt).
...
```bash
Seple T Agent gateway install          # user service
sudo loginctl enable-linger $USER   # one-time: start at boot, survive logout
```

After that, `Seple T Agent update` can restart the gateway without any privileges. If you prefer to keep the system service, either run updates with `sudo Seple T Agent update`, or grant the service account passwordless sudo for systemctl, e.g. in `sudo visudo -f /etc/sudoers.d/hermes-gateway`:

```
Seple T Agent ALL=(root) NOPASSWD: /usr/bin/systemctl --no-ask-password reset-failed hermes-gateway*, /usr/bin/systemctl --no-ask-password start hermes-gateway*, /usr/bin/systemctl --no-ask-password restart hermes-gateway*
```
:::

Avoid keeping both the user and system gateway units installed at once unless you really mean to. Seple T Agent will warn if it detects both because start/stop/status behavior gets ambiguous.

:::info Multiple installations
If you run multiple Seple T Agent installations on the same machine (with different `HERMES_HOME` directories), each gets its own systemd service name. The default `~/.Seple T Agent` uses `hermes-gateway`; other installations use `hermes-gateway-<hash>`. The `Seple T Agent gateway` commands automatically target the correct service for your current `HERMES_HOME`.
:::

### macOS (launchd)

```bash
Seple T Agent gateway install               # Install as launchd agent
Seple T Agent gateway start                 # Start the service
Seple T Agent gateway stop                  # Stop the service
Seple T Agent gateway status                # Check status
tail -f ~/.Seple T Agent/logs/gateway.log   # View logs
```

The generated plist lives at `~/Library/LaunchAgents/ai.Seple T Agent.gateway.plist`. It includes three environment variables:
...
- **HERMES_HOME** — scopes the gateway to your Seple T Agent installation.

:::tip PATH changes after install
launchd plists are static — if you install new tools (e.g. a new Node.js version via nvm, or ffmpeg via Homebrew) after setting up the gateway, run `Seple T Agent gateway install` again to capture the updated PATH. The gateway will detect the stale plist and reload automatically.
:::

:::info Multiple installations
Like the Linux systemd service, each `HERMES_HOME` directory gets its own launchd label. The default `~/.Seple T Agent` uses `ai.Seple T Agent.gateway`; other installations use `ai.Seple T Agent.gateway-<suffix>`.
:::
...
1. **Gateway log** (`~/.Seple T Agent/logs/gateway.log` or the systemd / launchd unit log). Search for the platform name and `circuit breaker`, `paused`, or `disabled`. The trip event includes the failure count and the last error.
