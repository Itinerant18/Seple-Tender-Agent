...
Connect Seple T Agent to **iMessage** through [Photon][photon], a managed
...
your first iMessage from Seple T Agent — just a phone number we can bind to
your account.
:::
...
both directions. Because the SDK is TypeScript-only, Seple T Agent runs it in a
...
Seple T Agent photon setup --phone +15551234567
```
...
2. **Finds or creates** the `Seple T Agent` project on your account.
...
Runtime credentials are written to `~/.Seple T Agent/.env`
...
(device token, dashboard project id) lives in `~/.Seple T Agent/auth.json` under
`credential_pool.photon` / `credential_pool.photon_project`.

## Authorizing users

Photon uses the same authorization model as every other Seple T Agent
channel. Choose one approach:

**DM pairing (default).** When an unknown number messages your Photon
line, Seple T Agent replies with a pairing code. Approve it with:

```bash
Seple T Agent pairing approve photon <CODE>
```

Use `Seple T Agent pairing list` to see pending codes and approved users.

**Pre-authorize specific numbers** (in `~/.Seple T Agent/.env`):

```bash
PHOTON_ALLOWED_USERS=+15551234567,+15559876543
```

**Open access** (dev only, in `~/.Seple T Agent/.env`):
...
By default Seple T Agent responds to every authorized DM and group message.
...
they match a wake-word pattern. The defaults match `Seple T Agent` and
`@Seple T Agent agent` variants. For a custom agent name, set regex patterns:
...
Send an iMessage to your assigned number and Seple T Agent will reply.

## Status & troubleshooting

```bash
Seple T Agent photon status
```

Prints saved credentials, sidecar health, your registered number, and the
assigned iMessage line Seple T Agent uses. When a Photon token and dashboard project
...
- **`sidecar deps : ✗ run Seple T Agent photon install-sidecar`** — Node is
  installed but `spectrum-ts` isn't. Run the suggested command.
- **`device token : ✗ missing`** — run `Seple T Agent photon setup` to log in.
- **`No iMessage line assigned yet`** — Spectrum is enabled but no line
  has been provisioned; re-run `Seple T Agent photon setup` or check the
  [dashboard][app].
- **Sidecar won't start** — confirm `node --version` is 18.17+ and that
  `Seple T Agent photon install-sidecar` completed without errors.
...
- **Outbound attachments are supported.** Seple T Agent sends images, voice
...
| `PHOTON_MENTION_PATTERNS` | Seple T Agent wake words  | JSON list / comma / newline regex patterns for group mentions |
