...
> Run `Seple T Agent gateway setup` and pick **SimpleX** for a guided walk-through.
...
2. Use **DM pairing** — send any message to the bot and it will reply with a pairing code. Enter that code via `Seple T Agent pairing approve simplex <CODE>`.
...
`simplex:group:12` as a cron `deliver=` target or in a `Seple T Agent send` call.
...
Or target a specific contact via the cron job's `deliver:` field, or from a shell script with the [`Seple T Agent send` CLI](/guides/pipe-script-output):

```bash
Seple T Agent send simplex:<contact-id> "Done!"
```

## Privacy notes

- SimpleX never reveals phone numbers or email addresses — contacts use opaque IDs
- The connection between Seple T Agent and the daemon is local WebSocket (`ws://127.0.0.1:5225`) — no data leaves your machine
