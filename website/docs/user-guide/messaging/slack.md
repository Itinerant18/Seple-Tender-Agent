...
sidebar_position: 4
title: "Slack"
description: "Set up Seple T Agent as a Slack bot using Socket Mode"
---

# Slack Setup

Connect Seple T Agent to Slack as a bot using Socket Mode. Socket Mode uses WebSockets instead of
public HTTP endpoints, so your Seple T Agent instance doesn't need to be publicly accessible — it works
behind firewalls, on your laptop, or on a private server.

:::warning Classic Slack Apps Deprecated
Classic Slack apps (using RTM API) were **fully deprecated in March 2025**. Seple T Agent uses the modern
...
The fastest path is to paste a manifest Seple T Agent generates for you. It
...
### Option A: From a Seple T Agent-generated manifest (recommended)

1. Generate the manifest. New Slack apps must use Agent view:
   ```bash
   Seple T Agent slack manifest --agent-view --write
   ```
   This writes `~/.Seple T Agent/slack-manifest.json` and prints paste-in
...
4. Enter an app name (e.g., "Seple T Agent") and select your workspace
5. Click **Create App**
...
it will only work in DMs. Without `files:read`, Seple T Agent can chat but **cannot reliably read user-uploaded attachments**.
...
Even with all the correct scopes and event subscriptions, Slack will not allow users to send direct messages to the bot unless the Messages Tab is enabled. This is a Slack platform requirement, not a Seple T Agent configuration issue.
:::
...
Seple T Agent uses Slack **Member IDs** (not usernames or display names) for the allowlist.
...
```bash
Seple T Agent gateway setup    # Select Slack when prompted
```

Then start the gateway:

```bash
Seple T Agent gateway              # Foreground
Seple T Agent gateway install      # Install as a user service
sudo Seple T Agent gateway install --system   # Linux only: boot-time system service
```
...
can spend the entire turn in hidden reasoning and never produce visible assistant text; Seple T Agent now
...
Every Seple T Agent command (`/btw`, `/stop`, `/new`, `/model`, `/help`, ...)
...
Seple T Agent command with its description.

Under the hood: Seple T Agent ships with a generated Slack app manifest (see
Step 1, Option A) that declares every command in
[`COMMAND_REGISTRY`](https://github.com/Novaedge/hermes-agent/blob/main/hermes_cli/commands.py)
...
New Slack apps use Slack's **Agent** messaging experience. Existing Seple T Agent
...
to `message.im`, `app_home_opened`, and `app_context_changed`, so Seple T Agent can
identify a Messages-tab DM and receive the user's active Slack context with a
turn. Seple T Agent only supplies that context as a label; it does not read the viewed
channel's history.

### Refreshing slash commands after updates

When Seple T Agent adds new commands (e.g. after `Seple T Agent update`), regenerate
the manifest and update your Slack app:

```bash
Seple T Agent slack manifest --write
```

Then in Slack:
1. Open [https://api.slack.com/apps](https://api.slack.com/apps) →
   your Seple T Agent app
2. **Features → App Manifest → Edit**
3. Paste the new contents of `~/.Seple T Agent/slack-manifest.json`
4. **Save**. Slack will prompt to reinstall the app if scopes or slash
   commands changed.

### Legacy `/Seple T Agent <subcommand>` still works

For backward compatibility with older manifests, you can still type
`/Seple T Agent btw run the tests` — Seple T Agent routes it the same way as `/btw
run the tests`. Free-form questions also work: `/Seple T Agent what's the
...
Slack never delivers them to Seple T Agent.

As a workaround, Seple T Agent recognises a leading `!` as an alternate
command prefix that works in threads (and anywhere else). Type
`!queue`, `!stop`, `!model gpt-5.4`, etc. as a regular thread reply —
Seple T Agent treats it identically to the slash form and replies in the same
thread.
...
Seple T Agent falls back to a text prompt, the prompt instructs you to reply
...
```bash
Seple T Agent slack manifest --slashes-only > /tmp/slashes.json
```
...
Understanding how Seple T Agent behaves in different contexts:

| Context | Behavior |
|---------|----------|
| **DMs** | Bot responds to every message — no @mention needed |
| **Channels** | Bot **only responds when @mentioned** (e.g., `@Seple T Agent what time is it?`). In channels, Seple T Agent replies in a thread attached to that message. |
| **Threads** | If you @mention Seple T Agent inside an existing thread, it replies in that same thread. Once the bot has an active session in a thread, **subsequent replies in that thread do not require @mention** — the bot follows the conversation naturally. |
...
Beyond the required environment variables from Step 8, you can customize Slack bot behavior through `~/.Seple T Agent/config.yaml`.
...
When `true` (the default), each user in a shared channel gets their own isolated conversation session. Two people talking to Seple T Agent in `#general` will have separate histories and contexts.
...
slack:
  ...
  # must @mention the bot before Seple T Agent will respond.
...
Set `SLACK_HOME_CHANNEL` to a channel ID where Seple T Agent will deliver scheduled messages,
...
Make sure the bot has been **invited to the channel** (`/invite @Seple T Agent`).

---

## Multi-Workspace Support

Seple T Agent can connect to **multiple Slack workspaces** simultaneously using a single gateway instance. Each workspace is authenticated independently with its own bot user ID.
...
In addition to tokens in the environment or config, Seple T Agent also loads tokens from an **OAuth token file** at:

```
~/.Seple T Agent/slack_tokens.json
```
...
- When a message arrives, Seple T Agent uses the correct workspace-specific client to respond.
...
Seple T Agent supports voice on Slack:
...
| Bot works in DMs but not in channels | **Most common issue.** Add `message.channels` and `message.groups` to event subscriptions, reinstall the app, and invite the bot to the channel with `/invite @Seple T Agent` |
...
| Bot responds but can't post in a channel | Invite the bot to the channel with `/invite @Seple T Agent` |
| Bot can chat but can't read uploaded images/files | Add `files:read`, then **reinstall** the app. Seple T Agent now surfaces attachment access diagnostics in-chat when Slack returns scope/auth/permission failures. |
...
7. ✅ Bot was **invited** to the channel (`/invite @Seple T Agent`)
...
- Tokens should be stored in `~/.Seple T Agent/.env` (file permissions `600`)
- Rotate tokens periodically via the Slack app settings
- Audit who has access to your Seple T Agent config directory
