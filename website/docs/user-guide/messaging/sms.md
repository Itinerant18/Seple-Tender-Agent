...
title: "SMS (Twilio)"
description: "Set up Seple T Agent as an SMS chatbot via Twilio"
---

# SMS Setup (Twilio)

Seple T Agent connects to SMS through the [Twilio](https://www.twilio.com/) API. People text your Twilio phone number and get AI responses back — same conversational experience as Telegram or Discord, but over standard text messages.
...
- **aiohttp** — `cd ~/.Seple T Agent/hermes-agent && uv pip install -e ".[sms]"`
...
## Step 2: Configure Seple T Agent
...
If you're running Seple T Agent locally, use a tunnel to expose the webhook:
...
Text your Twilio number — Seple T Agent will respond via SMS.
...
Seple T Agent validates that inbound webhooks genuinely originate from Twilio by verifying the `X-Twilio-Signature` header (HMAC-SHA1). This prevents attackers from injecting forged messages.
...
3. Check Seple T Agent gateway logs for Twilio API errors
