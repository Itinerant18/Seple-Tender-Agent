...
Use the Teams meeting pipeline when you want Seple T Agent to ingest Microsoft Graph meeting events, fetch transcripts first, fall back to recordings plus STT when needed, and deliver a structured summary to downstream sinks.

Prerequisites: see [Microsoft Teams](./teams.md) for the underlying bot/credential setup.

> Run `Seple T Agent gateway setup` and pick **Teams Meetings** for a guided walk-through.
...
Operator actions stay in the CLI (the `teams-pipeline` subcommand is registered by the `teams_pipeline` plugin — enable it via `Seple T Agent plugins enable teams_pipeline` or set `plugins.enabled: [teams_pipeline]` in `config.yaml`):
...
- a working Seple T Agent install
...
Add Graph app-only credentials to `~/.Seple T Agent/.env`:
...
Example `~/.Seple T Agent/config.yaml`:
...
Use this when you want Seple T Agent to post the summary through Microsoft Graph into a Teams chat or channel.
...
Start Seple T Agent normally after updating config:

```bash
Seple T Agent gateway run
```

Or, if you run Seple T Agent in Docker, start the gateway the same way you already do for your deployment.
...
Microsoft Graph caps webhook subscriptions at 72 hours and will not auto-renew them. You MUST schedule `Seple T Agent teams-pipeline maintain-subscriptions` before going live, or notifications will silently stop three days after any manual subscription creation. See [Automating subscription renewal](/guides/operate-teams-meeting-pipeline#automating-subscription-renewal-required-for-production) in the operator runbook — three options (Seple T Agent cron, systemd timer, plain crontab).
...
| Jobs do not appear in `Seple T Agent teams-pipeline list` | Confirm `msgraph_webhook` is enabled and that subscriptions point at the right notification URL |
