...
description: "Set up Seple T Agent as a Microsoft Teams bot"
---

# Microsoft Teams Setup

Connect Seple T Agent to Microsoft Teams as a bot. Unlike Slack's Socket Mode, Teams delivers messages by calling a **public HTTPS webhook**, so your instance needs a publicly reachable endpoint — either a dev tunnel (local dev) or a real domain (production).
...
> Run `Seple T Agent gateway setup` and pick **Microsoft Teams** for a guided walk-through.
...
Teams delivers @mentions as regular messages with `<at>BotName</at>` tags, which Seple T Agent strips automatically before processing.
...
Alternatively, configure via `~/.Seple T Agent/config.yaml`:
...
| `No inference provider configured` | Check that `ANTHROPIC_API_KEY` (or another provider key) is set in `~/.Seple T Agent/.env` |
...
| Teams shows "This bot is not responding" | The webhook returned an error. Check `docker logs Seple T Agent` for tracebacks |
...
- Store credentials in `~/.Seple T Agent/.env` with permissions `600` (`chmod 600 ~/.Seple T Agent/.env`)
