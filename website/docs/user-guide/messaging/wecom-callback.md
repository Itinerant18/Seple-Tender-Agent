...
Connect Seple T Agent to WeCom (Enterprise WeChat) as a self-built enterprise application using the callback/webhook model.

:::info WeCom Bot vs WeCom Callback
Seple T Agent supports two WeCom integration modes:
...
> Run `Seple T Agent gateway setup` and pick **WeCom Callback** for a guided walk-through.
...
3. Seple T Agent decrypts the message, queues it for the agent
...
(Use `Seple T Agent gateway start` only after `Seple T Agent gateway install` has registered the systemd/launchd service.)
...
console. A mismatch between the token configured in Seple T Agent and the token the
...
in `~/.Seple T Agent/.env` values around `=` will also break signature checks. After
fixing, restart `Seple T Agent gateway run`.
...
Check `Seple T Agent gateway run` logs for the bound host/port. If the adapter bound to
