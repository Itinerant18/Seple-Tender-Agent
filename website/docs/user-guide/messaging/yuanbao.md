...
sidebar_position: 16
title: "Yuanbao"
description: "Connect Seple T Agent to the Yuanbao enterprise messaging platform via WebSocket gateway"
---

# Yuanbao

Connect Seple T Agent to [Yuanbao](https://yuanbao.tencent.com/), Tencent's enterprise messaging platform. The adapter uses a WebSocket gateway for real-time message delivery and supports both direct (C2C) and group conversations.
...
After initial setup, verify these variables in `~/.Seple T Agent/.env`:
...
All standard Seple T Agent commands work on Yuanbao:
...
4. Review gateway logs: `tail -f ~/.Seple T Agent/logs/gateway.log`
...
Yuanbao has a maximum message size. Seple T Agent automatically chunks large responses with Markdown-aware splitting (respects code fences, tables, and paragraph boundaries).
...
```bash
HERMES_LOG_LEVEL=debug Seple T Agent gateway
```
...
```bash
Seple T Agent chat -q "Send 'Hello from CLI' to yuanbao:group:group_code"
```
