...
Seple T Agent is designed with a defense-in-depth security model. This page covers every security boundary — from command approval to container isolation to user authorization on messaging platforms.
...
Before executing any command, Seple T Agent checks it against a curated list of dangerous patterns. If a match is found, the user must explicitly approve it.

### Approval Modes

The approval system supports three modes, configured via `approvals.mode` in `~/.Seple T Agent/config.yaml`:
...
| `timeout` | `60` | Seconds Seple T Agent waits for an approval reply before timing out. |
...
1. **CLI flag**: Start a session with `Seple T Agent --yolo` or `Seple T Agent chat --yolo`
...
When YOLO is active, Seple T Agent shows two persistent visual reminders so it's hard to forget that approval prompts are bypassed:
...
Some commands are so catastrophic — irreversible filesystem wipes, fork bombs, direct block-device writes — that Seple T Agent refuses to run them **regardless** of:
...
Configure the timeout in `~/.Seple T Agent/config.yaml`:
...
| `tee` to `/etc/`, `~/.ssh/`, `~/.Seple T Agent/.env` | Overwrite sensitive file via tee |
| `>` / `>>` to `/etc/`, `~/.ssh/`, `~/.Seple T Agent/.env` | Overwrite sensitive file via redirection |
...
| `pkill`/`killall` Seple T Agent/gateway | Self-termination prevention |
...
Commands approved with "always" are saved to `~/.Seple T Agent/config.yaml`:
...
| Seple T Agent credential stores | `auth.json`, `.env`, `.anthropic_oauth.json`, `mcp-tokens/`, `pairing/` under HERMES_HOME (active profile and global root) |
...
- Set automatically in the [official Docker image](https://github.com/Novaedge/hermes-agent) (`HERMES_WRITE_SAFE_ROOT=/opt/data`)
- Supports multiple roots separated by `:` on Unix or `;` on Windows
- **Do not add to `~/.Seple T Agent/.env` casually.** If you set it to a project directory, the agent cannot write to `~/.Seple T Agent/cron/jobs.json`, profile skills, or other Seple T Agent state outside that prefix

To allow both a workspace and Seple T Agent home:
...
### Cron and other Seple T Agent state

Do not ask the agent to `patch` `~/.Seple T Agent/cron/jobs.json` directly. Use the `cronjob` tool, [`Seple T Agent cron`](./features/cron.md), or `/cron` — they update the job store through the supported API. The same applies to other Seple T Agent control files when write safety blocks direct edits.
...
When running the messaging gateway, Seple T Agent controls who can interact with the bot through a layered authorization system.
...
Set allowed user IDs as comma-separated values in `~/.Seple T Agent/.env`:
...
Set GATEWAY_ALLOW_ALL_USERS=true in ~/.Seple T Agent/.env to allow open access,
...
For more flexible authorization, Seple T Agent includes a code-based pairing system. Instead of requiring user IDs upfront, unknown users receive a one-time pairing code that the bot owner approves via the CLI.

**How it works:**

1. An unknown user sends a DM to the bot
2. The bot replies with an 8-character pairing code
3. The bot owner runs `Seple T Agent pairing approve <platform> <code>` on the CLI
4. The user is permanently approved for that platform

Control how unauthorized direct messages are handled in `~/.Seple T Agent/config.yaml`:
...
Seple T Agent pairing list

# Approve a pairing code
Seple T Agent pairing approve telegram ABC12DEF

# Revoke a user's access
Seple T Agent pairing revoke telegram 123456789

# Clear all pending codes
Seple T Agent pairing clear-pending
```

:::tip Docker users: run pairing commands as the `Seple T Agent` user
The official Docker image runs the gateway as the unprivileged `Seple T Agent` user
...
Always pass `-u Seple T Agent`:

```bash
docker exec -u Seple T Agent hermes-agent Seple T Agent pairing approve telegram ABC12DEF
```
...
[i10270]: https://github.com/Novaedge/hermes-agent/issues/10270
:::

**Storage:** Pairing data is stored in `~/.Seple T Agent/pairing/` with per-platform JSON files:
...
When using the `docker` terminal backend, Seple T Agent applies strict security hardening to every container.
...
Container resources are configurable in `~/.Seple T Agent/config.yaml`:
...
- **Persistent mode** (`container_persistent: true`): Bind-mounts `/workspace` and `/root` from `~/.Seple T Agent/sandboxes/docker/<task_id>/`
...
When loaded, Seple T Agent checks if these files exist in the active profile's `HERMES_HOME` and registers them for mounting:
...
| **terminal** (local) | Blocks explicit Seple T Agent infrastructure vars (provider keys, gateway tokens, tool API keys) | ✅ Passthrough vars bypass the blocklist |
...
- Seple T Agent infrastructure secrets (provider API keys, gateway tokens) should never be added to `env_passthrough` — they have dedicated mechanisms
...
Seple T Agent integrates [tirith](https://github.com/sheeki03/tirith) for content-level command scanning before execution. Tirith detects threats that pattern matching alone misses:
...
Tirith ships prebuilt binaries for Linux (x86_64 / aarch64) and macOS (x86_64 / arm64). On platforms with no prebuilt binary (Windows, etc.), tirith is silently skipped — pattern-matching guards still run, and the CLI does not surface an "unavailable" banner. To use tirith on Windows, run Seple T Agent under WSL.
...
4. **Store secrets securely** — keep API keys in `~/.Seple T Agent/.env` with proper file permissions
...
9. **Monitor logs** — check `~/.Seple T Agent/logs/` for unauthorized access attempts
10. **Keep updated** — run `Seple T Agent update` regularly for security patches
...
For maximum security, run the gateway on a separate machine or VM. Set `terminal.backend: ssh` in `config.yaml`, then provide host details via environment variables in `~/.Seple T Agent/.env`:
...
TERMINAL_SSH_USER=Seple T Agent
...
Seple T Agent ships with a built-in advisory scanner that flags Python packages in the active venv that match a curated catalog of known-compromised versions (supply-chain worms like the May 2026 `mistralai 2.4.6` poisoning). Implementation lives in `hermes_cli/security_advisories.py`.

How it runs:

- **CLI startup banner.** A one-line warning is printed if any advisory matches, with a pointer to `Seple T Agent doctor` for the full remediation.
- **`Seple T Agent doctor`.** Surfaces every active advisory with version specifics and 2-4 step remediation instructions.
...
```bash
Seple T Agent doctor --ack <advisory-id>
```
...
Many features (Mistral TTS, ElevenLabs, Honcho memory, Bedrock, Slack, Matrix, …) depend on Python packages that not every user needs. Seple T Agent installs these **lazily** on first use rather than eagerly under `hermes-agent[all]`. The implementation lives in `tools/lazy_deps.py`.
...
3. If the install fails or the user has disabled lazy installs, the call raises `FeatureUnavailable` with the actual pip stderr and a pointer at `Seple T Agent tools`.
...
When disabled, backends that need optional deps will tell the user to run the install manually (`pip install …`) or pick a different backend via `Seple T Agent tools`.
