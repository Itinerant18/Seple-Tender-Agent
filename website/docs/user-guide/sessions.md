...
Seple T Agent automatically saves every conversation as a session. Sessions enable conversation resume, cross-session search, and full conversation history management.
...
1. **SQLite database** (`~/.Seple T Agent/state.db`) — structured session metadata with FTS5 full-text search, plus full message history
...
Seple T Agent stores session history so it can resume conversations, but it does not
...
Seple T Agent explicitly injects for that turn.
...
For example, if a user sends an image and asks Seple T Agent to make a meme from it,
Seple T Agent may inspect that image once with vision and run an image-processing
...
`Seple T Agent sessions prune` only when you want to delete old ended sessions from
...
| Source | Description |
|--------|-------------|
| `cli` | Interactive CLI (`Seple T Agent` or `Seple T Agent chat`) |
...
Seple T Agent --continue
Seple T Agent -c

# Or with the chat subcommand
Seple T Agent chat --continue
Seple T Agent chat -c
```
...
Seple T Agent -c "my project"

# If there are lineage variants (my project, my project #2, my project #3),
# this automatically resumes the most recent one
Seple T Agent -c "my project"   # → resumes "my project #3"
```
...
Seple T Agent --resume 20250305_091523_a1b2c3d4
Seple T Agent -r 20250305_091523_a1b2c3d4

# Resume by title
Seple T Agent --resume "refactoring auth"

# Or with the chat subcommand
Seple T Agent chat --resume 20250305_091523_a1b2c3d4
```

Session IDs are shown when you exit a CLI session, and can be found with `Seple T Agent sessions list`.

### Conversation Recap on Resume

When you resume a session, Seple T Agent displays a compact recap of the previous conversation in a styled panel before the input prompt:

<img className="docs-terminal-figure" src={useBaseUrl('/img/docs/session-recap.svg')} alt="Stylized preview of the Previous Conversation recap panel shown when resuming a Seple T Agent session." />
...
To disable the recap and keep the minimal one-liner behavior, set in `~/.Seple T Agent/config.yaml`:
...
**Resume back to CLI:** when you want to come back to a desktop, just run `/resume <title>` (or `Seple T Agent -r "<title>"` from the shell) and pick up where the platform left off.
...
Seple T Agent automatically generates a short descriptive title (3–7 words) for each session after the first exchange. This runs in a background thread using a fast auxiliary model, so it adds no latency. You'll see auto-generated titles when browsing sessions with `Seple T Agent sessions list` or `Seple T Agent sessions browse`.
...
```bash
Seple T Agent sessions rename 20250305_091523_a1b2c3d4 "refactoring auth module"
```
...
When a session's context is compressed (manually via `/compress` or automatically), Seple T Agent creates a new continuation session. If the original had a title, the new session automatically gets a numbered title:

```
"my project" → "my project #2" → "my project #3"
```

When you resume by name (`Seple T Agent -c "my project"`), it automatically picks the most recent session in the lineage.
...
Seple T Agent provides a full set of session management commands via `Seple T Agent sessions`:

### List Sessions

```bash
# List recent sessions (default: last 20)
Seple T Agent sessions list

# Filter by platform
Seple T Agent sessions list --source telegram

# Show more sessions
Seple T Agent sessions list --limit 50
```
...
`Seple T Agent sessions export` is one surface for every export format, selected with `--format`:
...
Seple T Agent sessions export backup.jsonl

# Export sessions from a specific platform
Seple T Agent sessions export telegram-history.jsonl --source telegram

# Export a single session
Seple T Agent sessions export session.jsonl --session-id 20250305_091523_a1b2c3d4

# Redact API keys/tokens/credentials from the exported content
Seple T Agent sessions export backup.jsonl --redact
```
...
Seple T Agent sessions export --format html --session-id 20250305_091523_a1b2c3d4 transcript.html

# All Telegram sessions from the last week in one file, secrets redacted
Seple T Agent sessions export --format html --newer-than 1w --source telegram --redact archive.html
```
...
Seple T Agent sessions export prompts.jsonl --session-id 20250305_091523_a1b2c3d4 --only user-prompts

# Markdown, straight to stdout
Seple T Agent sessions export - --session-id 20250305_091523_a1b2c3d4 --only user-prompts --format md
```
...
Seple T Agent sessions export --format trace

# One session to a local trace file
Seple T Agent sessions export --format trace --session-id 20250305_091523_a1b2c3d4 trace.jsonl

# Upload straight to your private HF traces dataset
Seple T Agent sessions export --format trace --session-id 20250305_091523_a1b2c3d4 --upload
```
...
Pass `--format md` or `--format qmd` when you want a readable, file-based archive before hiding or deleting old sessions. Markdown/QMD exports write one file per session into a directory (default: `~/.Seple T Agent/session-exports`).

```bash
# Export one session to Markdown
Seple T Agent sessions export --format md --session-id 20250305_091523_a1b2c3d4

# Export a compression lineage as one logical document
Seple T Agent sessions export --format md --session-id 20250305_091523_a1b2c3d4 --lineage logical

# Preview ended sessions older than 90 days without writing files
Seple T Agent sessions export --format md --older-than 90 --dry-run

# Export ended Telegram sessions older than 2 weeks to QMD files
Seple T Agent sessions export --format qmd --older-than 2w --source telegram

# Export long Claude sessions, secrets redacted
Seple T Agent sessions export --format md --model sonnet --min-messages 50 --redact

# Only after verification, export and delete one explicitly named session
Seple T Agent sessions export --format md --session-id 20250305_091523_a1b2c3d4 --delete-after-verified --yes
```
...
Seple T Agent sessions delete 20250305_091523_a1b2c3d4

# Delete without confirmation
Seple T Agent sessions delete 20250305_091523_a1b2c3d4 --yes
```
...
Seple T Agent sessions rename 20250305_091523_a1b2c3d4 "debugging auth flow"
...
Seple T Agent sessions prune

# Custom age threshold — bare numbers are days
Seple T Agent sessions prune --older-than 30

# Durations work too: 5h, 30m, 2d, 1w
Seple T Agent sessions prune --older-than 12h

# Delete only a specific time window (e.g. a batch of test sessions
# created in the last 5 hours)
Seple T Agent sessions prune --newer-than 5h

# Explicit window with absolute timestamps
Seple T Agent sessions prune --after "2026-07-05 09:00" --before "2026-07-05 14:30"

# Only prune sessions from a specific platform (all ages — any filter
# disables the implicit 90-day default)
Seple T Agent sessions prune --source telegram
Seple T Agent sessions prune --source cron --older-than 60   # add a time flag to narrow

# More filters — all AND together
Seple T Agent sessions prune --newer-than 5h --title "smoke test"   # title substring
Seple T Agent sessions prune --older-than 30 --max-messages 3        # tiny sessions
Seple T Agent sessions prune --cwd ~/scratch --end-reason done       # by cwd / end reason
Seple T Agent sessions prune --model gpt-5 --older-than 1w           # by model (substring)
Seple T Agent sessions prune --provider openrouter --older-than 60   # by billing provider
Seple T Agent sessions prune --branch feature/old-experiment         # by git branch
Seple T Agent sessions prune --user 12345678 --chat-type group       # by messaging origin
Seple T Agent sessions prune --max-tokens 500 --older-than 7         # by token usage
Seple T Agent sessions prune --max-cost 0.01 --max-tool-calls 0      # cheap, tool-less runs

# Preview what would be deleted, without deleting anything
Seple T Agent sessions prune --newer-than 5h --dry-run

# Skip confirmation
Seple T Agent sessions prune --older-than 30 --yes
```
...
the implicit 90-day default, so `Seple T Agent sessions prune --source cron` or
`--model gpt-4o` matches all ages — add a time flag to narrow it. Only a
completely bare `Seple T Agent sessions prune` keeps the 90-day cutoff. Every
...
`Seple T Agent sessions archive` takes the same filters as `prune` but soft-hides
...
Seple T Agent sessions archive --newer-than 5h

# Archive by title substring, preview first
Seple T Agent sessions archive --title "dry run" --dry-run
Seple T Agent sessions archive --title "dry run" --yes
```

At least one filter is required — a bare `Seple T Agent sessions archive` refuses to
archive your entire history. Archived sessions are hidden from
`Seple T Agent sessions list` and `/resume` but remain in the database and can be
unarchived from the Desktop/Dashboard session list.

### Session Statistics

```bash
Seple T Agent sessions stats
```
...
For deeper analytics — token usage, cost estimates, tool breakdown, and activity patterns — use [`Seple T Agent insights`](/reference/cli-commands#hermes-insights).
...
When Seple T Agent cannot get a participant identifier for a shared chat, it falls back to one shared session for that room.

### Shared vs Isolated Group Sessions

By default, Seple T Agent uses `group_sessions_per_user: true` in `config.yaml`. That means:

- Alice and Bob can both talk to Seple T Agent in the same Discord channel without sharing transcript history
...
|------|------|-------------|
| SQLite database | `~/.Seple T Agent/state.db` | All session metadata + messages with FTS5 |
| Gateway messages    | `~/.Seple T Agent/state.db`   | SQLite — canonical store for all session messages |
| Gateway routing index | `~/.Seple T Agent/sessions/sessions.json` | Maps session keys to active session IDs (origin metadata, expiry flags) |

The SQLite database uses WAL mode for concurrent readers and a single writer, which suits the gateway's multi-platform architecture well.

:::warning `sessions.json` is not the session list
`~/.Seple T Agent/sessions/sessions.json` is the **gateway routing index** — it maps
...
`Seple T Agent sessions list`, `/sessions`, and the dashboard all read `state.db`,
which holds **every** session (CLI, TUI, and gateway). The `/save` snapshots
under `~/.Seple T Agent/sessions/saved/*.json` are convenience exports, not the index.

If CLI sessions genuinely don't appear in `Seple T Agent sessions list`, the cause is
`state.db` not receiving them — run `Seple T Agent sessions repair` and watch for a
...
`*.jsonl` files in `~/.Seple T Agent/sessions/`. They are no longer written or
read by Seple T Agent. Safe to delete after verifying the corresponding session
exists in state.db.
:::
...
- Pruning runs at most once per `sessions.min_interval_hours` (default 24); the last-run timestamp is tracked inside `state.db` itself so it's shared across every Seple T Agent process in the same `HERMES_HOME`

Default is **off** — session history is valuable for `session_search` recall, and silently deleting it could surprise users. Enable in `~/.Seple T Agent/config.yaml`:
...
Seple T Agent sessions delete <session_id>
