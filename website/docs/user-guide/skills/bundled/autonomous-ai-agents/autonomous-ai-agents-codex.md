...
Delegate coding tasks to [Codex](https://github.com/openai/codex) via the Seple T Agent terminal. Codex is OpenAI's autonomous coding agent CLI.
...
For Seple T Agent itself, `model.provider: openai-codex` uses Seple T Agent-managed Codex
OAuth from `~/.Seple T Agent/auth.json` after `Seple T Agent auth add openai-codex`. For the
...
## Seple T Agent Gateway Caveat

When invoking the Codex CLI from a Seple T Agent gateway/service context (for example,
