...
4. Store the token in `${HERMES_HOME:-~/.Seple T Agent}/.env` (or via `Seple T Agent setup`):
   ```
...
`-s` suppresses curl's progress bar — keep it set for every call so the tool output stays clean for Seple T Agent. Pipe through `python3 -m json.tool` (always present) or `jq` (if installed) for readable JSON.
...
## Typical Seple T Agent Workflow
...
## Important Notes for Seple T Agent
...
- **`AIRTABLE_API_KEY` flows from `${HERMES_HOME:-~/.Seple T Agent}/.env` into the subprocess automatically** when this skill is loaded — no need to re-export it before each `curl` call.
