...
| Version | `1.1.1` |
| Author | xdevplatform + openclaw + Seple T Agent |
| License | MIT |
...
- The user must fill `~/.xurl` with secrets manually on their own machine. In Docker, this must be the `~` seen by Seple T Agent tool subprocesses; see the Docker note below.
...
> **Docker HOME pitfall:** In the official Seple T Agent Docker layout, `/opt/data` is `HERMES_HOME`, but Seple T Agent tool subprocesses use `/opt/data/home` as `HOME`. That means `~/.xurl` resolves to `/opt/data/home/.xurl` for Seple T Agent-run `xurl` commands, not `/opt/data/.xurl`. Run the user setup with the same HOME:
> ```bash
...
> ```
> If `HOME=/opt/data xurl auth status` succeeds but `HOME=/opt/data/home xurl auth status` shows no apps or tokens, Seple T Agent tool calls will not see the credentials.
...
- **Token storage:** `~/.xurl` is YAML. In Docker, use the Seple T Agent subprocess HOME (`/opt/data/home` in the official image) so tokens land under `/opt/data/home/.xurl`. Never read or send this file to LLM context.
...
- Seple T Agent adaptation: reformatted for Seple T Agent skill conventions; safety guardrails preserved verbatim.
