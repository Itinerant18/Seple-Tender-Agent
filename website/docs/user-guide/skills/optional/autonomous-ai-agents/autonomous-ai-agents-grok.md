...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/autonomous-ai-agents/grok` |
| Path | `optional-skills/autonomous-ai-agents/grok` |
| Version | `0.1.0` |
| Author | Matt Maximo (MattMaximo), Seple T Agent |
| License | MIT |
...
# Grok Build CLI — Seple T Agent Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Seple T Agent terminal. Grok
...
1. Prepare stable input files first with Seple T Agent tools (`read_file`,
...
2. **Don't conflate Seple T Agent' xAI auth with the `grok` CLI's auth.** Seple T Agent'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Seple T Agent' xAI auth carries
   over.
