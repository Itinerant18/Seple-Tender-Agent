...
description: "Configure and use Honcho memory with Seple T Agent -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session su..."
---
...
Configure and use Honcho memory with Seple T Agent -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/autonomous-ai-agents/honcho` |
...
# Honcho Memory for Seple T Agent

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Seple T Agent profile its own peer identity while sharing a unified view of the user.
...
```bash
Seple T Agent memory setup honcho
...
See: https://docs.honcho.dev/v3/guides/integrations/Seple T Agent#running-honcho-locally-with-hermes

### Verify

```bash
Seple T Agent honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Seple T Agent profile's AI peer
...
Honcho models conversations as interactions between **peers**. Seple T Agent creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Seple T Agent instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Seple T Agent run |
| `global` | Single session across all directories |

Manual override: `Seple T Agent honcho map my-project-name`
...
Each Seple T Agent profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
# creates host block Seple T Agent.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `Seple T Agent.coder` host block in `honcho.json`
...
```bash
Seple T Agent honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Seple T Agent when Honcho memory is active.
...
Run `Seple T Agent honcho setup`. Ensure `memory.provider: honcho` is in `~/.Seple T Agent/config.yaml`.

### Memory not persisting across sessions
Check `Seple T Agent honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `Seple T Agent profile create <name> --clone`. For existing profiles: `Seple T Agent honcho sync`.
...
| Command | Description |
|---------|-------------|
| `Seple T Agent honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `Seple T Agent honcho status` | Show resolved config, connection test, peer info for active profile |
| `Seple T Agent honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `Seple T Agent honcho disable` | Disable Honcho for the active profile |
| `Seple T Agent honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `Seple T Agent honcho peers` | Show peer identities across all profiles |
| `Seple T Agent honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `Seple T Agent honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `Seple T Agent honcho sessions` | List known directory-to-session-name mappings |
| `Seple T Agent honcho map <name>` | Map current working directory to a Honcho session name |
| `Seple T Agent honcho identity` | Seed AI peer identity or show both peer representations |
| `Seple T Agent honcho sync` | Create host blocks for all Seple T Agent profiles that don't have one yet |
| `Seple T Agent honcho migrate` | Step-by-step migration guide from OpenClaw native memory to Seple T Agent + Honcho |
| `Seple T Agent memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `Seple T Agent memory status` | Show active memory provider and config |
| `Seple T Agent memory off` | Disable external memory provider |
