...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/autonomous-ai-agents/openhands` |
| Path | `optional-skills/autonomous-ai-agents/openhands` |
| Version | `0.1.0` |
| Author | Tim Koepsel (xzessmedia), Seple T Agent |
| License | MIT |
...
This skill is the headless-mode wrapper for batch / one-shot delegation. The interactive textual UI is not used from Seple T Agent.
...
For Claude-native, prefer `claude-code`. For OpenAI-native, prefer `codex`. For Seple T Agent-native subagents, use `delegate_task`.
...
- Sibling skills: `claude-code` (Anthropic-only), `codex` (OpenAI-only), `opencode` (multi-provider via OpenCode), `hermes-agent` (Seple T Agent subagents via `delegate_task`).
