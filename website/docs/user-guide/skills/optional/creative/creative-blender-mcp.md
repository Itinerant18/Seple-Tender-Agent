...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/creative/blender-mcp` |
| Path | `optional-skills/creative/blender-mcp` |
| Version | `2.1.0` |
| Author | alireza78a + kshitijk4poor + Seple T Agent |
...
Companion skill for the `blender` entry in the Seple T Agent MCP catalog. The MCP
...
1. Install the MCP server from the Nous catalog (one-time):
       ...
       Seple T Agent mcp install blender
...
3. Every session: start Blender FIRST, press N in the viewport, open the
   ...
   socket). Then start your Seple T Agent session so the MCP tools are loaded.
...
opt into its tools with `Seple T Agent mcp configure blender`.
...
- The blender MCP tools only exist if the server is installed and the session
  started after install. If they're missing, run `Seple T Agent mcp install blender`
...
- Render output paths must be absolute (`/tmp/render.png`), not relative —
  they resolve on the BLENDER host's filesystem, which matters if Seple T Agent and
