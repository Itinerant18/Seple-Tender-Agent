...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/creative/meme-generation` |
...
4. Find the skill's script directory:
   ```
   SKILL_DIR=$(dirname "$(find ~/.Seple T Agent/skills -path '*/meme-generation/SKILL.md' 2>/dev/null | head -1)")
   ```
5. Run the generator:
   ```bash
