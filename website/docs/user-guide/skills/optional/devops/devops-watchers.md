...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/devops/watchers` |
...
Watch a GitHub repo (set `GITHUB_TOKEN` in `${HERMES_HOME:-~/.Seple T Agent}/.env` to avoid the 60 req/hr anonymous rate limit):

```bash
python $HERMES_HOME/skills/devops/watchers/scripts/watch_github.py \
  --name hermes-issues --repo Novaedge/hermes-agent --scope issues
```
