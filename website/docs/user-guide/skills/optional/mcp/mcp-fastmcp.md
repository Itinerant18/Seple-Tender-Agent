...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/mcp/fastmcp` |
...
- smoke-test a server with the FastMCP CLI before wiring it into Seple T Agent or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Seple T Agent. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.Seple T Agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.Seple T Agent/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Seple T Agent integration, either:

- configure the server in `~/.Seple T Agent/config.yaml` using the `native-mcp` skill, or
...
### Seple T Agent cannot see the deployed server

The server-building part may be correct while the Seple T Agent config is not. Load the `native-mcp` skill and configure the server in `~/.Seple T Agent/config.yaml`, then restart Seple T Agent.
