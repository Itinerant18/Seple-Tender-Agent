---
title: "Openclaw Migration — Migrate a user's OpenClaw customization footprint into Seple T Agent"
sidebar_label: "Openclaw Migration"
description: "Migrate a user's OpenClaw customization footprint into Seple T Agent"
---
...
Migrate a user's OpenClaw customization footprint into Seple T Agent. Imports Seple T Agent-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.

## Skill metadata

| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/migration/openclaw-migration` |
| Path | `optional-skills/migration/openclaw-migration` |
| Version | `1.0.0` |
| Author | Seple T Agent (Novaedge) |
| License | MIT |
| Platforms | linux, macos, windows |
| Tags | `Migration`, `OpenClaw`, `Seple T Agent`, `Memory`, `Persona`, `Import` |
...
# OpenClaw -> Seple T Agent Migration

Use this skill when a user wants to move their OpenClaw setup into Seple T Agent with minimal manual cleanup.
...
```bash
Seple T Agent claw migrate              # Full interactive migration
Seple T Agent claw migrate --dry-run    # Preview what would be migrated
Seple T Agent claw migrate --preset user-data   # Migrate without secrets
Seple T Agent claw migrate --overwrite  # Overwrite existing conflicts
Seple T Agent claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `Seple T Agent setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hermes.py` to:

- import `SOUL.md` into the Seple T Agent home directory as `SOUL.md`
- transform OpenClaw `MEMORY.md` and `USER.md` into Seple T Agent memory entries
- merge OpenClaw command approval patterns into Seple T Agent `command_allowlist`
- migrate Seple T Agent-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map OpenClaw workspace settings to Seple T Agent working-directory configuration
- copy OpenClaw skills into `~/.Seple T Agent/skills/openclaw-imports/`
- optionally copy the OpenClaw workspace instructions file into a chosen Seple T Agent workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.Seple T Agent/tts/`
- archive non-secret docs that do not have a direct Seple T Agent destination
...
- `~/.Seple T Agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py`

Do not guess a shorter path like `~/.Seple T Agent/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.Seple T Agent/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Seple T Agent-compatible secrets, currently:
...
Seple T Agent CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported OpenClaw skills already exist in Seple T Agent. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the OpenClaw workspace instructions file into a Seple T Agent workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Seple T Agent skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.Seple T Agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.Seple T Agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.Seple T Agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.Seple T Agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.Seple T Agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.Seple T Agent/skills/migration/openclaw-migration/scripts/openclaw_to_hermes.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Seple T Agent unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Seple T Agent targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
...
6. Even in secret-migration mode, only migrate secrets with a clean Seple T Agent destination. Unsupported auth blobs must still be reported as skipped.
...
- Seple T Agent persona state imported
- Seple T Agent memory files populated with converted OpenClaw knowledge
- OpenClaw skills available under `~/.Seple T Agent/skills/openclaw-imports/`
