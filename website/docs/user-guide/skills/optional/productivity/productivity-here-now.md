...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/productivity/here-now` |
...
bash "$PUBLISH" {file-or-dir} --client Seple T Agent
```
...
```bash
PUBLISH="${HERMES_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client Seple T Agent
```
...
| `--client {name}`      | Agent name for attribution (e.g. `Seple T Agent`)    |
