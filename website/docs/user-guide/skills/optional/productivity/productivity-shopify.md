...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/productivity/shopify` |
...
4. Save to `${HERMES_HOME:-~/.Seple T Agent}/.env`:
   ```
...
}' '{"input":{"title":"Test Hoodie","status":"DRAFT","vendor":"Seple T Agent","productType":"Apparel","tags":["test"]}}'
```
