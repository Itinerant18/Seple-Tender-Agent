...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/productivity/siyuan` |
...
3. Store it in `${HERMES_HOME:-~/.Seple T Agent}/.env`:
   ```
...
```yaml
# In ~/.Seple T Agent/config.yaml under mcp_servers:
mcp_servers:
  siyuan:
    command: npx
