...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/research/parallel-cli` |
...
This is an optional third-party workflow, not a Seple T Agent core capability.

Important expectations:
- Parallel is a paid service with a free tier, not a fully free local tool.
- It overlaps with Seple T Agent native `web_search` / `web_extract`, so do not prefer it by default for ordinary lookups.
...
Prefer Seple T Agent native `web_search` / `web_extract` for quick one-off lookups when Parallel is not specifically requested.
...
7. Prefer Seple T Agent native tools unless the user wants Parallel specifically or needs Parallel-only workflows.
...
Recommended Seple T Agent workflow:
...
## Recommended Seple T Agent usage patterns
...
- Do not silently choose Parallel when Seple T Agent native tools are already sufficient.
