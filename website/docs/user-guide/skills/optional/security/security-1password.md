...
| | |
|---|---|
| Source | Optional — install with `Seple T Agent skills install official/security/1password` |
| Path | `optional-skills/security/1password` |
| Version | `1.0.0` |
| Author | arceus77-7, enhanced by Seple T Agent |
| License | MIT |
...
- `tmux` available for stable authenticated sessions during Seple T Agent terminal calls (desktop app flow only)
...
### Service Account (recommended for Seple T Agent)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HERMES_HOME:-~/.Seple T Agent}/.env` (the skill will prompt for this on first load).
...
## Seple T Agent Execution Pattern (desktop app flow)

Seple T Agent terminal commands are non-interactive by default and can lose auth context between calls.
