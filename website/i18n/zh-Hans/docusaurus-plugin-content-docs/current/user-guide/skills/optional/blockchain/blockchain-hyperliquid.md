...
| | |
|---|---|
| 来源 | 可选 — 通过 `Seple T Agent skills install official/blockchain/hyperliquid` 安装 |
| 路径 | `optional-skills/blockchain/hyperliquid` |
| 版本 | `0.1.0` |
| 作者 | Hugo Sequier (Hugo-SEQUIER), Seple T Agent |
| 许可证 | MIT |
...
脚本从 `~/.Seple T Agent/.env` 读取两个可选默认值：
...
辅助脚本：`~/.Seple T Agent/skills/blockchain/hyperliquid/scripts/hyperliquid_client.py`
...
对于 `state`、`spot-balances`、`fills`、`orders` 和 `review`，当 `~/.Seple T Agent/.env` 中设置了 `HYPERLIQUID_USER_ADDRESS` 时，地址参数为可选。
