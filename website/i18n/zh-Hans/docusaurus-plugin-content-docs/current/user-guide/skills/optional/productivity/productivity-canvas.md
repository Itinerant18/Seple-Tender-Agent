...
| | |
|---|---|
| 来源 | 可选 — 通过 `Seple T Agent skills install official/productivity/canvas` 安装 |
...
4. 为 token 命名（例如 "Seple T Agent"），设置可选的过期时间，然后点击 **Generate Token**
5. 复制 token 并添加到 `~/.Seple T Agent/.env`：
