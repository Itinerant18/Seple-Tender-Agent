...
def render_skill_page(
    ...
    hermes_meta = (fm.get("metadata") or {}).get("Seple T Agent") or {}
    ...
    if meta["source_kind"] == "bundled":
        ...
        info_rows.append(
            (
                "Source",
                "Optional — install with `Seple T Agent skills install official/"
                + meta["category"]
                + "/"
                + meta["slug"]
                + "`",
...
def build_catalog_md_bundled(entries: list[tuple[dict[str, Any], dict[str, Any]]]) -> str:
    ...
    lines = [
        ...
        'description: "Catalog of bundled skills that ship with Seple T Agent"',
        "---",
        "",
        "# Bundled Skills Catalog",
        "",
        "Seple T Agent ships with a large built-in skill library copied into `~/.Seple T Agent/skills/` on install. Each skill below links to a dedicated page with its full definition, setup, and usage.",
        "",
        "Seple T Agent also syncs bundled skills on `Seple T Agent update`, but the sync manifest respects local deletions and user edits. If a skill listed here is missing from your profile's `~/.Seple T Agent/skills/` tree, it is still shipped with Seple T Agent; restore it with `Seple T Agent skills reset <name> --restore`.",
        "",
